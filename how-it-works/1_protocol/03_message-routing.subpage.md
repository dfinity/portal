...
Message routing includes a determinstic *scheduler* that schedules messages in the induction pool for execution and invokes the execution of to-be-scheduled messages.
At this point, control is handed over to the *execution layer*, a virtual machine responsible for the execution of canister messages and the topmost layer of the core IC protocol stack.
Execution is deterministic, thus the state of the node is changed in the same way on every (honest) node of the subnet, which is crucial for achieving the replicated state machine properties of a subnet.
...
A remote message is placed into the outgoing *subnet queue* for the target subnet once it results from the execution of a canister message.
The xnet messages in the queue are certified (signed) using a Merkle-tree-style data representation, at the end of the round by the subnet using BLS threshold cryptography as part of the per-round state certification.
...
Thanks to using a Merkle-tree-like xnet queue, parts of the queue can be consumed in a round by the receiving subnets and signatures can still be validated.
...
Certification is always done using BLS threshold signatures computed collectively by the subnet, thus certifications are computed by the subnet as a whole in a decentralized manner.
...
At every checkpointing interval, that is, every multiple hundred rounds, which corresponds to around 10 minutes, the subnet computes a so-called checkpoint at which the state of a subnet is certified.
The state certification is done incrementally by incorporating the content of all pages changed since the last checkpoint certification into the Merkle state tree and propagating the changes up the tree.
Finally, the root of the Merkle tree and additional metadata is signed with a BLS threshold signature by the subnet, thereby certifying every leaf of the Merkle tree.
The signed result is called a *catch-up package* as it can be used by nodes to efficiently catch up to the point in time when the checkpoint was made.
...
The run time of this checkpointing operation is linear in the number of changed pages and not the overall state size on the subnet.
...
As described above, every checkpointing interval, i.e., around every 10 minutes, the complete subnet state is certified by the subnet through a BLS threshold signature on a Merkle tree and made available as a catch-up package.
One prominent use case of a catch-up package is to allow new nodes to join a subnet, e.g., if the subnet is to grow in size or a node needs to be replaced because of having been destroyed in a disaster, or if a node has to catch-up because it was down for some time.
...
Every chunk is authenticated through the catch-up package individually through its hash.
