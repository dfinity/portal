...
The [Bitcoin mainnet](/docs/current/building-dapps/chain-fusion/bitcoin/overview) system canister is used to facilitate connection from the ICP mainnet to the Bitcoin mainnet.
...
The [Bitcoin testnet](/docs/current/building-dapps/chain-fusion/bitcoin/overview) system canister is used to facilitate connection from the ICP mainnet to the Bitcoin testnet.
...
The [cycles ledger](/docs/current/developer-docs/defi/cycles/cycles-ledger) canister is used for cycles management and has been developed as an alternative to the cycles wallet. The cycles ledger allows principals to hold cycles directly rather than requiring a separate cycles wallet canister to be used.
...
The [cycles ledger index](/docs/current/developer-docs/defi/cycles/cycles-ledger) canister is the corresponding index canister for the cycles ledger.
...
The [cycles minting](/docs/current/developer-education/developer-concepts/gas-cost) canister is used to mint cycles on the mainnet.
...
The [ICP index](/docs/current/rosetta-defi/token-indexes/) canister is the corresponding index canister for the ICP ledger.
...
The [ICP ledger](/docs/current/rosetta-defi/token-indexes/) canister is used for sending and receiving ICP tokens on the mainnet. It can also be used to obtain account information, such as the account's current balance of ICP.
...
The [ICP ledger archive](/docs/current/rosetta-defi/token-indexes/) canisters are used to store historic data about ICP token transactions. There are two ICP ledger archive canisters.
...
The [Internet Identity](/docs/current/building-dapps/authentication/overview) canister is used to provide an authentication method used across ICP applications, such as the NNS dapp.
...
View the [Internet Identity specification](/docs/current/references/ii-spec).

## NNS canisters

The NNS is comprised of the following canisters:

- [NNS governance](/docs/current/move-to-learnhub/nns/overview): Canister ID [`rrkah-fqaaa-aaaaa-aaaaq-cai`](https://dashboard.internetcomputer.org/canister/rrkah-fqaaa-aaaaa-aaaaq-cai)
- [NNS lifeline](/docs/current/move-to-learnhub/nns/overview): Canister ID [`rno2w-sqaaa-aaaaa-aaacq-cai`](https://dashboard.internetcomputer.org/canister/rno2w-sqaaa-aaaaa-aaacq-cai)
- [NNS registry](/docs/current/move-to-learnhub/nns/overview): Canister ID [`rwlgt-iiaaa-aaaaa-aaaaa-cai`](https://dashboard.internetcomputer.org/canister/rwlgt-iiaaa-aaaaa-aaaaa-cai)
- [NNS root](/docs/current/move-to-learnhub/nns/overview): Canister ID [`r7inp-6aaaa-aaaaa-aaabq-cai`](https://dashboard.internetcomputer.org/canister/r7inp-6aaaa-aaaaa-aaabq-cai)
- [NNS UI](/docs/current/move-to-learnhub/nns/overview): Canister ID [`qoctq-giaaa-aaaaa-aaaea-cai`](https://dashboard.internetcomputer.org/canister/qoctq-giaaa-aaaaa-aaaea-cai)
...
The [SNS Wasm](/docs/current/building-dapps/governing-dapps/overview) canister is used for deploying and updating SNSes.
...
For example, Internet Identity can be integrated into a dapp to provide user authentication service to the project. Internet Identity is commonly integrated into a dapp through the [`dfx deps`](/docs/current/building-dapps/interacting-with-dapps/using-third-party-canisters) command. `dfx deps` allows you to pull a system canister into your local project for testing.
