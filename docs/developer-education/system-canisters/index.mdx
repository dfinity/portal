...
For example, Internet Identity can be integrated into a dapp to provide user authentication service to the project. Internet Identity is commonly integrated into a dapp through the [`dfx deps`](/docs/current/building-dapps/creating-dapps/using-third-party-canisters) command. `dfx deps` allows you to pull a system canister into your local project for testing.
