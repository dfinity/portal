...
The [ICP index](/docs/current/rosetta-defi/tokens/indexes) canister is the corresponding index canister for the ICP ledger.
...
The [ICP ledger](/docs/current/rosetta-defi/tokens/indexes) canister is used for sending and receiving ICP tokens on the mainnet. It can also be used to obtain account information, such as the account's current balance of ICP.
...
The [ICP ledger archive](/docs/current/rosetta-defi/tokens/indexes) canisters are used to store historic data about ICP token transactions. There are two ICP ledger archive canisters.
