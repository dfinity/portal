...
[Learn more about writing canister code](/docs/current/building-dapps/launching-a-dapp/write/overview).

## Create

A developer can create an empty canister on ICP by calling the system canister.
It can be done using [`dfx canister create`](/docs/current/building-dapps/developer-tools/dfx/dfx-canister#dfx-canister-create) in the command line or the [<GlossaryTooltip>NNS</GlossaryTooltip> frontend dapp](/docs/current/move-to-learnhub/nns/using-the-nns-dapp/nns-app-quickstart#deploying-and-managing-canisters-from-the-nns-dapp) in the browser.
...
[Learn more about creating canisters](/docs/current/building-dapps/launching-a-dapp/create).
...
Once an empty canister has been created, the canister's code must be compiled into a Wasm binary using [`dfx build`](/docs/current/building-dapps/developer-tools/dfx/dfx-build).

[Learn more about compiling code into Wasm](/docs/current/building-dapps/launching-a-dapp/compile).

## Install

The next step after writing the code, creating a canister, and compiling the code is installing the code into the canister.
...
The [`dfx canister install`](/docs/current/building-dapps/developer-tools/dfx/dfx-canister#dfx-canister-install) command is used for making this call.
...
Once a canister has an installed Wasm binary, it can be deployed to the local, playground, or mainnet environments with [`dfx deploy`](/docs/current/building-dapps/developer-tools/dfx/dfx-deploy).

[Learn more about deploying canisters](/docs/current/building-dapps/launching-a-dapp/deploy/overview).
...
[Learn more about different types of controllers](/docs/current/building-dapps/managing-dapps/control).
