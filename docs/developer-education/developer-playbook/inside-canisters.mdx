...
## Canister storage
There are two types of [storage](/docs/current/building-dapps/managing-dapps/storage) available to the canister: the Wasm memory and the stable memory.
...
## Cycles balance
Canisters pay for execution in [cycles](/docs/current/developer-education/developer-concepts/gas-cost).
The cost of execution depends on the number and type of executed Wasm instructions.
The system charges this cost from the cycles balance of the canister for both successful and failed executions.

## Controllers
Only [controllers](/docs/current/building-dapps/managing-dapps/control) of the canister can do certain system-level operations on the canister, such as stopping it, starting it, upgrading it, etc.
