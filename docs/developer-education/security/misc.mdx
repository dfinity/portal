...
#### Caveats:
- The `setup_randomness` must **always** be initialized in both the `init` and `post_upgrade` hook as `init` [is not invoked during an canister upgrade](/docs/current/references/ic-interface-spec#system-api-upgrades).
