...
### 2. Using `raw_rand` as a seed for a pseudo-random number generator (PRNG)
...
#### Caveats:
- The `setup_randomness` must **always** be initialized in both the `init` and `post_upgrade` hook as `init` [is not invoked during a canister upgrade](/docs/current/references/ic-interface-spec#system-api-upgrades).
...
Make canister builds reproducible. See this [recommendation](https://mmapped.blog/posts/01-effective-rust-canisters.html#reproducible-builds) (from [effective Rust canisters](https://mmapped.blog/posts/01-effective-rust-canisters.html)). See also the [developer docs on this](/docs/current/developer-education/developer-cookbook/developer-best-practices/reproducible-builds).
