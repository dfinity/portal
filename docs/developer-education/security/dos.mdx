...
Some calls (update or query) might be expensive in terms of the memory or cycles they consume. For example, any function using chain-key signing or HTTPS outcalls is relatively expensive. See the [additional documentation](https://internetcomputer.org/docs/current/developer-education/developer-concepts/gas-cost#special-features) on cycles cost details and for other examples.
