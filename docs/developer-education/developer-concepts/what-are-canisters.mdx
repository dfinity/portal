...
ICP has its own ABI language called **[Candid](/docs/current/building-dapps/calling-dapps/candid/candid-concepts)**, which is similar to JSON or Protobuf, but tailored for ICP.
...
3. Write an off-chain program that uses an [agent library](/docs/current/building-dapps/calling-dapps/agents/overview) to send messages to the canister.
