...
- Facilitating the network's governance through neuron staking. You'll dive deeper into staking tokens in [4.4 NNS governance and staking](/docs/current/tutorials/developer-liftoff/level-4/4.4-nns-governance).

<<<<<<< HEAD
- Creating cycles by converting ICP into cycles, which are then used to pay for canister computation and resources. The developer ladder briefly covered converting ICP into cycles in [1.4 Acquiring and using cycles](/docs/current/tutorials/developer-liftoff/level-1/1.4-using-cycles).
=======
- Creating cycles by converting ICP into cycles, which are then used to pay for canister computation and resources. The developer liftoff briefly covered converting ICP into cycles in [1.4 Acquiring and using cycles](/docs/current/tutorials/developer-liftoff/level-1/1.4-using-cycles).
>>>>>>> 4b60b8079175c13f1f9cb2b458951a9fdde4b3c5

- Rewarding NNS participants by providing ICP tokens to users that stake ICP in neurons and actively participate in voting on proposals. You'll dive deeper into NNS rewards in [4.4 NNS governance and staking](/docs/current/tutorials/developer-liftoff/level-4/4.4-nns-governance).
...
The ICP ledger can only be used to interact with ICP tokens; to interact with other tokens, such as ICRC tokens, an ICRC ledger will need to be used. This ledger is covered in the module [4.2 ICRC tokens](/docs/current/tutorials/developer-liftoff/level-4/4.2-icrc-tokens).
