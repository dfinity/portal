...
1. ICP tokens are converted into cycles, which are then used to pay for canister computation and resources. This process was covered in [1.4 Acquiring and using cycles](/tutorials/developer-liftoff/level-1/1.4-using-cycles).

2. NNS participants are rewarded in ICP tokens for staking ICP in neurons and actively participating in voting. You'll dive deeper into NNS rewards in [4.4 NNS governance and staking](/tutorials/developer-liftoff/level-4/4.4-nns-governance).
...
The ICP ledger can only be used to interact with ICP tokens; to interact with other tokens, such as ICRC tokens, an ICRC ledger will need to be used. That ledger is covered in the module [4.2 ICRC tokens](/tutorials/developer-liftoff/level-4/4.2-icrc-tokens).
...
The ICP ledger canister running on the mainnet is not meant to be used for other token deployments. To develop a new token or ledger, the ICRC ledger can be used, which will be covered in a [future module](/tutorials/developer-liftoff/level-4/4.2-icrc-tokens).
...
:::info
`MEMO` refers to a 64-bit numeric value as defined in the [IC specification](/references/ledger#_transferring_tokens). It is used to generate unique hash values for transactions.
:::
...
[Learn how to use `nns-js` to interact with the ledger from the web application](/defi/overview).

[Learn how to use the `ic-cdk` to make inter-canister calls to the ICP ledger](/defi/overview).
