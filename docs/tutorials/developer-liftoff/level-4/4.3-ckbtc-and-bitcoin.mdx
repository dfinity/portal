...
ICP's direct integration with Bitcoin uses a novel protocol for chain-key signatures based on **threshold signatures**. [Threshold ECDSA](/references/t-sigs-how-it-works/) and threshold Schnorr are protocol suites that include several features like [key generation](https://csrc.nist.gov/glossary/term/key_generation), [periodic key re-sharing](https://www.youtube.com/watch?v=gKUi-2T7tdc), [computing pre-signatures](/references/t-sigs-how-it-works/#:~:text=Computing%20pre%2Dsignatures%2C%20signing%3A,requests%20to%20precompute%20pre%20signatures), [XNet key re-sharing](/references/t-sigs-how-it-works/#:~:text=XNet%20key%20re,source%20subnet%20uses), and public key retrieval.
...
The Developer Liftoff will dive deeper into threshold signatures in a future tutorial. You can also take a deep dive in the [technology overview to learn more](/references/t-sigs-how-it-works/).

On the protocol level, the [Bitcoin integration](https://learn.internetcomputer.org/hc/en-us/articles/34211154520084-Bitcoin-Integration) and chain-key signatures each expose an API on the management canister. While the Bitcoin API is specific to the Bitcoin network, the chain-key ECDSA and Schnorr signature APIs can be used for other workflows, such as the integration with other blockchains like Ethereum.
...
Then, start the local development environment with the command:

```bash
dfx start --clean
```

Once the local development environment has been started, it should return output that indicates it has connected to the local Bitcoin node:
...
- [ckBTC developer documentation page](/defi/chain-key-tokens/ckbtc/overview).
