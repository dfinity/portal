...
:::info
The Motoko core library provides an **Array** module for immutable arrays and a **VarArray** module for mutable arrays. Import them with:

`import Array "mo:core/Array";` and `import VarArray "mo:core/VarArray";`
:::
...
To declare a mutable array with a dynamic size, such that the number of entries within the array can be updated, use the **VarArray** module from the core library (in core, `Array.init` was removed; use `VarArray` for mutable arrays):
...
actor {
  let size : Nat = 10;
  let x : [var Nat] = VarArray.tabulate<Nat>(size, func(_) = 5);
}
```
...
Using this syntax, each entry can be updated via an assignment to that individual entry. In this example, the index entry of `2` gets updated from `5` to `42`.
...
`canister_inspect_message` is not used for HTTP query calls, inter-canister calls, or calls made to the management canister. Please make sure to get familiar with the [canister development security best practices](/building-apps/security/iam#do-not-rely-on-ingress-message-inspection) before using this feature in production, because `canister_inspect_message` is not a secure way of implementing e.g. authorization checks.
...
[Learn more about working with `Option` and `Result` types](/motoko/error-handling#working-with-optionresult).
...
- [Motoko mutable state](/motoko/state).

- [Motoko message inspection](/motoko/messaging).

- [Local object classes](/motoko/fundamentals/types/objects-classes).

- [Motoko error handling](/motoko/error-handling).
