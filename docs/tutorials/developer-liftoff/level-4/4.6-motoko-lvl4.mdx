---
keywords: [advanced, tutorial, developer ladder, motoko]
---
...
In this final Motoko module of the developer ladder series, you'll cover the following Motoko concepts and components:
...
actor {
  let textImmutable  : Text = "developer ladder";
...
The only difference between these two variable declarations is the use of `let` for the immutable variable and `var` for the mutable variable. However, these variables contain much different meanings, especially in the context of larger projects and programs.
...
- Object classes: Object classes refer to blueprints used to define a series of related objects with customizable states. Motoko uses a class definition that simplifies building objects of the same implementation and type.
...
Now let's move on to object classes. An object class is a package of entities that share a common name. For example, let's define a class example for counters that start at zero:
...
When a canister receives an ingress message submitted through HTTP, the canister can inspect and either accept or decline the message before executing it. For example, when a canister receives an update call, it uses the canister method `canister_inspect_message` to determine if the message will be accepted. If a canister does not contain a Wasm module, the message will be rejected automatically. If the canister is not empty, but it does not implement the `canister_inspect_message` method, then the ingress method will be accepted. `canister_inspect_message` is not used for HTTP query calls, inter-canister calls, and calls made to the management canister. Please make sure to get familiar with the [canister development security best practices](/docs/current/developer-docs/security/security-best-practices/iam/) before using this feature in production, because `canister_inspect_message` is not a secure way of implementing e.g. authorization checks.
...
[Learn more about working with `Option` and `Result` types](/docs/current/motoko/main/writing-motoko/errors#working-with-optionresult).
...
- [Motoko mutable state](/docs/current/motoko/main/writing-motoko/mutable-state).

- [Motoko message inspection](/docs/current/motoko/main/writing-motoko/message-inspection).

- [Local object classes](/docs/current/motoko/main/writing-motoko/local-objects-classes).

- [Motoko error handling](/docs/current/motoko/main/writing-motoko/errors).
...
- [Developer ladder forum discussion](https://forum.dfinity.org/t/developer-liftoff-feedback-and-discussion/23893).
...
That'll wrap up level 4 of the developer ladder! Next, you'll dive into the final level, level 5:

- [5.1 Developing an encrypted notes dapp with vetKD.](/docs/current/tutorials/developer-liftoff/level-5/5.1-vetKeys-tutorial)