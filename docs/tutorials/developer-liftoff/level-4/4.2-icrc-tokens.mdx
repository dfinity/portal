...
Next, you'll use the `minter` identity you created in the last module, [4.1 Using the ICP ledger](/tutorials/developer-liftoff/level-4/4.1-icp-ledger). If you need to export the `minter` identity's principal again, run the commands:
...
Then, set the identity that you want to use to deploy the ledger. In the previous module [4.1 Using the ICP ledger](/tutorials/developer-liftoff/level-4/4.1-icp-ledger), you used the `DevLiftoff` identity, which you'll use again:
