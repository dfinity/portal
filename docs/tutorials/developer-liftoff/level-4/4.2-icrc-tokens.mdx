---
keywords: [advanced, tutorial, developer liftoff, icrc tokens, icrc ledger]
---
...
Use the following URL to download the Candid file: `https://raw.githubusercontent.com/dfinity/ic/<VERSION>/rs/ledger_suite/icrc1/ledger/ledger.did`. In this tutorial, you'll use the URL https://raw.githubusercontent.com/dfinity/ic/aba60ffbc46acfc8990bf4d5685c1360bd7026b9/rs/ledger_suite/icrc1/ledger/ledger.did.
...
Next, you'll use the `minter` identity you created in the last module, [4.1 Using the ICP ledger](/docs/current/tutorials/developer-liftoff/level-4/4.1-icp-ledger). If you need to export the `minter` identity's principal again, run the commands:
...
Then, set the identity that you want to use to deploy the ledger. In the previous module [4.1 Using the ICP ledger](/docs/current/tutorials/developer-liftoff/level-4/4.1-icp-ledger), you used the `DevJourney` identity, which you'll use again:
