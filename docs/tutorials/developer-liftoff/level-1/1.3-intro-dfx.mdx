...
This structure looks very similar to the [ICP Ninja example](../level-0/06-first-dapp.mdx) but there are a few differences:
...
New Motoko projects created by `dfx` will always include a default, template `main.mo` file. To take a look at the file's default contents, open the `src/hello_world_backend/main.mo` file in a code or text editor. The code will resemble the following:
...
This 'Hello, world!' program is a simplified version of the project deployed via [ICP Ninja](https://icp.ninja) in the previous module [0.6: Deploying your first dapp](../level-0/06-first-dapp.mdx).
...
Created a wallet canister on the "local" network for user "DevJourney" with ID "bnz7o-iuaaa-aaaaa-qaaaa-cai"
...
Once the canister has been deployed, you can open the Candid interface URL, which will resemble the interface that you used to interact with [deployed dapp in module 0.6: Deploying your first dapp](../level-0/06-first-dapp.mdx) through ICP Ninja.
