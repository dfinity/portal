...
A developer environment consists of tools and packages that are required to develop code projects. Usually, developer environments are stored and hosted on your local computer, but there are some situations where a virtual, web-based development environment can be used. An example of this is [ICP Ninja](https://icp.ninja) used in previous modules, which is a web-based, virtual developer environment that can be used by developers without having to set up a local environment.

:::info
The majority of modules in this tutorial series will use ICP Ninja for accessibility purposes. If you'd like to continue using it as your development environment, you can skip this module. Recall that ICP Ninja projects are only deployed for 20 minutes. That means for long-term development on ICP, you will need a local environment set up with the tools detailed on this page.
:::
