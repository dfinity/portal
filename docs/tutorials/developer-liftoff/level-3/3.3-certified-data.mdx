...
In a previous module, [2.2: Advanced canister calls](/tutorials/developer-liftoff/level-2/2.2-advanced-canister-calls), you briefly learned about certified variables. In this guide, you'll dive deeper into certified data and take a look at an example that displays how to use certified data.
...
3. Lastly, a [`#IC-Certificate`](https://learn.internetcomputer.org/hc/en-us/articles/34276431179412-Asset-Certification) header is added to each certified HTTP response.
...
- [`ic0.certified_data_set : (src: i32, size : i32) -> ()`](/references/ic-interface-spec#system-api-certified-data): This method can be used to update the canister's certified data. Data cannot be larger than 32 bytes.

- [`ic0.data_certificate_present : () -> i32`](/references/ic-interface-spec#system-api-certified-data): This method returns a `1` if a certificate is present or a `0` if not.

- [`ic0.data_certificate_size : () -> i32`](/references/ic-interface-spec#system-api-certified-data) and [`ic0.data_certificate_copy : (dst: i32, offset: i32, size: i32) -> ()`](/references/ic-interface-spec#system-api-certified-data): These methods are used to copy the certificate for the current value of the certified data to the canister.
...
When a canister would like to return a certified asset to a request, the response body will contain the asset and a response header with the name [`IC-Certificate`](https://learn.internetcomputer.org/hc/en-us/articles/34276431179412-Asset-Certification) and a value equal to the asset's certificate.

More information can be found in the [HTTP gateway protocol specification](/references/http-gateway-protocol-spec).
