...
Currently, there are two agents developed by DFINITY: the [ICP JavaScript/TypeScript](/building-apps/interact-with-canisters/agents/javascript-agent) agent and the [ICP Rust](/building-apps/interact-with-canisters/agents/overview) agent.
...
In JavaScript, type `Text` maps to type `String`. You can see a full mapping list of Candid types and their JavaScript equivalents in the [Candid types reference](/references/candid-ref).
...
That'll wrap things up for this module! You can read more about agents, such as the [Node.js agent](/building-apps/interact-with-canisters/agents/overview).
