...
Then, select the 'Sign in' button to authenticate with a local Internet Identity. Need a reminder on how to create an Internet Identity? Review the [3.5 Identities and authentication](/tutorials/developer-liftoff/level-3/3.5-identities-and-auth) level of the Developer Liftoff. Once authenticated, your II principal will be shown in the dapp's UI:
