---
keywords: [intermediate, tutorial, developer ladder, canister upgrades, canister storage, data persistence]
---
...
- Any stable variables are re-loaded as part of the `post_upgrade` hook. Note: Having a `pre_upgrade` hook is strongly discouraged and not best practice. After the stable variables are re-loaded, the stable memory bytes that stored those variables are overwritten with zeroes to minimize stable memory costs for the canister.
...
:::info
Upgrading canisters written in Rust and other languages use a different workflow which incorporates serialization of the canister's data. [Learn more about Rust upgrades](/docs/current/developer-docs/backend/rust/upgrading).
:::
