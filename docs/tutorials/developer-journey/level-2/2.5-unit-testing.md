...
:::caution
We'll be diving a bit into using an **agent** in this portion of the tutorial. Note that we've briefly mentioned agents thus far in our developer journey, but we haven't fully explored agents yet. We will cover agents in a future tutorial, but for this tutorial it is important to know that an agent a library used to make calls to the Internet Computer public interface.
:::
...
Next let's spin up the canister and run our test with the commands:
