...
You'll notice that this project structure is a bit different from the ones you've used thus far on the developer ladder. Since this project uses two backend canisters for it's functionality, there aren't `src` folders for a backend or frontend canister, just two folders for each backend canister.
