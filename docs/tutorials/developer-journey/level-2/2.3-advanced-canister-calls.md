...
- **Composite queries** are query calls that can only be invoked via ingress messages using `dfx` or through an agent such as a browser front-end.
...
Query calls are used to query the current state of a canister or make a call to a method that operates on the canister's state without making any changes to the canister's state, making them essentially 'read-only' operations. Query calls can be made to any node that holds the canister, since the result does not go through consensus. When a query call is submitted, it is executed synchronously and answered as soon as it is received by the node. Query calls can also be used to retrieve data that is stored in a canister's [stable memory](2.1-storage-persistence.md). 

Query calls return results faster than update calls, setting functions as query functions where appropriate can be an effective way to improve application performance. When designing your dapp, it is important to notice where query calls can be used in place of functions that perform queries. However, the trade-off of increased performance is decreased security, since queries do not go through consensus and are not reflected in the transactions of the blockchain. 

The amount of security your dapp needs will be dependent on your dapp's use case and functionality. For example, a blog dapp that uses a function to retrieve articles matching a tag don't need to go through consensus and can benefit from using query calls. In contrast, a dapp that retrieves sensitive information, such as financial data, would greatly benefit from increased security and validation that the query result is accurate and secure. 

As an alternative to query calls that may return untrustworthy or inaccurate, the IC supports **certified variables**, which we'll dive into next. 
...
Update calls are used to alter the state of the canister. Any changes made with an update call are persisted. Update calls are submitted to all nodes on a subnet, not just the node hosting a canister. This is because update calls must go through consensus on the subnet to return a result to the call. Since update calls must go through consensus, they are answered asynchronously. 
...
A composite query call is a type of query that can only be invoked via an ingress message, such as one generated by an agent such as a wen browser, or through `dfx`. Composite queries can be used for scenarios such as frontend code that wants to get information from the backend code. A great example of this is a partitioned key-value store setup, where there is one frontend canister and several different backend canisters, with each backend representing a partition of the key-value stores. Composite queries can be very useful by providing efficient data retrieval for dapps that scale vertically with data partitioned across multiple canisters.

:::info
Composite queries are supported in `dfx` versions `0.15.0 and newer`. 
:::

If you want to dive a bit deeper, you can take a look at the DFINITY example project using composite queries [here](https://github.com/dfinity/examples/tree/master/motoko/cert-var). 


## Inter-canister calls

Inter-canister calls refer to the ability to make calls between different canisters. This functionality is crucial for developers building complex dapps, as it enables you to reuse and share functionality between multiple canisters within the dapp. 

For example, consider a scenario where you want to create a social media dapp that includes functionality for organizing events, making posts, and making calls. The dapp might include social profiles for each user. When creating this dapp, you may create a single canister for storing these social profiles, then another canister that addresses event organization, and a third canister that handles social posts. By isolating the social profiles into one canister, you can create endless canisters that make calls to the social profile canister, allowing your dapp to continue to scale. 

To demonstrate this inter-canister call functionality, we're going to use an example known as a 'PubSub'. In this example, we have two canisters: a **publisher** and a **subscriber**. The publisher canister publishes a key-value pair for a 'topic' to any canister that is 'subscribed' to it. It publishes this value to the subscribed canisters using an inter-canister call. The subscriber canister sends an inter-canister call to the publisher canister to subscribe to one of it's topics, then whenever a value is 'published' to that topic, it receives it as an inter-canister call. 
...
actor Publisher {
  ...
  // Create a new type called 'Subscriber' that stores a key-value pair of 'topic: callback'. Callback refers to the inter-canister call that sends the 'Counter' key-value pair to canisters in the 'Subscriber list'.  
  type Subscriber = {
    topic : Text;
    callback : shared Counter -> ();
  };

  // Define a stable variable that stores the list of canisters 'subscribed' to the Publisher canister's callbacks. 
  ...
  // Define the function to send the inter-canister call containing the `Counter` key-value store to any canisters that have subscribed to a topic. 
