...
A composite query call is a type of query that can only be invoked via an ingress message, such as one generated by an agent such as a web browser, or through `dfx`. Composite queries can be used for scenarios such as frontend code that wants to get information from the backend code. A great example of this is a partitioned key-value store setup, where there is one frontend canister and several different backend canisters, with each backend representing a partition of the key-value stores. Composite queries can be very useful by providing efficient data retrieval for dapps that scale vertically with data partitioned across multiple canisters.

:::info
Composite queries are supported in `dfx` versions `v0.15.0 and newer`. 
:::

If you want to dive a bit deeper, you can read more on the developer blog [here](https://internetcomputer.org/blog/features/composite-query) or you can take a look at the DFINITY example project using composite queries [here](https://github.com/dfinity/examples/tree/master/motoko/cert-var). 
