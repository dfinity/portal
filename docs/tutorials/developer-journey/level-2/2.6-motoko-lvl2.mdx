---
keywords: [intermediate, tutorial, developer ladder, motoko]
---
...
As you've seen so far, **actors** are at the core functionality of canisters written in Motoko. In this tutorial, you'll explore actors further by taking a look at actor type definitions, how actors interact with async data, actor classes, and using multiple actors.
...
Want to learn more about actor classes? Take a look at the documentation on [actor class management](/docs/current/motoko/main/reference/language-manual#actor-class-management) for more information.
...
As you've seen in previous portions of the developer ladder, when a new project is created with `dfx`, a default `dfx.json` file is created in the project's directory. For this guide, you'll need to edit `dfx.json` to specify our two canisters and the location of their Motoko files.
...
```bash
dfx canister call character_count test '("developer ladder")'
```
...
That'll wrap up level 2 of the developer ladder! In the next level, you'll start by taking a look at using package managers for Motoko.

- [3.1 Using libraries with MOPS](/docs/current/tutorials/developer-journey/level-3/3.1-package-managers).
