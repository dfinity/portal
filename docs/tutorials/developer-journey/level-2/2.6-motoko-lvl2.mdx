...
A shared function's arguments and return value(s) must also be shared types. This is a subset of data types that includes shared function specifications, actor specifications, and immutable data, but does not include specifications to local functions or mutable data. Since interactions with actors are asynchronous, an actor's functions must return types in the form `async T`, where T is the data type.
