...
There are several terms associated with memory and storage on the IC. To avoid confusion between them, let's define them here. 
...
Motoko supports preserving a canister's state using the IC's stable memory through a Motoko-specific feature known as **stable storage**, which is designed to accommodate changes to both the application data and the Motoko compiler. Stable storage utilizes the IC's **stable memory** feature that was discussed previously. 
...
To upgrade our canister, first you need to deploy our initial version of the canister. Deploy the canister with the command:
...
:::info
For this tutorial, you're using the local replica environment to deploy our canisters. You can deploy yours on the mainnet with the flag `--network ic`. Remember that deploying to the mainnet will cost cycles.
:::
