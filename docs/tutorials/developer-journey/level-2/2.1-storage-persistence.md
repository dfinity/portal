...
The Internet Computer handles long-term data storage using a feature known as **stable memory**. **Stable memory** is a unique feature of the Internet Computer that defines a separate data store from the canister's regular Wasm memory data store, known as **heap storage**, since a canister's heap storage is not long-term storage and does not persist across canister upgrades; canister data and state stored in heap storage is removed when a canister is stopped, reinstalled, or upgraded. Stable memory is important since it persists across canister upgrades, has a much larger storage capacity than regular Wasm memory, and is very beneficial for vertically scaling a dapp. 

To use stable memory requires anticipating and indicating which canister data you want to be retained after a canister upgrade. For some canisters, this data might be all of the canister's data, only certain parts, or none. By default, stable memory starts empty, and can hold up to 32GB as long as the subnet the canister is running on has the available space. If a canister uses more stable memory than 32GB, the canister will trap and become unrecoverable. 

## Memory types and terms 

There are several terms associated with stable memory on the IC. To avoid confusion between them, we'll define them here. 

- **Stable memory:** Stable memory refers to the Internet Computer's long-term data storage feature. Stable memory is not language specific, and can be utilized by canisters written in Motoko, Rust, or any other language. Stable memory can hold up to 32GB if the subnet can accommodate it. 

- **Heap storage:** Heap storage refers to the regular Wasm memory data store for each canister. This storage is temporary and is not persisted across canister upgrades. Data stored in heap storage is removed when the canister is upgraded, stopped, or reinstalled. Heap storage is limited to 4GB. 
...
- **Stable variables:** Stable variables is a Motoko-specific term that refers to variables defined in Motoko that use the `stable` modifier which indicates that the value of the variable should be persisted across canister upgrades. 
...
Upgrading a canister is a key feature of the IC, since it allows canister smart contracts to persist using Wasm memory and globals that utilize the IC's stable memory feature. When a canister is upgraded, the existing state of the canister is preserved while there are changes made to the canister's code. The entire state of a canister is automatically restored before, then saved after, each message without explicit instruction. This preservation is referred to as **orthogonal persistence**.

### Motoko stable memory workflow

For canisters written in Motoko, stable memory can be utilized through the Motoko stable storage and stable variable features. These features as most notably used in the canister upgrade process. The upgrade process for Motoko canisters is as follows:
...
Before a canister is upgraded, the canister's data and state can be **serialized**, meaning they are converted from a data structure into a series of bytes. In Motoko, the bytes in stable memory that originate from stable variables are appended to the end of the stable memory through the `ExperimentalStableMemory` library. Motoko currently supports streaming out stable variables, but cannot currently stream in data, so when data is serialized the bytes are copied into the heap storage then deserialized from there. 
