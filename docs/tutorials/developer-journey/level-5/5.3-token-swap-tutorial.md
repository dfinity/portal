...
This example will use ICRC-2 tokens. To learn more about ICRC tokens, refer to the previous module [4.2: ICRC tokens](../level-4/4.2-icrc-tokens.md)
...
- The `token_a` canister: This canister is used to create `token_a`, an ICRC-2 token. This canister uses the most recent ICRC ledger Wasm and Candid files. To learn more about the ICRC Wasm and Candid files, refer to the previous module [4.2: ICRC tokens](../level-4/4.2-icrc-tokens.md)

- The `token_b` canister: The canister is used to create `token_b`, an ICRC-2 token. This canister uses the most recent ICRC ledger Wasm and Candid files. To learn more about the ICRC Wasm and Candid files, refer to the previous module [4.2: ICRC tokens](../level-4/4.2-icrc-tokens.md)
...
shared (init_msg) actor class Swap(
  ...
  public shared (msg) func withdraw(args : WithdrawArgs) : async Result.Result<Nat, WithdrawError> {
    ...
    // Since the IC uses asynchronous execution, the sender's account is debited first to ensure that the user cannot withdraw more than they have. 
...
### Export the token canisters as environmental variables
...
### Deposit tokens into the swap canister
