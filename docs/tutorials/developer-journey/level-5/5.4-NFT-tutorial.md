...
On the Internet Computer, ICP is a fungible token that can always be exchanged for ICP of the same value. However, an NFT token deployed on the IC cannot be traded 1:1 with another NFT token deployed on the IC. 
...
Some NFTs are '1 of 1' tokens, meaning they are globally unique and not part of a series of NFTs, often referred to as an NFT collection. Other times, NFT tokens are '1 of 100' or '1 of 500', meaning they are part of a larger series that several people can purchase and retain the same benefits. '1 of 1' NFTs are often used in cases of unique art or identifying documents, while '1 of 100' NFTs are often used for providing membership to exclusive websites or providing tickets to an event.  

Even if 5_000 NFTs of the same exact item are minted, such as membership NFTs to a decentralized video game or exclusive website, each token will have a unique identifier to define it among the others in the series. In some cases, each NFT in the collection will have a unique image to identify it visually; other times, each NFT will be exactly the same except for the unique token ID.
...
Currently on the IC, there are two NFT standards: DIP721 and ICRC-7.

### DIP721

The DIP721 NFT standard is designed as an Internet Computer adaptation of the ERC-721 non-fungible token standard. It is designed to provide a simple, non-ambiguous API for transferring and tracking the ownership of NFTs deployed on the IC.

DIP721 was designed to improve the existing IC token standards by implementing the following:
...
- Providing an API that closely follows the original EIP-721 standard, making the ability to import existing Ethereum contracts onto the IC more straightforward. 
...
ICRC-7 is a new standard for non-fungible tokens on the IC. Recall that "ICRC" stands for "Internet Computer Request for Comments", and is the standard created by the Internet Computer working group. An ICRC standard can be used for creating anything on the IC, not just fungible tokens such as the ICRC-1 and ICRC-2 token standards. 

The ICRC-7 standard is designed to be a minimal standard for allowing an NFT collection to be deployed on the IC. In an NFT collection, each NFT may have unique metadata information. This metadata may include a unique image, traits or tags, or a description describing the NFT. 
...
Verify that you are using the local identity that you'd like to be using for deploying and minting your NFTs. This example will showcase using the `DevJourney` identity:
...
To query all token IDs owned by your user, you can make a call to the `getTokenIdsForUserDip721` method. At this point in the tutorial, your identity only ones a single NFT. To own multiple NFTs, first mint another with the command:
...
Likewise, to query the NFT's name, you can make a call to the `nameDip721` method using the command:
