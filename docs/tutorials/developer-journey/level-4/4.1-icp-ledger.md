...
All transactions are recorded in the ledger canister as a **hashed blockchain**. A **hashed blockchain** is a blockchain instance running within a canister smart contract, in this case the ledger canister, which runs on the NNS system subnet blockchain on the mainnet. 
