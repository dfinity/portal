...
As we've covered in previous modules, ICP is the native token of the Internet Computer. ICP tokens have three key functionalities on the Internet Computer:
...
- Creating cycles by converting ICP into cycles, which are then used to pay for canister computation and resources. We'll dive deeper into NNS rewards in [4.4 NNS governance and staking](4.4-nns-governance.md).
