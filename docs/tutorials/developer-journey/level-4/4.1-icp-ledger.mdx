...
For example, in this tutorial you'll use the URL https://download.dfinity.systems/ic/08f32722df2f56f1e5c1e603fee0c87c40b77cba/canisters/ledger-canister.wasm.gz.
...
In this tutorial, you'll use the URL https://raw.githubusercontent.com/dfinity/ic/08f32722df2f56f1e5c1e603fee0c87c40b77cba/rs/rosetta-api/icp_ledger/ledger.did.
...
:::info
`MEMO` refers to a 64-bit numeric value as defined in the [IC specification](/docs/current/references/ledger#_transferring_tokens). It was originally used to generate unique hash values for transactions, however the protocol has since developed other ways of making each transaction's hash unique, such as through the use of timestamps. The same `MEMO` value can be used for different unique transactions.
:::
