...
[Mops](https://mops.one) and [Vessel](https://github.com/dfinity/vessel) are package managers for Motoko. Mops utilizes a fully on-chain package registry and ships as part of `dfx`, while Vessel must be downloaded and built separately. In this guide, we'll explore how to download and install Mops and Vessel, then how to import and use different packages. 

## What is a package manager?

A **package manager** is a collection of tools that automates installing, upgrading, configuring, and removing software packages. They help efficiently manage the dependencies of a project. In relation to Motoko, Mops and Vessel provide workflows for packages to be downloaded, upgraded, and imported into Motoko code files. Mops supports over 60 packages for Motoko, spanning several different functionalities such as utility packages, encoding packages, cryptography packages, data structure packages, and more. 

Package managers help simplify things such as update management, since they provide an easy way to update all packages within your project. 
