---
keywords: [beginner, tutorial, developer ladder, cycles, using cycles]
---
...
To obtain cycles, the Internet Computer Protocol's utility token, ICP, can be converted into cycles and transferred into a canister to be used to pay for that canister's consumed resources. Cycles have a fixed price in [XDR](https://internetcomputer.org/docs/current/developer-education/glossary/index#xdr) in order to make canister costs predictable and independent of the price of ICP. One trillion cycles always correspond to one XDR.
...
However, your developer identity principal is not the same as your account identifier that is specified in the [ledger](/docs/current/rosetta-defi/tokens/token-standards). Your identity principal and your account identifier are related, but they use different formats.
...
- **[Converting ICP tokens into cycles](/docs/current/rosetta-defi/cycles/converting_icp_tokens_into_cycles)**.
