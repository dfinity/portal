...
Alternatively, if no space is used in the argument, it can be passed without quotes, since [Candid](/docs/current/building-apps/calling-dapps/candid/candid-concepts) will infer the data type as `Text`, such as:
