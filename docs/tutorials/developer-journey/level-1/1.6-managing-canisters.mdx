---
keywords: [beginner, tutorial, developer ladder, manage canisters]
---
...
```bash
dfx canister status 5o6tz-saaaa-aaaaa-qaacq-cai
```
...
```bash
dfx canister update-settings 5o6tz-saaaa-aaaaa-qaacq-cai --reserved-cycles-limit 42
dfx canister status 5o6tz-saaaa-aaaaa-qaacq-cai
```
...
```bash
dfx ledger top-up 5o6tz-saaaa-aaaaa-qaacq-cai --amount 2.7
```
...
```bash
dfx canister stop jqylk-byaaa-aaaal-qbymq-cai
dfx canister delete jqylk-byaaa-aaaal-qbymq-cai
```
...
To confirm that the cycles were withdrawn properly, check your cycles balance again with `dfx cycles balance --network=ic`. The balance should be increased, such as:
...
```bash
dfx canister stop poll_frontend
```

Then you can run the command:

```bash
dfx canister delete poll_frontend
```
