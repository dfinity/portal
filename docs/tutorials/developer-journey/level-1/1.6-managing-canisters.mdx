...
```bash
dfx canister update-settings poll_backend --reserved-cycles-limit 42
dfx canister status poll_backend
```
...
```bash
dfx ledger top-up poll_backend --amount 2.7
```

### Using the cycles ledger

If your canister is running on the mainnet, you can top up a canister using the cycles balance associated with your identity and the cycles ledger with the command:

:::danger
This command must be run on the mainnet.
:::
