...
Recall from our previous modules that an actor is *a process with encapsulated state that communicates with other running actors*. Defining an actor is necessary for a deployed canister to communicate with othr canisters and alter it's private state. 
