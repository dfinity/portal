---
keywords: [beginner, tutorial, developer liftoff, rust, fundamentals, basics, rust basics, rust fundamentals]
---
...
- `init`: The initialization entry point of the canister.
...
Candid interface description files (`.did`) for Rust canisters must be handwritten or manually exported by first defining the `export_candid!` macro, which exposes a method called `get_candid_pointer`. Then, you must use the [`candid-extractor`](https://crates.io/crates/candid-extractor) tool to extract the canister's Candid file. This macro must be called once at the end of your canister's code outside of any query or update definitions.
...
Rust relies on the Wasm binary and the associated Candid interface file to define and expose the canister's methods. In a `dfx.json` file that defines a Rust canister (`"type": "rust"`), you will need to specify the package's name and where in the project the Candid file is located:
...
Specify the `package` name in your Cargo.toml file, such as:
...
- You cannot create threads. Instead, use [`ic-cdk::spawn`](https://docs.rs/ic-cdk/0.18.3/ic_cdk/futures/fn.spawn.html).

- You cannot sleep. Instead, use [`ic-cdk-timers`](https://docs.rs/ic-cdk/0.18.3/ic_cdk/futures/ic-cdk-timers/).

- You cannot use `Instant`. Instead, use [`ic-cdk::api::time`](https://docs.rs/ic-cdk/0.18.3/ic_cdk/api/fn.time.html).
...
- You cannot use `tokio`, use [`ic_cdk::spawn`](https://docs.rs/ic-cdk/0.18.3/ic_cdk/futures/fn.spawn.html).

- You cannot use crates that make or serve HTTP requests. Instead, use [HTTP outcalls](/docs/building-apps/network-features/using-http/https-outcalls/overview) or the [gateway API](/docs/references/http-gateway-protocol-spec/) respectively.
