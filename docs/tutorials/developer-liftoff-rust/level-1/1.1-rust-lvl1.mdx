...
Refer to the [`canister_init` specification](/references/ic-interface-spec/#entry-points) for more information.
...
Candid interface description files (`.did`) for Rust canisters must be handwritten or manually exported by first defining the `export_candid!` macro, which exposes a method called `get_candid_pointer`. If defined, the [`candid-extractor`](https://crates.io/crates/candid-extractor) tool can be used to extract the canister's Candid file. This macro must be called once at the end of your canister's code outside of any query or update definitions.
...
- You cannot create threads. Instead, use [`ic_cdk::spawn`](https://docs.rs/ic-cdk/0.18.3/ic_cdk/futures/fn.spawn.html).
...
- You cannot use crates that make or serve HTTP requests. Instead, use [HTTP outcalls](/building-apps/network-features/using-http/https-outcalls/overview) or the [gateway API](/references/http-gateway-protocol-spec/) respectively.
