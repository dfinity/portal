...
```rust reference
https://github.com/dfinity/examples/blob/master/rust/inter-canister-calls/src/caller/src/lib.rs#L62-L80
```

[Open this example in ICP Ninja to make calls and experiment with this code](https://icp.ninja/editor?g=https://github.com/dfinity/examples/tree/master/rust/inter-canister-calls).
...
While unbounded-wait calls offer simple error handling, they rely on the assumption that the callee won't trap. If it does, and the caller is waiting indefinitely, that wait can block clean upgrades of the caller canister since upgrades require all outstanding calls to complete.
...
```rust reference
https://github.com/dfinity/examples/blob/master/rust/inter-canister-calls/src/caller/src/lib.rs#L82-L99
```
...
```rust reference
https://github.com/dfinity/examples/blob/master/rust/inter-canister-calls/src/caller/src/lib.rs#L139-L181
```
