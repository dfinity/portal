...
1. Pay for processing costs: For example, using the [threshold signature API](/building-apps/network-features/signatures/t-ecdsa), where a canister signs a message using a shared cryptographic key.

2. Transfer cycles as assets: For example, as done by the [cycles ledger](/defi/token-ledgers/cycles-ledger/).
