...
While unbounded-wait calls offer simple error handling, they rely on the assumption that the callee won't trap. A callee trap produces a return value and allows the caller to continue, same as a callee response. There is a failure to upgrade risk if the call is delayed, not halted, because of either serious system load or a pathological callee that will keep making more outcalls to keep the context alive.
