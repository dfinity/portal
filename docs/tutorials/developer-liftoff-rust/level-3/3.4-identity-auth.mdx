---
keywords: [beginner, tutorial, developer liftoff, identity, authentication, integrate internet identity, implement authentication ]
---
...
- **Principals**: Generic identifiers that are used for users, canisters, and potentially other future concepts. You can learn more in the IC [specification](/references/ic-interface-spec#principal).

- **Account identifier**: The identifier associated with your ICP ledger account, as specified in the [ledger](/defi/token-standards/) specification.
...
An implementation of the WebAuthn API is used to prove your identity to Internet Identity by signing a challenge issued by Internet Identity. Once signed, Internet Identity issues 'delegations' on the user's behalf, which are signed by with the delegation's own keys. This private material never leaves the user's device, as it is stored in the local hardware securely. 

For more details on how the Internet Identity architecture works, please refer to the [official Internet Identity specification](/references/ic-interface).
...
```json title="dfx.json" file=../../../references/samples/rust/who_am_i/dfx.json
```
...
```js title="frontend/src/App.jsx" file=../../../references/samples/rust/who_am_i/frontend/src/App.jsx
```

This code does the following:

- First, it creates an `AuthClient` instance using `AuthClient.create()`. This is from the [`auth-client`](/current/developer-docs/developer-tools/off-chain/agents/javascript-agent#authentication) library used for handling authentication with Internet Identity.
...
The backend's `whoami` function is defined in `backend/lib.rs`:

```rust title="backend/lib.rs" file=../../../references/samples/rust/who_am_i/backend/lib.rs
```
