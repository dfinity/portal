...
If a canister has no controller, or if access to all controllers is lost, it cannot be upgraded, deleted, or maintained. In cases where a canister traps with an unrecoverable error and cannot be upgraded, it might still be possible to recover it using canister snapshot.
