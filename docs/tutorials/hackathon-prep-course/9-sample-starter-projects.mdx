...
ICP canister smart contracts can read information from EVM smart contracts in a secure manner through [HTTPS outcalls](https://internetcomputer.org/https-outcalls) or using the [EVM RPC](https://internetcomputer.org/docs/current/building-dapps/chain-fusion/ethereum/evm-rpc/overview) canister. Then, to submit transactions to and write to other chains, ICP uses chain-key signatures, i.e., [Threshold ECDSA](https://internetcomputer.org/docs/current/building-dapps/using-network-features/signatures/t-ecdsa).
...
-   [dfx](/docs/current/building-dapps/getting-started/install) `>= 0.18`
