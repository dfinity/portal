...
To begin developing applications on ICP, the tool [ICP Ninja](https://icp.ninja) provides a browser-based development environment that can be accessed from anywhere using any device. It is a free platform that doesn't require you to [download tools](/building-apps/getting-started/install), create or authenticate with an [identity](/building-apps/getting-started/identities), or obtain [cycles](/building-apps/getting-started/tokens-and-cycles).

ICP Ninja does, however, impose a few [restrictions](/building-apps/developer-tools/icp-ninja#limitations) that should be noted when using it. It is not designed to develop and deploy applications for long-term, production use.
...
- **Backend canister**: Used to store the application’s data and to provide endpoints to access and modify the data. Backend canisters can be written in [a variety of different languages](/building-apps/developer-tools/cdks/) (Motoko, Rust, TypeScript, and more).
...
[Learn more about canister calls](/tutorials/developer-liftoff/level-2/2.2-advanced-canister-calls).
...
Open the "Backend Internet Computer URL" in your web browser. This will open a [Candid API](/building-apps/interact-with-canisters/candid/candid-concepts) interface you can use to call the method `greet` defined in the backend.
...
Then click "Redeploy" to [upgrade](/tutorials/developer-liftoff/level-2/2.1-storage-persistence#upgrading-canisters) your canister's code. Once deployed, you can test the `greet` method in the Candid API, and it should return your new greeting.