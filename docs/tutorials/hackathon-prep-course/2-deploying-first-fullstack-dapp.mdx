...
- **Account identifier**: The identifier associated with your ICP ledger account, as specified in the [ledger specification](https://internetcomputer.org/docs/current/specifications/ledger#_accounts).
...
- [x] Download and install the IC SDK package as described in the [installing the IC SDK](/docs/current/building-apps/getting-started/install) page. The IC SDK includes the CLI tool `dfx`, which is the primary tool used for creating, managing, and deploying canisters. It also includes Motoko, which is ICP's native programming language.
