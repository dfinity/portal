---
keywords: [beginner, tutorial, hackathon, hackathon prep course, what is icp, icp, what icp, what is the internet computer, internet computer protocol, vision of icp]
---
...
The Internet Computer (ICP) is a transparent and secure blockchain network that enables developers to create and deploy fully decentralized applications. Decentralized applications are created through deploying smart contracts, which are known as **canisters**.
...
A subnet is a collection of nodes that run their own instance of the ICP consensus algorithm, essentially running their own blockchain. Each subnet hosts canister smart contracts. Each canister's code, along with its state and computation, is replicated across every node on the subnet.

Each subnet operates concurrently with and independently of the other subnets but can communicate asynchronously with other subnets. Multiple independent subnets run in parallel, allowing ICP to break through the single-machine barrier that limits traditional blockchains.

![Subnets](./_attachments/subnets.png)

### Deterministic decentralization

The term deterministic decentralization refers to a concept used to maximize the decentralization and diversity of each subnet on ICP. It measures and maximizes the decentralization of every layer of the ICP infrastructure by maximizing the number of different node providers, data centers, geographic locations, and jurisdictions. The purpose of deterministic decentralization is to ensure that the network remains diverse and decentralized.

You can read more about deterministic decentralization on the [ICP wiki.](https://wiki.internetcomputer.org/wiki/Deterministic_Decentralization#firstHeading)

### Chain-key cryptography

A core component of ICP is the protocol's advanced cryptographic mechanisms, which are collectively referred to as chain-key cryptography. Chain-key cryptographic methods enable ICP to achieve functionalities and scalability that other networks can't.
...
- Content received from ICP can be verified through validating a signature without needing to sync the entire network.
...
- Transactions that are targeted at other blockchain networks can be computed fully onchain, allowing for integrations with other chains such as Bitcoin and Ethereum.

[Learn more about chain-key cryptography](/how-it-works/chain-key-technology/).

## ICP superpowers

### Seamless user onboarding

Users can be onboarded into the ICP ecosystem seamlessly through dozens of different educational resources in all different formats, lengths, and focuses.

A few onboarding resources include:

- [ICP Ninja](https://icp.ninja): A web-based IDE with several example projects that can be deployed directly from the browser.

- [ICP.Hubs](https://linktr.ee/icp_hubs_network): Local communities dedicated to promoting adoption and awareness of ICP through education, strategic partnerships, and project acceleration.
...
ICP is able to directly integrate with other blockchain networks through the ICP's advanced cryptography. Through threshold signatures, coupled with chain-key cryptography, ICP canisters can natively sign and submit transactions on other blockchains like Ethereum and Bitcoin. This allows for dapps deployed on ICP to interact directly with other networks and their smart contracts, tokens, and other digital assets.

ICP is integrated with the Bitcoin network using chain-key ECDSA and Schnorr signatures and a protocol-level integration. A canister can create a Bitcoin address, then send or receive bitcoin directly as if they were a regular Bitcoin user. ICP also uses chain-key ECDSA to facilitate an integration with Ethereum enables Ethereum smart contracts and digital assets like ERC-20 tokens to be used in ICP canisters.

Using Bitcoin or Ethereum through ICP is faster, cheaper, and more performant than using the other networks directly.

### Future-proof

Key components of ICP's future-proof capabilities include:
...
To obtain cycles, the Internet Computer's utility token, ICP, can be converted into cycles and transferred into a canister. Cycles have a fixed price in [XDR](/docs/references/glossary#x) to make canister costs predictable and independent of the price of ICP. One trillion cycles always correspond to one XDR.
...
- **Rust**: Supported on ICP through the IC SDK and the DFINITY Rust CDK.
