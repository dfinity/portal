...
### Feature: dfx deps subcommands
The `dfx pull` command has been renamed and reworked into the `dfx deps` command. This command was reworked to create an intuitive, complete user experience, and includes a set of subcommands to help achieve that. These subcommands are:

- `dfx deps pull`: this command pulls the dependencies directly from the mainnet and generates a `deps/pulled.json` file. The associated candid files of any direct dependencies will be put into the `deps/candid/` directory. 
- `dfx deps init`: this command can be used to set the init arguments that should be used for the pulled dependencies. Any arguments set will be saved in the `deps/init.json` file. 
- `dfx deps deploy`: this command deploys the pulled dependencies onto a local replica using the init arguments stored within the `deps/init.json` file.  

As a note, it is encouraged that all generated files in the `deps/` directory be version controlled.
...
### Feature: Users are now asked for consent when they remove themselves as the principal. 
