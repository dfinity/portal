...
It’s now possible to specify that a canister in dfx.json specifications a "remote" canister on a specific network, that is, a canister that already exists on that network and is managed by some other project.
...
This way, you can define public update/query functions to aid in local testing, but when building/deploying to the mainnet, specifications to methods not found in `ledger.public.did` will be reported as compilation errors.
