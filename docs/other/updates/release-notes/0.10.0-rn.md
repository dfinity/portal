...
The local replica sets its parameters according to the [subnet type](/docs/current/building-dapps/creating-dapps/subnets/overview#subnet-blockchains) defined in defaults.replica.subnet_type, defaulting to 'application' when none is specified.
...
Example how to specify the [subnet type](/docs/current/building-dapps/creating-dapps/subnets/overview#subnet-blockchains):
```
{
  "defaults": {
    "replica": {
