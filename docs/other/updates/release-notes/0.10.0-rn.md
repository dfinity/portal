...
The local replica sets its parameters according to the [subnet type](/docs/current/concepts/nodes-subnets#subnet-blockchains) defined in defaults.replica.subnet_type, defaulting to 'application' when none is specified.
...
Example how to specify the [subnet type](/docs/current/concepts/nodes-subnets#subnet-blockchains):
```
{
  "defaults": {
    "replica": {
