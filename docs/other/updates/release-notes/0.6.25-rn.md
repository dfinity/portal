...
-   This release introduces environment variables for specifying canister identifiers.
    ...
    The environment variables are intended to replace the `import` syntax for dependent canisters described in [Entry and output configuration](/building-apps/frontends/using-an-asset-canister) which is being deprecated and will be removed in a future release.
