...
# BTC local developer environment 

<MarkdownChipRow labels={["Advanced", "Bitcoin"]} />

It is recommended that developers set up a local Bitcoin network on their machine, as it allows you to mine blocks quickly and at will, which
...
## Setting up a local Bitcoin network with `bitcoind`
...
## Using `dfx` for local Bitcoin development

:::caution
Using `dfx` for local development has several limitations compared to running a local Bitcoin node with `bitcoind`. Using `dfx`, you will make calls to the Bitcoin API that either target the Bitcoin testnet or mainnet, neither of which you will have control over and thus testing may take longer.

Using a local Bitcoin node, you will have full control over the local network's configuration and when blocks are mined, allowing you to iterate faster.
:::

You can start the local development environment with Bitcoin support enabled.

```
dfx start --enable-bitcoin --background
```

Alternatively, in your project you can edit the `dfx.json` file to include the Bitcoin configuration:
...
This indicates that `dfx` isn't able to connect to your Bitcoin node. Make sure your Bitcoin node is up and running, and you're setting the correct port (default is `18444`). The port used can be changed in your project's `dfx.json` file.
...
## Interacting with the local Bitcoin node
...
A key difference between working with a local Bitcoin node and the Bitcoin testnet or mainnet is how you receive Bitcoin. In order to receive BTC on your local Bitcoin network, you need to manually mine blocks. BTC is issued as a reward for each block mined.

:::info Coinbase maturity

One caveat of using a local Bitcoin node and receiving block rewards is that the rewards are subject to the [Coinbase maturity rule](https://github.com/bitcoin/bitcoin/blob/bace615ba31cedec50afa4f296934a186b9afae6/src/consensus/consensus.h#L19), which states that, in order for you to spend rewarded BTC, you will first need to mine 100
blocks.
