...
It is recommended that developers set up a local Bitcoin testnet on their machine, asmay occur it allows them to mine blocks quickly and at will, which facilitates testing various cases without having to rely on the Bitcoin testnet or the Bitcoin mainnet. Alternatively, you can test dapps using the Bitcoin testnet or mainnet through the ICP Bitcoin API. Both workflows are detailed below.
