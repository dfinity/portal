...
## Local Bitcoin testnet (Bitcoin regression testing mode)
...
Popular libraries like [Rust's bitcoin crate](https://crates.io/crates/bitcoin) [Motoko's bitcoin package](https://mops.one/bitcoin) can be used within ICP smart contracts.
...
```jsx
https://github.com/dfinity/examples/blob/master/rust/basic_bitcoin/dfx.json#L20-34
```
...
###  Bitcoin API canister initialization arguments

Canister initialization arguments determine the behavior of the Bitcoin API running in the local development environment.

By default, the local Bitcoin API mirrors the settings used in the production environment. You can view these settings by searching for the Bitcoin API canisters on the ICP Dashboard and calling their `get_config` endpoint.

- [Mainnet Bitcoin API Canister](https://dashboard.internetcomputer.org/canister/ghsi2-tqaaa-aaaan-aaaca-cai)

- [Testnet Bitcoin API Canister](https://dashboard.internetcomputer.org/canister/g4xu7-jiaaa-aaaan-aaaaq-cai)
...
- `syncing` (Flag): Indicates whether the Bitcoin canister is actively ingesting blocks to update its state. -
...
`api_access` (Flag): Controls access to the canister's endpoints.

    - Options: `enabled`, `disabled`

    - Default: `enabled`

`disable_api_if_not_fully_synced` (Flag): Determines whether API access is automatically disabled when the watchdog canister detects that the Bitcoin canister's state is more than **two blocks** behind the Bitcoin blockchain.
...
- `watchdog_canister` (Option<Principal>): Specifies the principal ID of the watchdog canister. If this canister detects that the Bitcoin canister is lagging behind, it is authorized to disable API access.
