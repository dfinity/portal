...
- [x] [Setup the local developer environment](/build-on-btc/btc-dev-env).
...
:::caution
When you set up your [developer environment](/build-on-btc/btc-dev-env), if you created the subdirectory for your `bitcoin_data` files in another project's directory, you either need to [make them again or copy them](/build-on-btc/btc-dev-env#step-2-create-a-subdirectory-for-bitcoin-data) into this project's folder.
:::
...
The Bitcoin network uses different types of addresses (e.g., P2PKH, P2SH), most of which can be generated from an ECDSA public key. The `basic_bitcoin` example implements a function for generating a P2PKH address using the [`ecdsa_public_key`](/references/ic-interface-spec#ic-ecdsa_public_key) API endpoint.
