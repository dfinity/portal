...
To read information about the Bitcoin network's state, the Bitcoin API exposes the following methods:
...
To read unspent transaction outputs from the Bitcoin network, make a call to the `bitcoin_get_utxos` [Bitcoin API method](btc-api.mdx). You can create a function in your canister to call this method, such as:
...
To read the current balance of a Bitcoin address, make a call to the `bitcoin_get_balance` [Bitcoin API method](btc-api.mdx).
...
To read the block headers within a provided range of start and end heights, make a call to the `bitcoin_get_block_headers` [Bitcoin API method](btc-api.mdx)
