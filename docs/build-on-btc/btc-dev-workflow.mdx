...
Developers on ICP can create dapps that interact directly with the Bitcoin mainnet and testnet networks through ICP's direct integration with Bitcoin. To use this integration, developers must implement a specific workflow to prepare transactions and submit them to the Bitcoin network.

To build Bitcoin dapps, a developer needs to make calls to the ICP system API and the Bitcoin integration API and complete the following steps:
