...
Before a transaction can be sent to the Bitcoin network, it must be signed using Threshold ECDSA API method `sign_with_ecdsa` and an ECDSA public key, which can be returned by calling the API method `ecdsa_public_key`.

An code example using these API methods can be found below:
