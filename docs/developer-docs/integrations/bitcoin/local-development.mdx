...
Having your own local Bitcoin network allows you to mine blocks quickly and at-will, which facilitates testing various cases without having to rely on the Bitcoin testnet or mainnet.
...
## Deploying an example Bitcoin dapp

For the next steps,you'll be leveraging the [example project](https://github.com/dfinity/examples) to showcase some of the key concepts. 
...
or with the Candid UI using the link you see in the output above.
...
If successful, you'll see an output that looks similar to this:
...
:::info
Syncing the first block can take up to 30 seconds. Subsequent blocks sync nearly instantly.
:::
...
You should see a balance of 5_000_000_000 Satoshi, which is 50 BTC. This is the reward you received for mining one block.
...
Mine 100 additional blocks by running the following command:
...
In the logs of `dfx`, you should see something like this:
