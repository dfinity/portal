...
the ICP Ledger is running on the NNS subnet. The second type of ledgers that you will find on the internet computer are ICRC-1 Ledgers. In this overview both ledger types
...
It is a standard created by the internet computer working group that defines general functionalities of ledgers. All tokens and their corresponding ledgers that wish to be support this standard have to fullfill all requirements
...
Additionally to the ICRC-1 standard there have been discussions around further specifications and functionalitites. One of the results of these discussions is an extension
...
The purpose of the ICRC-1 Ledger thus is to create a universially accepted standard for making and recording transactions for tokens on the internet computer. 
...
all follow the same standard. The reference implementation and the ICP Ledger share a lot of similarities. However, they do have different endpoints, different transaction and block objects and quite a few other more suttle differences. 

The biggest and most important difference between the ICP Ledger and the ICRC-1 standard (and thus its reference implementation) is how they define accounts. Both are account-based but the ICRC-1 standard [specifies](https://github.com/dfinity/ICRC-1/blob/main/standards/ICRC-1/README.md#account) `Accounts` as struct that contans a `principal` and an optional `subaccount`.
...
way round. This provides the benefit of providing a certain degree of anonymity to the ICP Ledger, but it also means that the ICP Ledger can never have the same internal representation as ICRC-1 Ledgers. 
...
The ICP Ledger, due to its use of AccountIdentifiers, cannot fullfilled this requirement. The ICP Ledger successfully implements all ICRC-1 endpoints an they can be used like for any other ICRC-1 endpoint. 
Should a future ICRC-1 extension standard or through the use of index canisters result in the expore of the internal representation of accounts, any ICRC-1 Ledger will deviate from the ICP Ledger. 
