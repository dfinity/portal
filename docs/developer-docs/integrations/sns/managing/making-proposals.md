...
* **The target and validator methods should check that only the SNS governance canister can be the caller of the method.** Otherwise, it would not be enforced that the actions defined by the generic proposals can only be triggered by an SNS DAO decision.
