...
In addition,
* if the argument `include_neurons_readable_by_caller` is set to true, the neurons who the user is authorized to read are also included (i.e., the neurons for which the user is a controller or hot key).
...
* For each requested neuron that exists and for which the calling user is a controller or hot key, the full neuron is returned.
