...
Hotkeys cannot increase dissolve delay, start dissolving, or disburse a neuron's stake.

A neuron can have multiple hotkeys, with at most 15 hotkeys per topic on a neuron. One common workflow using hotkeys is to set the primary controller of the neuron as a hardware wallet, then vote or view neuron information through a software wallet set as a hotkey.
