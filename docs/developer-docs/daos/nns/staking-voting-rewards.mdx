...
The Internet Computer is a decentralized computer whose evolution is decided by its stakeholders through voting.
This means decision impacting the future of the Internet Computer are made by people vested in the outcome.
In return for participation in governance, the Internet Computer provides voting rewards.
Voters can vote actively, or they can use a form liquid democracy to automatically follow other voters.
...
As explained in the [Neuron overview](/docs/current/developer-docs/daos/nns/neuron-overview), maturity can then either be staked or spawn.
We next explain the maturity modulation in more detail.
