...
If you are looking to verify the ledger suite, which includes the ICP ledger and all ICRC ledgers (used for ckBTC, ckETH, ckERC20) apart from regularly checking open proposals, you might want to consider the following information for proposals submitted by the DFINITY foundation:
...
If you are looking to verify the ledger suite orchestrator, which manages all ledger suites of all ckERC20 tokens, the following sub-directories in the [IC repository](https://github.com/dfinity/ic) are relevant:
