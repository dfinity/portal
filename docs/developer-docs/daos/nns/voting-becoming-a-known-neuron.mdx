...
It can be useful for known neuron candidates to be an (unregistered) actively voting neuron for a while before registering the neuron.
During this time, the neuron holder can get familiar with voting and start becoming an expert in a topic. They can also share their neuron ID for others to follow them.
In this way, by the time that they register the neuron as a known neuron, there is already some evidence of their voting behavior that NNS participants can base their decision on.
