...
To become a known neuron in the NNS DAO, you first need to create a neuron and then submit an NNS proposal to register the known neuron in the NNS governance. After that it will be visible to all NNS DAO users who are setting up their following. A know neuron can have a name and optionally a short description. Often the owers use this to describe the principle according to which they plan to vote and the proposal topics which the known neuron will vote on. Recall that following is done based on proposal topics, so this is relevant information for followers as they want to follow neurons that vote in order not to miss out on [voting rewards](./staking-voting-rewards.mdx).
