...
If you already have ICP tokens on an exchange, you can transfer some of them to your NNS dapp account as also explained in this video.
...
:::info
Not all exchanges support all features. If you want to do this it is advisable to check if a given exchange
:::

### Get tokens from a developer identity using the CLI

If you have selected self-custody for holding your ICP utility tokens and the tokens are associated with your developer identity instead of your registered Internet Identity, you need to transfer ICP utility tokens to your accounts if you want to manage them using the [NNS dapp](https://nns.ic0.app).
...
If you want to transfer one ICP utility token to this account, you can run the following command:
...
The destination address can be any address in the ledger canister which might
correspond to wallet that is owned by a developer principal, by an account you have added using the NNS dapp,
...
Similarly to the CLI transfer above, the destination address can be any address in the ledger canister which might
correspond to wallet that is owned by a developer principal, by an account you have added using the NNS dapp,
or the address for a wallet you have on an exchange.