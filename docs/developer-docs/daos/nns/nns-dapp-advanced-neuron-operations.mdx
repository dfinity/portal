...
We focus here on the tutorisals.
For more background on neurons, including the concepts of voting rewards called maturity and what the different neuron operations do in details,
we refer to [Neurons](/docs/current/developer-docs/daos/nns/neurons-overview).
...
When the maturity for a locked stake reaches a minimum threshold of one ICP, you can spawn a new neuron. The spawn operation creates a new neuron that locks a new balance of ICP on the ledger.
