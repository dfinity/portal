...
## Overview
<iframe width="560" height="315" src="https://youtu.be/1uX-fRgvXjU?si=MRArgvl_8csI3mFg" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

The Network Nervous System is the decentralized autonomous organization (DAO) that governs the full Internet Computer.
It is a stake-based, open governance system, which means that everyone can participate by staking some ICP utiltiy tokens.

## The need for the NNS
There are a few main motivations for the NNS.
The Internet Computer protocol is a
Why we need the NNS:

Many nodes, protocol, smart canisters
As w/ any software, need to update (evolve, fix bugs). make decisiosn when to update
topology changes: ICP consists of many nodes that talk to each other, located in different DCs, organized into different subnets
=> to scale ICP, need to be able to add more subnets/ nodes

NNS governs full ICP, protoco upgrades, topology changes, updates of governance itself


## Neurons
participants of governance
high level: locked tokens, everyone can pariticpate by locking tokens
