...
Any eligible neuron can submit a proposal. Therefore, the command to [submit a proposal](/docs/current/developer-docs/developer-tools/cli-tools/quill-cli-reference/sns/quill-sns-make-proposal)
...
* `<PATH_TO_PEM_FILE>` is the path to the PEM file of the identity that owns the neuron that is submitting the proposal. To generate a PEM file, see [here](/docs/current/developer-docs/developer-tools/cli-tools/quill-cli-reference/quill-generate).
...
For example, if the treasury contains 75_000 XDR worth of SNS tokens, then this amount is considered
"small". In this case up to 75_000 XDR worth of SNS tokens can be transferred from the treasury.

To assess the tokens in the treasury, external price information is fetched at the time of proposal
submission.

The price of ICP is taken from the cycles minting canister's
[get_average_icp_xdr_conversion_rate](https://sourcegraph.com/github.com/dfinity/ic@8efb7e81a1f1257a38621b1f12eccc8b8b1c7e39/-/blob/rs/nns/cmc/src/main.rs?L901:33-901:44)
method.
...
:::warning
Because this proposal requires passing wasm, which is unwieldy to copy/paste into the command line as binary, it is recommended that developers use the specially-made [`make-upgrade-canister-proposal`](/docs/current/developer-docs/developer-tools/cli-tools/quill-cli-reference/sns/quill-sns-make-proposal) command in `quill sns`.
...
Later, additional parameters might be added.
...
The canisters whose settings should be updates are specified in `canister_ids` and the proposal allows updating
the freezing threshold, the reserved cycles limit, the log visibility (which can be visible to just controllers or public),
...
See more [general security best practices](/docs/current/developer-docs/security/general-security-best-practices).
...
The [SNS asset canister](/docs/current/developer-docs/daos/sns/managing/sns-asset-canister) is a canister used to store and retrieve static assets. A dapp controlled by an SNS may have its own associated asset canister.
...
To do this, the SNS governance needed to add a new proposal type, one that would execute the custom [`commit_proposed_batch` function](/docs/current/developer-docs/daos/sns/managing/sns-asset-canister/#sns-genericnervoussystemfunctions).
