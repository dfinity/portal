...
value of the tokens in the treasury in [XDR], denoted as T. For the purposes of capping, T can fall
within three ranges:
...
<table>
  ...
  <tbody>
    <tr>
      <td>Small</td>
      <td>0 &le; T &le; 100_000</td>
    </tr>
    <tr>
      <td>Medium</td>
      <td>100_000 < T &le; 1_200_000</td>
    </tr>
    <tr>
      <td>Large</td>
      <td>1_200_000 < T < ♾️</td>
...
L(T) defines the rules that depend on the amount T in the treasury and is defined by the following
table.
...
[get_average_icp_xdr_conversion_rate][icp-price] method.

[icp-price]: https://sourcegraph.com/github.com/dfinity/ic@8efb7e81a1f1257a38621b1f12eccc8b8b1c7e39/-/blob/rs/nns/cmc/src/main.rs?L901:33-901:44

The price of the SNS token in ICP is taken from the Swap Canister's [get_derived_state] method.

[get_derived_state]: https://sourcegraph.com/github.com/dfinity/ic@8efb7e81a1f1257a38621b1f12eccc8b8b1c7e39/-/blob/rs/sns/swap/proto/ic_sns_swap/pb/v1/swap.proto?L1304-1325
...
The [SNS asset canister](https://internetcomputer.org/docs/current/developer-docs/daos/sns/managing/sns-asset-canister) is a canister used to store and retrieve static assets. A dapp controlled by an SNS may have its own associated asset canister.
...
To do this, the SNS governance needed to add a new proposal type, one that would execute the custom [`commit_proposed_batch` function](https://internetcomputer.org/docs/current/developer-docs/daos/sns/managing/sns-asset-canister/#sns-genericnervoussystemfunctions).
