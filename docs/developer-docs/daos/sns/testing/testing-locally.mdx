...
* If you have started using `sns-testing` before August 2023 and are using the old legacy flow of launching an SNS (please see [here](../launching/index.mdx) for more context), please continue your work with the legacy documentation on [this old README](https://github.com/dfinity/sns-testing/blob/v1-legacy/README.md). In particular,
...
For simplicity, next the stages introduced in the [SNS launch stages documentation](../launching/launch-summary-1proposal.mdx) in the documentation [commands and actions to go through SNS launch](../launching/launch-steps-1proposal.mdx) are mapped to the relevant scripts in the `sns-testing` repo so you can learn what part of the launch is tested in which script.
...
:::info
If you see an error along the lines of "missing field \`Principals\` at line 4 column 1", it means you need to upgrade `dfx sns` to version 0.4.1 or later. The version is `dfx extension install sns --version 0.4.1`. You may first need to remove the existing version of `dfx sns` by running `dfx sns uninstall sns`.
:::
...
launched or use the [SNS testflight on mainnet](testing-on-mainnet.mdx).
