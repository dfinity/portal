...
developers and large investors of the dapp as well as airdrop neurons for smaller
investors and users.
...
the initial neurons use principals generated by `sns-quill`. As argued [above](#tools),
...
principal, i.e., the principal and account ID as stated above, to someone else.
...
Also **you must ensure that you control one of the principals associated with an
initial SNS neurons** as otherwise you cannot send the proposals necessary to launch
the SNS.
...
2. This principal must a controller of the dapp that you would like to start an SNS for. 
