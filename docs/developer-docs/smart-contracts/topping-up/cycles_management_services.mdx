...
Once a <Tooltip text="A smart contract on ICP.">canister</Tooltip> is deployed to the Internet Computer, the compute and storage it utilizes must be pre-payed. The process of burning ICP to obtain cycles and transferring them to a canister is referred to as "topping-up" a canister.
