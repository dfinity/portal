...
Replicated queries, also referred to as "query-as-update" execution mode, are query calls executed as updates. The queries still discard the state changes, but execution happens on all nodes and the execution results goes through consensus.
