...
Replicated queries, also referred to "query-as-update" execution mode, is a query call executed as an update. The query still discards the state changes, but the execution happens on all nodes and the result of execution goes through consensus.
