...
Update calls are executed on all nodes of a <GlossaryTooltip>subnet</GlossaryTooltip> since the result must go through consensus. This makes them slower compared to query calls. They are submitted asynchronously and answered synchronously.
...
Authorized calls that return a value of `()` indicate that the function may not take an argument, or no meaningful value is returned.
