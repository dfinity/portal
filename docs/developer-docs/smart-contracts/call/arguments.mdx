...
Azle refers to the `Vec` type to represent the equivalent of an `array` in Typescript. This is because `Vec` aligns with the [Candid](/docs/current/developer-docs/smart-contracts/candid/candid-conceptscandid-concepts) type.
