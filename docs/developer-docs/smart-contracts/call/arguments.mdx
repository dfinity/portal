...
export default class {
    @query([Numbers], IDL.Text)
    getText(num: Numbers): string {
        if ('1' in num) {
            return num.1;
        }
        ...
        throw new Error(`Variant "Numbers" has unknown tag`);
    }
}
```
...
class Numbers(Variant, total=False):
    1: str
    2: str
    3: str
    4: str
