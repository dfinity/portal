...
Canisters can sign threshold Schnorr messages without holding the Schnorr keys themselves. Each key is derived from a master key held by the dedicated threshold Schnorr subnet. When a canister needs a signature, a request is sent to the threshold Schnorr subnet that computes the signature based on the requesting canister's root key, then returns the signature to the canister.
...
The `sign_with_schnorr` method requires cycles to be attached. When the method in the Rust CDK is used, the required cycles are automatically attached. In Motoko, the cycles need to be added manually.
...
Verification can be done in other languages using libraries and packages that support `bip340secp256k1` signing with arbitrary message length.
...
- [Threshold Schnorr sample dapp - Motoko](https://github.com/dfinity/examples/blob/master/motoko/threshold-schnorr/).

- [Threshold Schnorr sample dapp - Rust](https://github.com/dfinity/examples/blob/master/rust/threshold-schnorr/).
