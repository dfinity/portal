---
keywords: [advanced, tutorial, signing messages, signatures]
---
...
The Threshold ECDSA API allows canisters to securely sign messages and transactions without anyone ever having direct access to the corresponding private keys. Each canister can control an unlimited number of keys by specifying different key derivatation paths.
The API provides two methods: `sign_with_ecdsa` and `ecdsa_public_key`. The `sign_with_ecdsa` method is used to sign messages, while the `ecdsa_public_key` method is used to obtain public keys.

## Signing messages

To sign a message, the `sign_with_ecdsa` method is used. It follows the following Candid interface:
...
The `sign_with_ecdsa` method allows to specify an optional `derivation_path` which allows a canister to request signatures for different keys controlled by the canister. The `key_id` field specifies the curve and name of the key to sign with.
...
- `test_key_1`: Test key available on ICP mainnet.
- `key_1`: Production key available on ICP mainnet.

### Cost

The `sign_with_ecdsa` method requires cycles to be attached. The `sign_with_ecdsa` method exposed in the Rust CDK automatically attaches the required cycles. In Motoko, the cycles have to be added manually.
...
The Internet Computper provides the `ecdsa_public_key` method to obtain public keys. The method follows the following Candid interface:
...
The `ecdsa_public_key` method can be used to obtain public keys controlled by canisters. In contrast to the `sign_with_ecdsa` method, it allows to specify an optional `canister_id`, i.e. the caller can request public keys controlled by other canisters, but not signatures.
