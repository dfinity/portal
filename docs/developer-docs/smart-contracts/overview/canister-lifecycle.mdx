...
It can be done using the [<GlossaryTooltip>NNS</GlossaryTooltip> frontend dapp](/docs/current/developer-docs/daos/nns/using-the-nns-dapp/nns-app-quickstart#deploying-and-managing-canisters-from-the-nns-dapp) in the browser or [`dfx`](/docs/current/developer-docs/developer-tools/cli-tools/cli-reference/) in the command line.
