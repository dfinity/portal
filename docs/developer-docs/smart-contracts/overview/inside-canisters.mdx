...
<Tooltip text="A smart contract on ICP."placement={"bottom"}>Canisters</Tooltip> are WebAssembly (Wasm) programs with additional features that allow them to store persistent data, communicate with users and other canisters, and be managed by entities such as DAOs.
...
## Canister ID
A canister has a unique identifier that is often referred to as the address of the canister or the <Tooltip text="An entity that can be authenticated by ICP - user, canister, etc.">principal</Tooltip> of the canister.
