...
Canisters are WebAssembly (Wasm) programs with additional features that allow them to store persistent data, communicate with users and other canisters, and be managed by DAOs.
This means that in addition to the standard Wasm components such as Wasm code and Wasm memory, canisters also have other components that are explained in the rest of this page.
...
## Canister Id
A canister has a unique identifier that is often referred to as the address of the canister or the principal of the canister.
...
The system then routes the message to the target canister and puts it in the input queue of that canister.

## Input queue
Canisters have queues for incoming messages.
When a canister is scheduled for execution, it takes one message from the queue and runs the corresponding Wasm function specified by the message.
...
If the canister has no controllers, then it is immutable in the sense that its Wasm code will not change.
...
The list of all settings is available [here](https://internetcomputer.org/docs/current/references/ic-interface-spec#ic-create_canister).