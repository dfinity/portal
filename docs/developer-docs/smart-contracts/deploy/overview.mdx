...
deploy it either on the
[local canister execution environment](/docs/current/developer-docs/getting-started/deploy/local),
a sandbox environment such as the
[ICP playground](/docs/current/developer-docs/developer-tools/ide/playground) or
[on the mainnet](/docs/current/developer-docs/getting-started/deploy/mainnet).
Your canister must be deployed before you can make calls to it and execute
methods.

## Testnets on ICP

A testnet typically refers to a network that is used by developers to test their
smart contracts in a production-like environment before deployment to the
mainnet. This is analagous to a staging environment that is used to "stage" code
changes before releasing them widely.

There are no alternative networks on ICP — the ICP mainnet can be used to
emulate a range of environments, such as a testnet.

You can deploy your canister to the ICP mainnet for free, thereby isolating your
deployments from your production stack by using the ICP playground.
[Learn more](/docs/current/developer-docs/developer-tools/ide/playground).
...
[default project template document](/docs/current/developer-docs/setup/first-canister).
...
To deploy to the
[ICP playground](/docs/current/developer-docs/developer-tools/ide/playground),
use the command:
...
[Learn more about cycles and how to acquire them](/docs/current/developer-docs/setup/cycles/cycles-faucet).

This command deploys all canisters configured in your `dfx.json` file. To deploy
just one canister, specify the canister's name:
...
These commands both deploy to the local network since they do not include a
`--network` flag.
...
[timers](/docs/current/developer-docs/backend/periodic-tasks) can be useful.
