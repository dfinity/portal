...
Once a dapp has been deployed to the mainnet, the <Tooltip text="A smart contract on ICP.">canister</Tooltip> serves the dapp's frontend via a URL. This URL can be shared with anyone, allowing them to view and interact with your dapp without you needing to configure domain names, DNS records, or other networking configurations.
