...
For examples of how to write Candid service descriptions for Rust programs, see the [Rust CDK examples](https://github.com/dfinity/cdk-rs/tree/next/examples) and the [Rust tutorials](/docs/current/developer-docs/backend/rust). You can find instructions on how to generate Candid files for Rust canisters [here](/docs/current/developer-docs/backend/rust/generating-candid).
