...
By default, `dfx` makes the `candid:service` metadata information public to promote interoperability and make it convenient to use the Candid UI. Candid UI reads this metadata to determine the interface of your canister, and it doesn’t publish any more information than is already public. This metadata can be returned through the `dfx` command:
