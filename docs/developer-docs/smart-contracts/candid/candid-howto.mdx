...
:::info
While Candid can be used to facilitate inter-operation between services and canisters, it is not required. Arbitrary bytes can be sent directly to and from canisters if all canisters part of the communication stream are designed to understand them (such that they are written in the same language, etc.).
