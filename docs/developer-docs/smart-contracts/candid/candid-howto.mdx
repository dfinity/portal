...
As discussed in the [Candid concepts](./candid-concepts.mdx) page, Candid provides a language-agnostic way to interact with canisters.

:::info
While Candid can be used for inter-operation between services and canisters, it is not required to be used. Arbitrary bytes can be sent directly to and from canisters if all canisters part of the communication stream are designed to understand them (such that they are written in the same language, etc.).
...
For additional information and libraries to help you create Candid services or canisters in Rust, see the documentation for the [Candid crate](https://docs.rs/candid/), [Rust CDK examples](https://github.com/dfinity/cdk-rs/tree/next/examples) and the [Rust tutorials](/docs/current/developer-docs/backend/rust/index).
