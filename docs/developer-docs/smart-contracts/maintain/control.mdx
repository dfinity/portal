...
A <Tooltip text="A smart contract on ICP.">canister</Tooltip> is managed by a list of controllers. A controller is specified by a <Tooltip text="An entity that can be authenticated by ICP - user, canister, etc.">principal</Tooltip>, which can be self-authenticating, e.g. a `dfx` developer identity, or another canister. Canisters can have multiple controllers where each controller has the same administrative rights, or it has no controller, in which case the canister becomes immutable, or blackholed, and cannot be upgraded or deleted.
...
- Getting the status of the canister which includes its <Tooltip text="Currency unit used to pay for ICP gas fees.">cycles</Tooltip> balance, memory usage, and list of controllers.
