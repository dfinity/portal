...
[From the WebAssembly overview page](https://webassembly.github.io/spec/core/intro/overview.html)
...
invocations. If this is not possible, then it's recommended to use the [cleanup system API call](https://internetcomputer.org/docs/current/references/ic-interface-spec#system-api-call
) to ensure that any cleanup action is performed
(e.g. releasing a lock that was acquired before making an inter-canister call).
