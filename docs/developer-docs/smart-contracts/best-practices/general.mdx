...
Refer to [this resource](/docs/current/developer-docs/production/topping-up-canister#managing-cycle-depletion-with-freezing-threshold) to learn how to configure the `freezing_threshold` of a canister. 
...
Use [canister timers](/docs/current/developer-docs/backend/periodic-tasks) over plain heartbeats for reduced daily costs.
...
6. Be mindful of calling an [actor class]((https://internetcomputer.org/docs/current/motoko/main/actor-classes), as the overhead is similar to that of installing a fresh canister.
