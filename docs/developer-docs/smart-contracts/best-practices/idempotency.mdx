...
Thus, it is important to design and/or use canister APIs such that it is possible to retry requests safely, even when the ICP doesn't provide any information as to previous request attempts. This page describes general approaches to safe retries, both from the side of the canister and from the side of the client.
...
1. It limits concurrency. The user has to sequentialize all their calls. This can be difficult when the user is using multiple clients or devices to access the canister, for example.
...
Thus, the deduplication is usually time limited to a certain time window. For example, the ICP ledger use a 24 hour window, and the ICRC standard defines a configuration parameter `TX_WINDOW` that determines the window length.
...
In absence of idempotent endpoints, or even as an addition to them, clients may be able to use other endpoints to make their retries safe.
...
1. If the result of the update is unknown (e.g., not present in the ingress history any more), query the call result endpoint to determine whether the call was applied or not. Moreover, one needs to ensure that the previously sent call call cannot be applied in the future. If both of these are true, the call might be retried, or safely reported as failed.
