...
connection before the request status has been removed from the state tree, since ICP will remove the request from the system state tree some time after the ingress expiry.
