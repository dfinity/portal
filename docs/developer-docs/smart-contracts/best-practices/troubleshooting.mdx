...
On subnets with higher loads, your application may experience elevated latencies or reduced accessibility. To get more information about the subnet your canister is running on, view the canister's information on the [ICP dashboard](https://dashboard.internetcomputer.org/canisters).

### Recommendation: Consider moving the canister to another subnet.

If your canister is running on a subnet that has a consistently high load and thus elevated latency, consider adding an endpoint within the canister to export and import canister data. Then, export the data from your canister and create a new canister on another subnet with a lower load. Import the data from the previous canister into the new canister.

:::info
Additional solutions and recommendations for dealing with subnet latencies are currently in progress. Stay up to date by participating in the [forum discussion on subnet latencies](https://forum.dfinity.org/t/subnets-with-heavy-compute-load-what-can-you-do-now-next-steps/35762).
:::
