...
To interact with a canister smart contract's methods, there are two primary types of calls that can be used: update calls and query calls.
...
Certified variables are verifiable pieces of data that come along with a certificate to prove the data's authenticity. Certified variables can be set via an update call and can then be read via a query call.
