...
In theory, any language that can be compiled into a WebAssembly module, can produce modules [tailored for deploying on ICP](/docs/current/references/ic-interface-spec) as an ICP smart contract.
...
- **Motoko**
  ...
  - See [introduction to developing canisters in Motoko](/docs/current/developer-docs/backend/motoko/index).
...
- **Rust**
  ...
  - See [introduction to developing canisters in Rust](/docs/current/developer-docs/backend/rust/index).
- **Python**
...
It is also possible to split your work between multiple languages. Different canister smart contracts talk to each other using the [Candid](/docs/current/developer-docs/smart-contracts/candid/index) language (an [IDL](https://en.wikipedia.org/wiki/Interface_description_language) used commonly in ICP smart contracts). What language works behind the Candid interface, however, does not matter.
