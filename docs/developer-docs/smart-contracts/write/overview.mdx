...
Apps can range from single smart contracts to complex, multi-<Tooltip text="A smart contract on ICP.">canister</Tooltip> projects that are controlled by a DAO, and everything in between.
...
1. The developer uses the default asset <Tooltip text="A smart contract on ICP.">canister</Tooltip> created by `dfx` or writes their own custom canister to store the web assets on-chain.
...
A single <Tooltip text="A smart contract on ICP.">canister</Tooltip> can host the entire application stack including its web assets, core logic, and data.
...
ICP scales horizontally via <Tooltip text="A collection of nodes that run their own instance of the ICP consensus algorithm.">subnets</Tooltip>, so smart contracts can also scale by utilizing more subnets.
