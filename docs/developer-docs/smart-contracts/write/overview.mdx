...
Apps can range from single smart contracts to complex, multi-canister projects that are controlled by a DAO, and everything in between.
...
1. The developer uses the default asset canister created by `dfx` or writes their own custom canister to store the web assets on-chain. 
...
A common pitfall for new developers is architecting the application for millions of users from the get go without understanding the underlying trade-offs of the system.
...
A single canister can host the entire application stack including its web assets, core logic, and data.
...
ICP scales horizontally via subnets, so smart contracts can also scale by utilizing more subnets.
