...
The [HTTP Gateway protocol](docs/current/references/http-gateway-protocol-spec) of ICP allows browsers to load the web assets such as JS, HTML, CSS from a canister via HTTPS.
...
1. The canister serves the web assets to the browser via its `http_request` endpoint that gets invoked for each HTTPS request.
