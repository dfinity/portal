...
This page demonstrates the concept of auto-scaling architectures using an older workflow with heap storage. For current projects, it is recommended use a recommended to use either a single canister architecture with stable memory, as described in the [write overview page](/docs/current/developer-docs/smart-contracts/write/overview), or create an auto-scaling architecture that creates new canisters on a different subnet.
