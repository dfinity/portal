...
One of the special features of ICP is its direct integration with the [Bitcoin](/docs/current/developer-docs/integrations/bitcoin/) network. This is exposed to users through a set of APIs on the management canister that allow users to sign transactions using [threshold ECDSA](/docs/current/developer-docs/smart-contracts/encryption/t-ecdsa) signatures and submit transactions to the Bitcoin network, all done through smart contracts on ICP. The relevant APIs are [sign_with_ecdsa](https://internetcomputer.org/docs/current/references/ic-interface-spec#ic-sign_with_ecdsa) and [bitcoin_send_transaction](https://internetcomputer.org/docs/current/references/ic-interface-spec#ic-bitcoin_send_transaction).
