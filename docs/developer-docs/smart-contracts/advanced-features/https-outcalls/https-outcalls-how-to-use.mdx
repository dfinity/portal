...
This feature allows smart contracts to directly make calls to HTTPS(S) servers external to the blockchain and use the response in the further processing of the smart contract, without the need for oracles.

## Key concepts

### Methods supported

The feature currently supports `GET`, `HEAD`, and `POST` methods for HTTPS requests.
...
-   `headers`: List of HTTPS request headers and their corresponding values.
...
-   `headers`: List of HTTPS response headers and their corresponding values.
