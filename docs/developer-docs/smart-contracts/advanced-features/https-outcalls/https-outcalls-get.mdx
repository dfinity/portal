...
A minimal example to make a `GET` HTTPS request. The purpose of this dapp is only to show how to make HTTPS requests from a canister.
...
The canister in this tutorial will have only **one public method** named `get_icp_usd_exchange()` which, when called, will trigger an HTTPS `GET` request to an external service. The canister will not have a frontend (only a backend), but like all canisters, you can interact with its public methods via the Candid web UI, which will look like this:
...
// ]
  ...
  public func get_icp_usd_exchange() : async Text {
    ...
    //As per the type declarations in `src/Types.mo`, the BODY in the HTTPS response
...
- Sample code of [HTTPS GET requests in Rust.](https://github.com/dfinity/examples/tree/master/rust/send_http_get)
- Sample code of [HTTPS GET requests in Motoko.](https://github.com/dfinity/examples/tree/master/motoko/send_http_get)
