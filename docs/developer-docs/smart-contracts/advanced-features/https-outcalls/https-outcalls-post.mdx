...
A minimal example of how to make a `POST` HTTPS request. The purpose of this dapp is only to show how to make HTTPS requests from a canister.
...
:::caution
The HTTPS outcalls feature only works for sending HTTPS POST requests to servers or API endpoints that support **IPV6**.
:::

### Candid web UI of canister

The canister in this tutorial will have only **one public method** which, when called, will trigger an HTTPS `POST` request. The canister will not have a frontend (only a  backend), but like all canisters, you can interact with its public methods via the Candid web UI, which will look like this:

![Candid web UI](../_attachments/https-post-candid-2-motoko.webp)

When you call the method, the canister will send an HTTPS `POST` request with the following JSON in the response body:
...
### Verifying the HTTPS POST request

In order to verify that your canister sent the HTTPS request you expected, this canister is sending HTTPS requests to a [public API service](https://putsreq.com/aL1QS5IbaQd4NTqN3a81/inspect) where the HTTPS request can be inspected. As you can see the image below, the `POST` request headers and body can be inspected to make sure it is what the canister sent.
...
The recommended way for HTTPS `POST` requests is to add the idempotency keys in the header so the destination server knows which `POST` requests from the client are the same.
...
//method that uses the HTTPS outcalls feature and returns a string
  public func foo() : async Text {
    ...
    //2. SETUP ARGUMENTS FOR HTTPS GET request
    let request : Types.HttpRequestArgs = {
        //construct the request
    };

    //3. ADD CYCLES TO PAY FOR HTTPS REQUEST
    //code to add cycles
...
module Types {
    ...
    //type declarations for HTTPS requests, HTTPS responses, management canister, etc...
...
//This method sends a POST request to a URL with a free API you can test.
  public func send_http_post_request() : async Text {
    ...
    //1. DECLARE MANAGEMENT CANISTER
    //You need this so you can use it to make the HTTPS request
    let ic : Types.IC = actor ("aaaaa-aa");

    //2. SETUP ARGUMENTS FOR HTTPS GET request

    // 2.1 Setup the URL and its query parameters
    //This URL is used because it allows you to inspect the HTTPS request sent from the canister
    ...
    //The management canister will make the HTTPS request so it needs cycles
    ...
    //As per the type declarations in `Types.mo`, the BODY in the HTTPS response
...
async fn send_http_post_request() -> String {
    ...
    //Since the body in HTTPS request has type Option<Vec<u8>> it needs to look something like this: Some(vec![104, 101, 108, 108, 111]) ("hello" in ASCII)
...
module Types {
    ...
    //you do not use this function. You will pass "null" to the HTTPS request.
...
* Sample code of [HTTPS POST requests in Rust](https://github.com/dfinity/examples/tree/master/rust/send_http_post)
* Sample code of [HTTPS POST requests in Motoko](https://github.com/dfinity/examples/tree/master/motoko/send_http_post)
