...
A minimal example of how to make a `POST` HTTPS request. The purpose of this dapp is only to show how to make HTTP requests from a canister.
...
Because HTTPS outcalls go through consensus, a developer should expect any HTTPS `POST` request from a canister to be sent many times to its destination. Even if you ignore the Web3 component, multiple identical POST requests are not a new problem in HTTP where it is common for clients to retry requests for a variety of reasons (e.g. destination server being unavailable).
