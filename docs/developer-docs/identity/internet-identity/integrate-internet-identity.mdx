...
When prompted during the creation of a new project to **Add extra features**, select **Internet Identity**.

This extra feature will add the **pullable** version of the Internet identity canister to your project. A pullable canister is a canister that provides a public service at a static canister ID. To learn more about pullable canisters, please [see the documentation](/docs/current/developer-docs/smart-contracts/maintain/import).

<details>
<summary>If you already have a project without the extra <strong>Internet Identity</strong> feature</summary>

1. Add the canister configuration:
   ```json title="dfx.json"
   {
     "canisters": {
       "internet_identity": {
...
2. Then deploy the canister locally:
   ```bash
   dfx deploy
   ```
...
The frontend code in this tutorial is based on a new project with a **Vanilla JS** frontend. If another option was chosen or if you already have a project, you'll need to adapt the code in this tutorial to your project.

:::

### Step 2: Add "Who am I" function

1. For this project, you'll need to add a simple "Who am I?" function to the backend canister:
...
2. Then build to make the new backend declarations available to the frontend:
   ```bash
   dfx build
   ```

3. Now in the frontend, add a method to the `App` class to call the backend function:
...
4. Lastly update the `#render` method to add the new "Who am I?" button and a click listener:
   ```js title="src/ii-integration-frontend/src/App.js"
   #render() {
     let body = html`
       <main>
...
5. Then deploy the updated frontend:
   ```bash
   dfx deploy
   ```

6. Visit your deployed frontend, click the "Who am I?" button and it should return the anonymous principal:
   ```
...
2. Add a method to the `App` class to get the correct Internet Identity url:
   ```js title="src/ii-integration-frontend/src/App.js"
   /**
   ...
   In case you're developing on Safari, you need to change the value returned for a local development environment to `http://localhost:4943?canisterId=<canister_id>`.

   :::

3. Add a method to create an `AuthClient` instance and call it in the `App` class constructor:
   ```js title="src/ii-integration-frontend/src/App.js"
   constructor() {
     this.#render();
...
4. Add methods to the `App` class to login and logout:
   ```js title="src/ii-integration-frontend/src/App.js"
   /**
   ...
   Make sure to always create a single AuthClient instance on page load and re-use it within your click handlers as shown in the previous and current step.

   :::

5. Lastly update the `#render` method to add the new "Login" and "Logout buttons and click listeners:
   ```js title="src/ii-integration-frontend/src/App.js"
   #render() {
     let body = html`
       <main>
...
6. Then deploy the updated frontend:
   ```bash
   dfx deploy
   ```

7. Click the button to login, and you'll be redirected to the II frontend. Since you're running this locally, you will be using a local, non-production Internet Identity. To create one, follow the on-screen steps.

8. Click the "Who am I" button and it should return your Internet Identity principal:
   ```
   ...
   The above principal is an example, the principal returned will be different based on the account and the environment where the dapp and Internet Identity are running.
