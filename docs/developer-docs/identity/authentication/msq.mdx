...
[MSQ](https://github.com/fort-major/msq/tree/master) is a MetaMask Snap wallet designed authentication and integration with dapps deployed on ICP. MSQ uses a scoped identity architecture, meaning each user gets a unique and dedicated identity for each dapp or website they interact with. Scoped-identity architectures help protect users from signature-stealing attacks.
