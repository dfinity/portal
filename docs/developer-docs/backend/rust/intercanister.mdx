...
<MarkdownChipRow labels={["Beginner", "Rust", "Concept"]} />
...
pub async fn call_get(counter: Principal) -> Result<(), String> {
    match Call::bounded_wait(counter, "get")
        ...
        .change_timeout(1)
        .await {
        Ok(bytes) => Ok(bytes.decode_candid::<Nat>().map_error(|e| "Candid decoding failed: {:?}", e)),
