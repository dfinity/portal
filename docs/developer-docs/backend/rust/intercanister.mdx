...
To understand more details about how inter-canister calls execute, how different call types and  consult the documentation [inter-canister calls and async code](/docs/current/developer-docs/smart-contracts/advanced-features/async-code)
As noted in the example, an `update` method can always call any method of any other canister. In cases where you only need to call query methods on other canisters, and if you are sure that these canisters are on the same subnet as your canister, you can also use [composite query calls](/docs/current/developer-docs/smart-contracts/call/overview) methods instead of update methods.
