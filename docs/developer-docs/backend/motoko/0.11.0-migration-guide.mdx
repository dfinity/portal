...
<MarkdownChipRow labels={["Motoko", "Migration guide"]} />

## Overview

Motoko v0.11.0 and newer introduces a new safety feature that could cause breaking changes to existing workflows.

In previous Motoko versions, third-party library functions could make calls to sensitive functions such as `ExperimentalCycles.add(...)` and `Timer.setTimer(...)` without providing indication that to the caller that sensitive system functionality was being used.
...
Another change in v0.11.0 and newer is a revision of `ExperimentalCycles.add`, which has been changed from `Nat -> ()` to `<system>Nat -> ()`. This change supports the system capability requirement of the special type parameter `<system>`.
