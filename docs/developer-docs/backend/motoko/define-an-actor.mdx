---
keywords: [intermediate, motoko, tutorial, actors]
---
...
For more information about using a query call, see [query calls](/docs/current/concepts/canisters-code#query-update) in [canisters include both program and state](/docs/current/concepts/canisters-code#canister-state).
...
## Next steps
Next, let's take a look at using [multiple-actors](multiple-actors.mdx).