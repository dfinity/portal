---
keywords: [intermediate, motoko, tutorial, multiple actors]
---
...
```bash
dfx new multiple_actors --no-frontend
```
...
In the next guide, let's discuss [access control using identities](./access-control.mdx)