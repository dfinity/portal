...
Python canisters can be written using the Kybra canister development kit. For Kybra projects, you will need to install `dfx` version `0.19.0+`, and you must run all `dfx` commands in a Python virtual environment.
...
```bash
dfx canister call hello_backend greet '("Hello World!")'
```
