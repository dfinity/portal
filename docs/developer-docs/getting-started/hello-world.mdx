...
This command starts a local instance of the ICP <Tooltip text="The ICP software used to run a node.">replica</Tooltip> using a single node testnet. The output will return a dashboard URL that can be used to view information about the local replica:
...
You will be prompted to select a language that your backend <Tooltip text="A smart contract on ICP.">canister</Tooltip> will use:
...
Then, select a frontend framework for your frontend <Tooltip text="A smart contract on ICP.">canister</Tooltip>, or select 'No frontend canister':
...
Deploy the project's <Tooltip text="A smart contract on ICP.">canisters</Tooltip> locally:
...
- It sets up a special `cycle-wallet` smart contract for storing <Tooltip text="Currency unit used to pay for ICP gas fees.">cycles</Tooltip>. All operations require payments in cycles. In your local testnet, this wallet comes prefilled with cycles.
...
- It sends the message to the local testnet (<Tooltip text="The ICP software used to run a node.">replica</Tooltip>).
