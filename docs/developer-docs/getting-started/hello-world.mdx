...
To deploy a simple 'Hello, world!' smart contract, you can setup a local developer environment, or you can use a cloud-based developer environment such as Gitpod or GitHub Codespaces that already has the necessary dependencies and tools installed. For this example, Gitpod and Codespaces examples are available for the Motoko and Rust programming languages. Local development using `dfx` supports Motoko, Rust, Python, and TypeScript. 
...
In this project, there are two canisters, `hello_backend` and `hello_frontend`.

The `hello_frontend` canister is used to store the dapp's frontend assets. These include files and code such as HTML, CSS, JavaScript, React, images, videos, the app's user interface. 
...
It is important to note that canister is capable of storing both the frontend assets and backend code. However, `dfx` by default uses a project template with a dedicated canister for the frontend means since this allows any language to be used for the backend canister without needing to use a library for the assets storage API. 
