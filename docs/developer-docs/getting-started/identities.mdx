...
On the Internet Computer (ICP), a developer **identity** is essential to developing canisters. Developer identities use a private/public key pair for authentication. The public key can be distributed openly, as it will be used by services to encrypt messages. The private key must be kept secret and stored securely.

Developer identities have a **principal**, which is a generic identifier value that is used to identify users, <GlossaryTooltip>canisters</GlossaryTooltip>, and potentially other entities on the network in the future. Each developer identity's principal value is derived from the identity's public key.
...
Then, create your personal password-protected developer identity with the command:
...
The private key for an identity is stored in the `~/.config/dfx/identity/IDENTITY_NAME/identity.pem.encrypted`. ***This file should be backed up to a secure location.***
