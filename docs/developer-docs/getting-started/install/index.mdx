...
When developing on ICP, there are two primary tools used: `dfx`, a CLI tool used to create, deploy, and manage <GlossaryTooltip>canisters</GlossaryTooltip>, and a **canister development kit** (CDK) which provides an environment for writing canister code in different programming languages while supporting ICP features.
...
* Ubuntu Running 2`}
    ...
    <p>WSL 2 has node.js <code>10.x.x</code> installed by default. But the latest <code>dfx</code> requires
      node.js <code>16.x.x</code> or <code>18.x.x</code>.</p>
...
If you would like to compile `dfx` by yourself, head over to the [IC SDK repo](https://github.com/dfinity/sdk). The README contains the instructions on how you can build it on your own.
