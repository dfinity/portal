...
When developing on ICP, there are two primary tools used: `dfx`, a CLI tool used to create, deploy, and manage <Tooltip text="A smart contract on ICP.">canisters</Tooltip>, and a **canister development kit** (CDK) which provides an environment for writing canister code in different programming languages while supporting ICP features.
