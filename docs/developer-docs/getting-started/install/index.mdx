...
To install the IC SDK, install `dfxvm`. `dfxvm` is a command-line tool 
used to installing and switching between different versions of `dfx`. 
It enables developers to pivot between different projects that are built 
with different dfx versions. Read more about `dfxvm` [here](/docs/current/developer-docs/developer-tools/dev-tools-overview#dfxvm).

You can download and install the latest version of 
[`dfxvm`](https://github.com/dfinity/dfxvm) by running the command below:
