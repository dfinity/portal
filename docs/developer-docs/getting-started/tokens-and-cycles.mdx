...
You can obtain ICP tokens from an [exchange](https://www.coingecko.com/en/exchanges) or through a [developer grant](https://dfinity.org/grants). If you are using an exchange, initiate a withdraw transaction, then enter the ledger account ID as the "destination" address to send ICP tokens to.
