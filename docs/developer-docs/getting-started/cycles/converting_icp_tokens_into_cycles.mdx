...
## Overview
To convert ICP tokens into <Tooltip text="Currency unit used to pay for ICP gas fees.">cycles</Tooltip>, you first need to obtain some ICP and transfer to the right account. You can get ICP tokens on exchanges, or ask someone you know to send you some. To figure out which account to transfer the ICP tokens to, run the following:
...
The two values you have to substitute are your own <Tooltip text="An entity that can be authenticated by ICP - user, canister, etc.">principal</Tooltip> and the amount of tokens you want to convert.
