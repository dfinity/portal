...
In addition to classical DeFi smart contracts, such as ledgers and exchanges, ICP can run compute- and storage-heavy applications such as [image classification](https://github.com/dfinity/examples/tree/master/rust/image-classification) fully on-chain.
...
The private keys are not stored on the blockchain. The blocks in the blockchain contain transaction data and shouldn’t store confidential information, for example, private keys. Also, II doesn’t manage the private keys but rather 

Since smart contracts can provide user accounts as a service. **Internet Identity** is an example of a smart contract that allows Web2 users to have blockchain accounts that leverage user authentication to map the user to an identity and provides delegation to the relying party.
