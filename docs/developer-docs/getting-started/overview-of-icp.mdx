...
Smart contracts can provide user accounts as a service. **Internet Identity** is an example of a smart contract that allows Web2 users to have blockchain accounts that leverage user authentication to map the user to an identity and provides delegation to the relying party.
