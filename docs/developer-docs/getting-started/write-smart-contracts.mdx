...
        let randomBytes = await CallRand.raw_rand();
...
HTTPS outcalls can be used to obtain data from any external source, including other blockchains, traditional Web2 APIs, and other web services. When HTTPS outcalls are used locally, the returned result is not validated since the local replica is only a single node. HTTPS outcalls used on the mainnet, however, validate the returned result through the subnet's consensus, providing security that the data obtained has not been maliciously tampered with during transport.
...
      // This code provides the logic to call the http_request method of the management canister.
      let HttpRequest : actor {
...
      // Define the http_response
      let http_response : Types.HttpResponsePayload = await HttpRequest.http_request(http_request);
...
This HTTP outcalls code references custom types, as indicated by the import statement `import Types "Types";` and code statements like `Types.CanisterHttpResponsePayload`. Custom type definitions can be created from within the `main.mo` file, but for types that may be used in several different source code files, it can be beneficial to define them in a single `Types.mo` file, then import them as needed.
...
3. Timers will be used to generate a new number every 30 seconds and send a new HTTPS outcall every 35 seconds.
...
    // Stable variables persist across canister upgrades.
    stable var currentRandomNumber : Nat = 0;
    stable var proposalNumber : Text = "1";
...
            proposalNumber := Nat.toText(currentRandomNumber);
        };
    };

    // Define a public function to get proposal information.
    // Public functions can be called by users or other canisters.
    // Private functions can only be called by other functions within this code.
    public func getIcpInfo() : async Text {
...
      // Define the API endpoints to obtain data from.
      // This example uses the IC API, but any API endpoint can be used.
      let url = "https://ic-api.internetcomputer.org/api/v3/proposals/" # proposalNumber ;
      let transform_context : Types.TransformContext = {
        function = transform;
        context = Blob.fromArray([]);
      };
...
    // Initialize timer to generate a new number every 30 seconds.
    let timer1 = Timer.recurringTimer<system>(#seconds 30, generateNewNumber);
    let timer2 = Timer.recurringTimer<system>(#seconds 35, printResults);
}
```
