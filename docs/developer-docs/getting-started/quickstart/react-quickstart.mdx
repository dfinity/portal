...
On ICP, frontend UIs for decentralized applications can be hosted in a <Tooltip text="A smart contract on ICP.">canister</Tooltip>, often referred to as the dapp's frontend canister or asset canister. Frontend canisters host the code and assets for a dapp's user interface, which typically includes HTML, CSS, JavaScript, or React code.
...
### Step 2: Start a local <Tooltip text="The ICP software used to run a node.">replica</Tooltip>.
...
The output of this command will return the local <Tooltip text="A smart contract on ICP.">canister</Tooltip> URLs for the frontend and backend canisters:
