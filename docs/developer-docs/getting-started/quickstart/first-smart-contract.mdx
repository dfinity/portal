...
<input type="checkbox"/> Download and install a Linux distribution using Windows Subsystem for Linux: <code>wsl --install -d <Distribution Name></code>
<div>
</div>
<input type="checkbox"/> Open the WSL Linux environment. Run all next steps within this environment.
<div>
</div>
