...
Getting cycles back from a canister
   - To withdraw cycles from a canister, the [canister must be deleted](/docs/current/tutorials/developer-liftoff/level-1/1.6-managing-canisters#getting-cycles-back-from-a-canister).
