...
Canisters pay for consumed resources by burning cycles. These resources include storage, messaging, execution, and special features. To obtain cycles, ICP tokens can be converted into cycles.
...
Replication refers to the number of times a canister is replicated. Replication is dependant on the canister's subnet.
