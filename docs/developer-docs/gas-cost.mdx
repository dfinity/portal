...
The reverse gas model allows dapp developers to provide a their users a smooth experience, since end users don't need to obtain tokens, configure wallets, or sign and approve every transaction they perform when interacting with a dapp on ICP.
...
Replication refers to the number of instances of a canister. Each canister is automatically replicated on every node on a subnet. The amount of cycles that a canister is charged will vary based on the amount of times the canister is replicated:
...
- Canisters deployed to a **13-node subnet** are replicated 13 times, as they are deployed on 13 nodes. Cycle cost amounts are based on a 13-node subnet, i.e., there is no calculation needed to determine the cost based on the subnet's replication factor.
