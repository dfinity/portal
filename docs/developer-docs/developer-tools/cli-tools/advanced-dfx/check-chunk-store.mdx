...
To verify that chunk store is the cause of this error, verify the chunk store has an existing `vec` value by running the [command shown above](#checking-chunk-store).
