...
To generate the schema to a file yourself, use `dfx schema --outfile path/to/outfile.json` as described in the [`dfx schema` reference](/docs/current/developer-docs/developer-tools/cli-tools/cli-reference/dfx-schema.mdx).
