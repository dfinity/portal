...
The IC SDK includes a CDK for the Rust programming language, which is developed and maintained by DFINITY. This Rust CDK is installed automatically when the IC SDK is installed, and can be used through the [`dfx`](/docs/current/developer-docs/getting-started/install/) CLI tool.
