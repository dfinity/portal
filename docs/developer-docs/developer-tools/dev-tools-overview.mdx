...
[`dfxvm`](/docs/current/developer-docs/developer-tools/cli-tools/dfxvm/docs/cli-reference/dfx/) is a command-line tool used to installing and switching between different versions of `dfx`. It enables developers to pivot between different projects that are built with different `dfx` versions.
...
- Dart [`agent_dart` by AstroX](https://github.com/AstroxNetwork/agent_dart) (supports mobile development with Flutter)
