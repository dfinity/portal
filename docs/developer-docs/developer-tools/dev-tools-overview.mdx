...
[`dfxvm`](/docs/current/developer-docs/developer-tools/cli-tools/dfxvm) is a command-line tool used to installing and switching between different versions of `dfx`. It enables developers to pivot between different projects that are built with different `dfx` versions.
