...
<p>If you don’t have internet access on WSL, for instance you cannot ping any server successfully, probably the nameserver on WSL is set to an internal WSL proxy nameserver. You can check the <code>/etc/resolv.conf</code> file to see if it’s the case. If it’s true, please follow the below steps to set to a valid nameserver: <br/ >
<ul>
