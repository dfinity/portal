...
As discussed in [Tokens and cycles](concepts/tokens-cycles.xml), ICP tokens can be converted into **cycles** to power canister operations. Cycles reflect the operational cost of communication, computation, and storage that dapps consume.
...
There are dapps that can help you convert ICP to cycles and create a new cycles wallet, e.g., [NNS dapp](../token-holders/nns-app-quickstart.xml#_deploy_a_canister_with_cycles).
