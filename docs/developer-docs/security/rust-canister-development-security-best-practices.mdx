...
- A guard that makes sure that every resource from a set of resources can only be processed once, independently of the caller who triggered the processing. [View example project](https://github.com/dfinity/examples/tree/master/rust/guards).
...
##### Caveats:
- The `setup_randomness` must **always** be initialized in both the `init` and `post_upgrade` hook as `init` is not invoked during an canister upgrade [see here](https://internetcomputer.org/docs/current/references/ic-interface-spec#system-api-upgrades).
