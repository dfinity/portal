...
### Using secure randomness in the Internet Computer

Canister developers often require access to secure randomness in their canisters to perform certain operations. The requirements for secure randomness source include 
* Unbiased : The value shouldn't be influenced by anyone
* Unpredictable : The value is unknown to anyone before it is released.

The Internet Computer exposes the system API ic0.raw_rand for this exact purpose, which accepts no input and returns 32 bytes of cryptographically secure randomness. Below, we see two examples of using this system API to obtain secure randomness in Motoko and Rust while discussing the security concerns and caveats around using them. 

#### Using ic0.raw_rand as the PRNG
...
##### Benefits:
- Randomness is unpredicable as guaranteed by the system API.
- It can be used in a query.
##### Caveats:
- When large amount of random bytes is required, this becomes impractical as it's an asynchronous system API call every 32bytes which doesn't scale. 

#### Using ic0.raw_rand as the seed for the PRNG.
...
#[ic_cdk::init]
fn init() {
    // Initialize randomness at canister install or reinstall
    ic_cdk_timers::set_timer(Duration::ZERO, || {
        ic_cdk::spawn(async {
            setup_randomness().await;
        })
    });
}
...
#[ic_cdk::post_upgrade]
fn post_upgrade() {
    // Initialize randomness after canister upgrade
    ic_cdk_timers::set_timer(Duration::ZERO, || {
        ic_cdk::spawn(async {
            setup_randomness().await;
        })
...
// This must always be an update method or randomness state won't be updated
#[ic_cdk::update]
fn random_bytes(size: u32) -> String {
...
##### Benefits:
- Uses the system API only once as seed for initialization. This eliviates the pressure around the async nature of the system API.
- Extremely fast for large amount of randomness.

##### Caveats:
- One must be careful to always intialize the randomness in the init and post_upgrade hook.
- The randomness must be always obtained in an update call so the updated state of the PRNG is preserved. Else, the method will serve the same random bytes everytime.
