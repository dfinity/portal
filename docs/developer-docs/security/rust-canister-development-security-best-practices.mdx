...
In this Motoko example, the canister provides the requested size of random bytes by calling `raw_rand`. However, `raw_rand` can only generate 32 bytes of secure randomness in a single message and thus subsequent calls to the system API is required to fill the requested size.
...
- The method can be used in a query as it doesn't store any state.
...
##### Caveats:
- The `setup_randomness` must **always** be intialized in both the `init` and `post_upgrade` hook as `init` is not invoked during an canister upgrade (see here)[https://internetcomputer.org/docs/current/references/ic-interface-spec#system-api-upgrades].
- The `init` and `post_upgrade` methods doesn't allow async calls and thus a timer is immediately scheduled to setup the randomness.
