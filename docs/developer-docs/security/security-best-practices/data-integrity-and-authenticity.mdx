...
In certain use cases, there is a third option whereby query results can return data that has been certified by the subnet in an earlier update call. This is the concept of certified data and it requires changes to the update call to create the certification, query call to return the certificate and frontend to verify the certificate. Using certified data provides the best of both worlds with query-like response times and update-like certified responses. This forms the core of [certified variables](https://internetcomputer.org/docs/current/tutorials/developer-journey/level-2/2.2-advanced-canister-calls#certified-variables).
