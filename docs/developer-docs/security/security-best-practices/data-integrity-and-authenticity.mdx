...
ICP offers three modes of operations for canisters namely `update`, `query` and `composite_query`. For the sake of simplicity, we will club `composite_query` under queries for the rest of the section. Let's look into the differences between the two modes of operation,

| Query calls                                                                                            | Update Calls                                                                                                                                              |
| ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Query call is analogous to _READ_                                                                      | Update call is analogous to _WRITE_                                                                                                                       |
| The query call is executed on a single replica node chosen on random in a subnet by the boundary nodes | On an update call, the request is replicated among all the nodes in the subnet, goes through consensus and the response is threshold signed by the subnet |
| **Very fast response time**                                                                            | Relatively slow and more expensive (depending on the size of the subnet).                                                                                 |
| Relatively less secure since there is no integrity protection. The responding node maybe be malicious  | **Secure response, since it’s certified by the subnet and guarantees integrity**                                                                          |


Queries have fast response time but lack integrity guarantees, which makes them unfit for security critical applications. To obtain query like response times and update like certified responses, developers could make use of `certified_data` in the state tree. Alternatively, these calls have to be issued as update calls by the caller (e.g. in agent-js), but that impacts performance by taking a few seconds. Note that every query can also be issued as an update by the caller. Examples are asset certification in [Internet Identity](https://github.com/dfinity/internet-identity/blob/b29a6f68bbe5a49d048e12bc7a3263a9f43d080b/src/internet_identity/src/main.rs#L775-L808), [NNS dapp](https://github.com/dfinity/nns-dapp/blob/372c3562127d70c2fde059bc9c268e8ae858583e/rs/src/assets.rs#L121-L145), or the [canister signature implementation in Internet Identity](https://github.com/dfinity/internet-identity/blob/main/src/internet_identity/src/signature_map.rs).

> [!IMPORTANT]
>  ICP also provides Replica signed queries, where query responses are signed by the answering replica node but however it doesn't have the same security guarantees as `update` calls. Replica signed queries are enabled by default on both `agent-rs` and `agent-js`. You can read more about the feature [here](https://internetcomputer.org/blog/features/replica-signed-queries)

### What's Certified data?
...
* During query call, the canister can obtain a certificate about that data, which can be verified by the IC root public key.

In the rest of the section, we will see an example of how to design a canister, which can serve a certified response for a `query` using `certified_data` and how to verify the response. The examples are written in Rust but can be imported to other languages following the same recommendations.

### Building a canister with Certified Variables
Let's consider the following canister,
...
* **set_user** - The caller provides a `User` object to the canister. The canister records it and serves a corresponding `index` for the entry as the response. Since `certified_data` can only store 32 bytes of data, we use a specialized data strucutre from `ic_certified_map` to store the `User` data. 
  * The data strucutre internally stores the data in a `HashTree` and we record the `root_hash` of the data structure in the `certified_data`, which is 32 bytes. 
  * The `root_hash` cryptographically guarantees that only one tree can correspond to that hash. You can read more about it [here](https://docs.rs/ic-certified-map/latest/ic_certified_map/struct.RbTree.html).
* **get_user** - The caller provides a `index: nat64` to the canister and gets a Certified response for the corresponding `User`. The CertifiedUser response must have the following strucutre for verifying the response.
  ...
  * **witness** - The witness allows us to complete the final verification of the response with the requested input and `certified_data`.
...
### Verifying the Certified Variables

Once you have the response `CertifiedUser`, for the secureness guarantee, one must verify the certification in the response. This is broken down into several steps with an example below implementing these steps in Rust.
...
1. Recompute the `root_hash` of `certificate.tree` (pruned state tree with the canister's `certified_data`) and verify the `certificate.signature` with `root_hash` as message and `certificate.delegation` and IC root_key as the public key. This confirms that the signature is valid for the current state tree. 
2. Validate that the response is not stale by verifying the time at `/time` in `certificate.tree` is less than a certain delta of currrent time. The recommended delta is 5 minutes.
3. Recompute the `root_hash` of witness and verify equality with the `certified_data`. The `certified_data` can be obtained from `certificate.tree` under the path `/canister/<canister_id>/certified_data`
4. Check if query parameters are in the witness. In our example, the lookup path is `/user/<index>` and it should be present in the witness. 
5. Validate if the value found in `/user/<index>` matches `user` from the response. 
6. If all of previous steps succceed, return `user` as the valid response.
...
async fn main() {
    ...
    // Step 1: Check if signature in the certificate can be validated with the root_hash of the tree in certificate as message and root_key as public_key
