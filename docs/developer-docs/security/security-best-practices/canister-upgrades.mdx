...
- Avoid using `pre_upgrade` hooks if possible. Panics in the `pre_upgrade` hook prevent upgrade, and since the `pre_upgrade` hook is controlled by the old code, it can permanently block upgrading.
...
- In Motoko canisters, canister-wide timers should be set up in the actor initialiser during canister installation or re-installation. Canister-wide timers should be set in the `post_upgrade` hook for upgrades, as timers do not survive upgrades and must be explicitly set up thereafter.
