...
- Avoid using `pre_upgrade` and `post_upgrade` hooks. Panics in the `pre_upgrade` hook prevent upgrade, and since the `pre_upgrade` hook is controlled by the old code, it can permanently block upgrading.
