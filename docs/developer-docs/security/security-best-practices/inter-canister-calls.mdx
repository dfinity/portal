...
- In Motoko, one can use `try`/`finally` control flow construct. This construct guarantees lock release execution in the `finally` block regardless of any exceptions in the `try` block.
...
actor {
  ...
  let pending_requests = Map.HashMap<Text, Bool>(10, Text.equal, Text.hash);

  private func guard(principal : Principal) : Result.Result<Principal, Error> {
    if (pending_requests.get(Principal.toText(principal)) != null) {
      #err(Error.reject("Already processing a request for principal " # Principal.toText(principal)));
    } else {
      pending_requests.put(Principal.toText(principal), true);
      #ok(principal);
    };
  };

  private func drop(principal : Principal) : () {
    let _ = pending_requests.remove(Principal.toText(principal));
  };

  public shared (msg) func example_call_with_locking_per_caller() : async Result.Result<(), (Error.ErrorCode, Text)> {
    ...
    try {
      ...
      switch (guard(caller)) {
        case (#err(e)) {
          return #err(Error.code(e), Error.message(e));
        };
        case (#ok(_p)) {
          success := true;
        };
...
- A global lock that does not only lock per caller. For this, set a boolean flag in the canister state instead of using a `BTreeSet<Principal>` (rust) or `Map.HashMap<Text, Bool>` (Motoko).
