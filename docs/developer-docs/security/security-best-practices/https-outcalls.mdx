...
[Data confidentiality security recommendations.](/docs/current/developer-docs/security/security-best-practices/misc#data-confidentiality-on-the-internet-computer)
...
Make sure the endpoints, called by an HTTPS outcall, are idempotent, i.e. the queried endpoint has the same behaviour with the same request payload, no matter the number of times it is called.
