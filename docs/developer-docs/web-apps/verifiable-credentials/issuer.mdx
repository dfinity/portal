...
An **issuer** is a service or app that can issue a verifiable credential (VC) to a user. To become an issuer, your canister must implement the issuer API described in [verifiable credential spec](https://github.com/dfinity/internet-identity/blob/main/docs/vc-spec.md).
