...
First, the user must send a consent message to the issuer API endpoint `vc_consent_message` through the identity provider. A consent message is a text shown to the user to confirm that they consent to the issuing of a VC.
...
Next, the issuer API endpoint `derivation_origin` is called by the identity provider to obtain an URL to be used as the derivation origin for user's principal.
...
Before a credential can be issued, the request for the credential must be checked for validity using the `prepare_credential` API endpoint. The objective of this endpoint is to check the validity of the request, and upon success, prepare the actual credential requested by the user.
