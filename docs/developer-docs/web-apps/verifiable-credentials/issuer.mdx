...
An issuer is a service or app that can issue a verifiable credential to a user. To become an issuer, your canister must implement the verifiable credential interface using the issuer API. The issuer API is used to issue verifiable credentials, while another API, the identity provider API, is used by the [relying party](relying-party.mdx) to communicate with Internet Identity. Internet Identity is used to relay information between the issuer and the relying party when a verifiable credential is requested for a user.
...
First, the user must send a consent message to the issuer using the `vc_consent_message` API endpoint. A consent message is an approval from the user to validate that they consent the issuing of a VC.
...
If the response is an approval, the identity provider displays the consent message from `Icrc21ConsentInfo`.
...
:::info
The returned derivation origin is subject to verification via
...
:::info
The value of `prepared_context` is used to transfer information between the `prepare_credential` and `get_credential` steps. It is up to the issuer to decide on the content of this field since the issuer creates `prepared_context` and is the only entity that
consumes it.
...
`prepared_context` contains a time-stamped yet unsigned VC, for which the canister signature will be available through the `get_credential` call.
...
Lastly, the issuer must implement the `get_credential` API endpoint to issue the credential requested by the user. The issuer performs the same checks used to prepare the credentials, with an additional verify step to confirm that `prepared_context` is consistent with `prepare_credential`. If successful, the issuer returns the signed credential in JWS-format.
