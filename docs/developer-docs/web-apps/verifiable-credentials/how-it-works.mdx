...
Verifiable credentials on ICP rely on Internet Identity, which provides different and unlinkable identities for each dapp. A dapp utilizing verifiable credentials can’t ask for a user's credentials from the issuer of the VC directly. When a relying party wants to obtain a credential from an issuer, it must request it through Internet Identity.
...
4. The user decides whether to allow or revoke the action.

5. The II frontend shows all the details to the user: the relying party, the consent message, and the issuer, then waits for the user action.
