...
A **relying party** is a service or app that requests a user's verifiable credentials from the [issuer](issuer.mdx) API. Relying parties use the identity provider API to communicate with the issuer API using an **identity provider** such as Internet Identity. Identity providers must support the [verifiable credentials specification](https://github.com/dfinity/internet-identity/blob/main/docs/vc-spec.md). The identity provider API uses the [`window.postMessage()`](https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage) interface.
...
1. Load an identity provider such as Internet Identity: Open an identity provider that will act as the communication bridge between the relying party and the issuer API.
