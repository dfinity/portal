...
Note how the issuer's principal never appears in the credentials. Instead, the relying party and the issuer know about the `id_alias`. Therefore, this `id_alias` is also a credential to prove the link between the user's principal and the `id_alias`.
...
:::info
There is no signature in this first JWT.
:::
