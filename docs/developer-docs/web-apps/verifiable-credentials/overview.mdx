...
The second workflow allows you to [become a relying party](relying-party.mdx) for verifiable credentials. By requesting verifiable credentials from issuers, you can ensure that the claims that users make are verified by parties (issuers) that you trust, while preserving the user’s privacy.
