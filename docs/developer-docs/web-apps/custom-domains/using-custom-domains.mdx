...
By default, all canisters on the Internet Computer are accessible through the domain `icp0.io`
and their canister ID. In addition to that default domain, one can also make a
canister accessible under a custom domain using one of the following approaches:
...
The two approaches differ in the ease of use and configurability. When registering
...
the certificates, and provide your own infrastructure with support for the [HTTP gateway protocol](../../../references/http-gateway-protocol-spec.md). However, there is more configuration flexibility using this approach.
...
using the boundary nodes. First, the configuration steps are described, then
an [example is used to illustrate these steps](#concrete-example), followed by some
...
These records must be configured before continuing the next steps.
...
By default, `dfx` excludes all files and directories whose names start with a `.` from the asset canister. To include the `ic-domains` file, you need to create an additional file, called `.ic-assets.json` inside the same top-level directory as the `.well-known` directory:
...
need to update the DNS record of your domain, then notify a boundary node:
