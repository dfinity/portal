...
When a project has a frontend canister configured in its `dfx.json` file as `"type": "asset"`, `dfx` will compile the project's web assets into an **asset canister**. An asset canister is a special type of canister that uses Rust to compile the project's asset files into a Wasm module that can be installed and deployed to ICP. This is because canisters deployed on ICP must have a Wasm module.
