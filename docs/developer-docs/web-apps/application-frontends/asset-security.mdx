...
By default, asset canisters created with `dfx new` contain the following Content Security Policy (CSP) in the project's `.ic-assets.json` file:
...
This CSP includes `img-src data` as data images are frequently used, and `frame-ancestors: none` is used to mitigate [clickjacking attacks](https://owasp.org/www-community/attacks/Clickjacking).
...
- Use the [CSP Evaluator](https://csp-evaluator.withgoogle.com/) tool to validate your security policy. to validate the CSP you define.
...
- It is recommended to include [script hashes in combination with strict-dynamic](https://csp.withgoogle.com/docs/faq.html) in the CSP to account for nonces not being able to be used.
...
### `dfx v0.22.0` and older

`dfx` versions `0.22.0` and older include `script-src 'unsafe-eval'` in the default security policy. This is required for older versions because previous versions of `agent-js` used a WebAssembly module for the BLS signature validation. This has since been removed and is no longer included in the most recent versions of `dfx`.
