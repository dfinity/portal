...
2. **Fully on-chain inference using models trained off-chain**:
This assumes that the model was trained off-chain and then uploaded on-chain.
...
3. **Storing the model on-chain and running inference on a user's device**:
This assumes that the model was trained off-chain and then uploaded on-chain.
