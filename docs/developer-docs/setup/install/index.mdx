...
The [IC SDK](https://github.com/dfinity/sdk) is a **software development kit** used for creating and managing canister smart contracts on the ICP blockchain. The IC SDK supports [Motoko](../../backend/choosing-language) and [Rust](../../backend/choosing-language) programming languages by default, but [developers can use other languages](../../backend/choosing-language).

# Installing the IC SDK

You can download and install the latest version of the [IC SDK](https://github.com/dfinity/sdk) for creating and managing canister smart contracts by running the command below. The topics here provide additional information about installing, upgrading, and removing the IC SDK.
...
CDKs that are not installed but available for developers include the [Python CDK](https://demergent-labs.github.io/kybra/) and 
[TypeScript CDK](https://demergent-labs.github.io/azle/). Since CDKs are components used by the SDK, some developers choose to use the CDK directly (without the IC SDK), 
...
If a new version of the IC SDK is available for download after your initial installation, you should install the updated version at your earliest convenience to get the latest fixes and enhancements as soon as possible. You can use the `dfx upgrade` command to compare the version you have currently installed against the latest version available for download. If a newer version of `dfx` is available, the `dfx upgrade` command automatically downloads and installs the latest version.
...
The install script looks for the environment variable `DFX_VERSION` to check if it is supposed to install a specific version of the ICSDK or to simply install the latest version.
