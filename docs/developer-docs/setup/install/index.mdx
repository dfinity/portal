...
There is no native support for <code>dfx</code> on Windows. However, by installing the Windows Subsystem for Linux (WSL 2), you can run dfx also on a Windows system as described below (not all features may be supported though).
