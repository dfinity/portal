...
This command starts a local instance of the ICP replica using a single node testnet. ICP does not have a public testnet for deployment. The output will return a dashboard URL that can be used to view information about the local replica:
...
// Motoko is an Actor-based language and it represents the smart contract as an actor with various method that the users and other smart contract can call.
actor {
// This hello-world actor has a single function called greet. It is marked as query because it doesn't modify the state of the actor. 
...
Alternatively, you can interact with the smart contract via the web browser if a frontend smart contract has been deployed. [Learn more about using the frontend smart contract in the web browser](deploy-locally.md)


## Next steps

- [Create a developer account](accounts.md).
