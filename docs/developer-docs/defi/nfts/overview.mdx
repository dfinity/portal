...
ICP also supports dynamic NFTs that can be curated to evolve based on both data stored onchain or obtained through external APIs. NFT assets can be modified in real time by making external API requests using [HTTPS outcalls](/docs/current/developer-docs/smart-contracts/advanced-features/https-outcalls/https-outcalls-overview).
...
Learn how to [create an NFT collection on ICP](nft-collections.mdx).
