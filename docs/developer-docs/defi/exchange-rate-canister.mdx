...
The XRC calculates exchange rates using a unique aggregation method that selects a specific moment's data from exchanges, avoiding conventional time (TWAP) or volume-weighted (VWAP) techniques. It can derive complex rates (e.g., A/B from A/C and B/C rates) and, when direct pairs exist, it chooses the median rate after filtering. This approach ensures accuracy, timeliness, and minimizes manipulation risks, providing reliable rate information for decentralized applications.
