...
There are multiple ways of how to integrate with the ICP ledger or ICRC-1 ledgers. Previously, we have presented how users can integrate with the two ledgers using either 
ICRC Rosetta or ICP Rosetta. While it is commonly recommended to use these two implementations to integrate with the ledgers, it might be interesting for certain developers to
directly integrate with these ledgers. What a direct integration means, how it can be achieved and how it differs to the integration with Rosetta is topic of this section. 
...
Tokens on the internet computer are managed by smart contracts. For example, the ICP Ledger canister manages the list of balances for ICP tokens and implements methods for transferring tokens. It builds locally, i.e. in the persisted memory of the smart contract, a blockchain (chain of blocks) consisting of all ICP token transactions since genesis. The chain is authenticated by having the internet computer sign the tip of the chain. External parties can retrieve this chain and verify it is genuine by:
- verifying this signature (signature verification only, requires the public key of the Internet Computer) 
...
:::info 
Smart contract tokens other than ICP, e.g. ckBTC or CHAT, follow the ICRC suite of standards (ICRC-1, ICRC-3, etc) and can be integrated basically in the same way as the main ICP token. For differences in detail see below.
:::
...
*Rosetta integration* requires running, within the trust domain of an integrator, an instance of a Rosetta node. This is an abstraction layer that hides the ICP network and Ledger canister APIs. Interaction with the Rosetta node follows the [Rosetta API standard](https://docs.cloud.coinbase.com/rosetta/docs/welcome) proposed and maintained by Coinbase. 
...
Relevant aspects of execution semantics of ICP messages:
- The distinction between update calls and querry calls
- Reading the result of update calls
- Deduplication guarantees of the internet computer protocol
- Ingress message lifecycle 
- How to verify the result of calls to canisters
- API of the token canister(s) & their semantics
- Deduplication guarantees by the ledger 
- How to fetch the token ledger & verify it is genuine
