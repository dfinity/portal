...
def stake_maturity_operation(address,neuron_index = 0, operation_identifier = 0, percentage_to_stake = 100):
    return {
      "operation_identifier": { "index": operation_identifier },
      "type": "STAKE_MATURITY",
      "account": {
...
def main():
    ...
    stake_maturity_ops = stake_maturity_operation(client.construction_derive()["account_identifier"]["address"])
    neuron_info_ops = neuron_info_operation(client.construction_derive()["account_identifier"]["address"])
    print(json.dumps(client.make_and_submit_transaction([stake_maturity_ops]),indent=4))
