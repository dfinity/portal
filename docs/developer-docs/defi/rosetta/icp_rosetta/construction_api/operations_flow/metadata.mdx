...
An example of a call to the metadata endpoint would look like the following. 
```bash
...
and the response would look something like this. For the request type `TRANSACTION` the metadata includes the suggested fee for this operation. 
