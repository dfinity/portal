...
An example of a call to the derive endpoint would look like the following. 
```bash
...
and the response would look something like this. It represents the accountidentifier used by the ICP ledger. 
...
Building on the previous python code [snipped](/docs/developer-docs/defi/rosetta/icp_rosetta/construction_api/operations_flow/index.md) a call to the `construction/derive` endpoint written in python could be implemented like the following. Here the constant `NETWORK_ID` is the network_identifier
that can be fetched at the `network/list` [endpoint](/docs/developer-docs/defi/rosetta/icp_rosetta/data_api/network.md). 
