...
A call to the `construction/derive` endpoint written in Python could be implemented such as the following. 
In this example the python file for the Rosetta-Client is called `icp_rosetta_client.py` and the private key is located at `~/.config/dfx/identity/default/identity.pem`. 
```bash
