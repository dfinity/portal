...
which canister to call, at what endpoint and with which function parameters. The payloads is stipulated by the Rosetta-API standard and each payload gives information about which account has to sign what bytes in order for 
