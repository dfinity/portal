...
### Vote on proposal
Once you are sure about which proposal you want to vote on you will need the proposal id. It is contained in the proposal info which you retreived in the previous step. 
In this tutorial we will be voting on the proposal with the proposal id `12`. 
We also have to decide on what to vote meaning:
...
In this case we will be voting against the proposal. 
An example call for the `construction/payloads` endpoint looks like the following. 
```
...
The example implementation in python is.
```python
...
def main():
    ...
    register_vote_ops = register_vote_operation(client.construction_derive()["account_identifier"]["address"],proposal_id=12,vote=2, neuron_index = 1)
    print(json.dumps(client.make_and_submit_transaction([register_vote_ops]),indent=4))
