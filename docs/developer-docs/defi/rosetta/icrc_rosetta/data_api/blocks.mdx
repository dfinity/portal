...
```json
curl --location '0.0.0.0:8093/call'  --header 'Content-Type: application/json' --data '{
    "network_identifier": {
            "blockchain": "Internet Computer",
            "network": "mxzaz-hqaaa-aaaar-qaada-cai"
    },
...
If `highest_block_index > X` with `X` being the most recent block in the Rosetta database, then Rosetta will only return the blocks in the range `[highest_block_index,highest_block_index-number_of_blocks)` which have an block index that is smaller or equal `X`. 
...
A recommended way of using this endpoint is to first fetch the highest block index in the datbase through for example the `network/status` endpoint and then use this index to form a query for multiple blocks. 
