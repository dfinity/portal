# Overview
The ICRC Rosetta allows for communication with the ICRC-1 compatible ledgers through the Rosetta-API standard. Examples for them are the [ckBTC ledger](https://dashboard.internetcomputer.org/canister/mxzaz-hqaaa-aaaar-qaada-cai) and the [ckETH ledger](https://dashboard.internetcomputer.org/canister/ss2fx-dyaaa-aaaar-qacoq-cai).
This guide is split up into the endpoints of the [Data-API](/docs/developer-docs/defi/rosetta/icrc_rosetta/data_api/index.md) and that of the [Construction-API](/docs/developer-docs/defi/rosetta/icrc_rosetta/construction_api/index.md). 

## Set up an ICRC Rosetta node

You can set up a Rosetta API-compliant node to interact with the Internet Computer and your ICRC-1 ledger of choice.
...
### Step 2:  Pull the latest `dfinity/ic-icrc-rosetta-api` image from the Docker Hub by running the following command:
...
You will need to know the canister id of the ICRC-1 ledger you are trying to connect to. For example if we wanted to connect the to ckBTC ICRC-1 ledger with the canister id `mxzaz-hqaaa-aaaar-qaada-cai` which is running on mainnet, we would call the following. We also specify a port using `--port` which we then publish through docker. 
``` bash
docker run \
    --interactive \
    --tty \
...
```
The default port if not specified is `8080`. The default network is the testnet. For further help on what the options for ICRC-Rosetta are you can run.
``` bash
docker run \
    --interactive \
    --tty \
...
It **does not** connect to any ICRC-1 ledger canister running on the Internet Computer blockchain mainnet.
...
The reason is that each transaction's `created_at` field refers to an existing transaction (essentially `last_index` available locally at the time of transaction creation). More on this topic can be read on the section on [deduplication](https://github.com/dfinity/ICRC-1/blob/main/standards/ICRC-1/README.md#transaction_deduplication).
