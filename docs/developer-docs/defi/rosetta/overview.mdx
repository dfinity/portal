...
The communication with the governance canister will provide users with neuron management capabilities, fetching the state of neurons and proposals, as well as allowing for state changes in regards to staking, proposal voting. For details on how to call these endpoints, look at the guides for [ICP Rosetta](/docs/developer-docs/defi/rosetta/icp_rosetta/index.md). 
