...
There exist two implementations of the Rosetta-API standard. One for the [ICP Ledger](/docs/developer-docs/defi/rosetta/icp_rosetta/index.mdx) and the other for ledgers that implement the [ICRC-1](/docs/developer-docs/defi/rosetta/icrc_rosetta/index.mdx) standard. 

The Rosetta API standard consists of two parts: The [data API](https://www.rosetta-api.org/docs/data_api_introduction.html) is used to fetch data from the network. It is a read only endpoint and does not make any changes to the state of Rosetta. [View more details on how to use the data-api endpoints](/docs/developer-docs/defi/rosetta/icp_rosetta/data_api/index.mdx).
![Data API](data_api.png)

 The second part is the [construction API](https://www.rosetta-api.org/docs/construction_api_introduction.html). It is used to create transactions that require a valid signature from the user. More details on how to use the construction-api endpoints can be found [here](/docs/developer-docs/defi/rosetta/icp_rosetta/index.mdx) for ICP Rosetta and [here](/docs/developer-docs/defi/rosetta/icrc_rosetta/index.mdx) for ICRC Rosetta.
![Data API](construction_api.png)

## Canister communication
The Rosetta API is communicating with two separate canisters in the background. The communication with the ledger canister will provide the user with account balance statements, transferring, 
...
For details on how to call these endpoints, look at the guides for [ICP Rosetta](/docs/developer-docs/defi/rosetta/icp_rosetta/index.mdx) and [ICRC Rosetta](/docs/developer-docs/defi/rosetta/icrc_rosetta/index.mdx). 
