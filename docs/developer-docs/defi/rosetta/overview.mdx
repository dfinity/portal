...
After hex decoding, the first 4 bytes is the big-endian CRC32 checksum of the rest of the address. Then, call [`address_from_hex`](https://github.com/dfinity/rosetta-client#working-with-account-addresses) in the JavaScript SDK. It returns an error if checksum doesn’t match.
