...
This guide will show you how to deploy an index canister locally, and includes instructions for both an ICP ledger or an ICRC ledger. This guide will demonstrate how to connect it to your local ledger and how to interact with the index canister.

If you are working in a local development environment, you can't access the mainnet ledger nor the mainnet index canister. If your application is using the mainnet index canister and you want to test it, you can setup the index and ledger canisters locally. Neither of the two canisters will have any information about the state of the ledger on the mainnet. You will have to create your own transactions on the ledger so that the index can serve them through its endpoints.
...
- [Optional]: Create a new project folder. It is recommended to use the same project folder that you created during the local ledger setup, but alternatively, you can create a new one for the index canister.

:::caution

If you created a new project folder, you will either have to make sure the `dfx.json` file contains the correct data on the ledger of your choice, or you communicate with the ledger from its project folder that contains the correct `dfx.json` file. If you try to communicate with the ledger from a new project folder where the `dfx.json` file does not contain information on the ledger canister, you will not be able to create transactions.
...
Go to the [releases overview](https://dashboard.internetcomputer.org/releases) and copy the latest replica binary revision (IC_VERSION). At the time of writing, this is `b9a0f18dd5d6019e3241f205de797bca0d9cc3f8`.
...
```
curl -o index.wasm.gz "https://download.dfinity.systems/ic/b9a0f18dd5d6019e3241f205de797bca0d9cc3f8/canisters/ic-icp-index-canister.wasm.gz"
```
...
```
curl -o index.did "https://raw.githubusercontent.com/dfinity/ic/b9a0f18dd5d6019e3241f205de797bca0d9cc3f8/rs/rosetta-api/icp_ledger/index/index.did"
```
...
```
curl -o index.wasm.gz "https://download.dfinity.systems/ic/b9a0f18dd5d6019e3241f205de797bca0d9cc3f8/canisters/ic-icrc1-index-ng.wasm.gz"
```
...
:::caution
In this workflow, you cannot deploy this canister to the playground (using flag `dfx deploy --playground`) because the Motoko playground does not accept gzipped Wasm files.
:::
...
The ICP ledger uses an `AccountIdentifier`, which is calculated by hashing a Principal and Subaccount. This also means that the returned transactions will show accounts as the hash in bytes rather than the actual Accounts.
...
The ICRC-1 ledger uses an `AccountIdentifier`, which is calculated by hashing a Principal and Subaccount. This also means that the returned transactions will show accounts as the hash in bytes rather than the actual `Accounts`.
