...
Custom tokens can be created and deployed on ICP using the [ICRC-1](https://github.com/dfinity/ICRC-1/tree/main/standards/ICRC-1) or [ICRC-2](https://github.com/dfinity/ICRC-1/tree/main/standards/ICRC-2) token standards. Read about [token standards](token-standards.mdx) to learn more about what token standards are and what functionalities each one supports.

The ICRC-1 standard is considered the 'base' standard, as it defines a set of general functionalities that all ledgers must adhere to. Any tokens and their corresponding ledgers must fulfill all requirements of the ICRC-1 standard.

This guide will provide a quickstart on how you can create and deploy your own custom token on ICP using the ICRC-1 standard. [View the ICRC-2 example](https://github.com/dfinity/examples/tree/master/motoko/icrc2-swap) to learn how to create and swap ICRC-2 tokens.

## Setup a local ICRC-1 ledger

To create a new ICRC-1 token, you will need to first download and configure a local project for the ICRC-1 ledger canister. [Follow the ICRC-1 ledger documentation for instructions](/docs/current/developer-docs/defi/icrc-1/icrc1-ledger-setup).
...
The [controller principal](/docs/current/developer-docs/defi/cycles/cycles-wallet#controller-and-custodian-roles) of the archive canisters.
...
The amount of cycles that will be sent to the archive canister when it is created.
...
A flag for enabling or disabling certain extension standards to the ICRC-1 standard. In this case, the reference implementation also can support ICRC-2 transactions.
...
- Specify the initially minted tokens value by setting `initial_values = vec {<INITIAL_VALUES>}`. View the `ledger.did` file for the details of the argument.
...
[Learn how to interact with the ICRC-1 ledger](../icrc-1/using-icrc1-ledger.mdx)

## Resources

- [ICRC-1 ledger documentation](/docs/current/developer-docs/defi/icrc-1/icrc1-ledger-setup).
