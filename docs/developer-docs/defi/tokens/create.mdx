...
To create a new ICRC-1 token, you will need to first download and configure a local project for the ICRC-1 <GlossaryTooltip>ledger</GlossaryTooltip> <GlossaryTooltip>canister</GlossaryTooltip>. This is because each token will need its own ledger canister to record the token's balances and transactions. [Follow the ICRC-1 ledger documentation for instructions](/docs/current/developer-docs/defi/icrc-1/icrc1-ledger-setup).
