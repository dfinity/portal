...
Transfers sent to the minting account will generate `Burn` transactions, destroying tokens. A `icrc1_transfer` call from any principal with the 'to' value set as the minting account will burn tokens.
