...
Note that a such an upgrade can also be performed by upgrading the ledger canister to the same WASM that it is currently running, which results in the metadata being changed, but the functional behavior of the ledger otherwise remaining the same.
...
The above values SHALL be set using the explicitly provided fields in the `InitArgs` or `UpgradeArgs`, for canister installs or upgrades, respectively, e.g., the `token_symbol` field for setting the token symbol.
Other metadata, such as the token logo, SHALL be set as `(key, value)` pairs in the `metadata` `Vec`.
The format SHOULD follow what is specified in the [standard](https://github.com/dfinity/ICRC-1/tree/main/standards/ICRC-1#metadata), e.g., for the token logo, the metadata key SHALL be `icrc1:logo`.
