...
### dfx
Here is an example command for a token transfer using `dfx`:
```bash
...
### Rust
Here is an example Rust code snippet for making a token transfer:
```bash
...
### dfx
Here is an example command for an approval transaction using `dfx`:
```bash
...
### Rust
Here is an example Rust code snippet for making an approval transaction:
```bash
...
### dfx
Here is an example command for a TransferApproved transaction using `dfx`:
```bash
...
### Rust
Here is an example Rust code snippet for making a TransferApprove transaction:
```bash
let to = icrc_ledger_types::icrc1::account::Account{ owner:ic_principal::Principal::from_slice(&[1, 2, 3, 4]) , subaccount: None};
