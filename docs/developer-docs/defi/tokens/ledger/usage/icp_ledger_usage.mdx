...
pub async fn icp_transfer(from_subaccount: Option<Subaccount>, to: Account, memo: Option<Vec<u8>>, amount: NumTokens) -> Result<(), String> {
    ...
    let args = TransferArg {
        ...
        // sender's account. The fee is fixed to 10_000 e8s (0.0001 ICP). It is specified here
        // to ensure that it hasn't changed. Using None would use whatever the current fee is, even
        // if it changes.
        fee: Some(NumTokens { 10_000 }),
        // The created_at_time is used for deduplication, which is not used in this example as
        // it uses unbounded-wait calls. You should, however, set it if you opt to use bounded-wait
