...
`dfx` does not come with an ICP ledger instance installed by default. To be able to use this command you will need to install the ICP ledger locally. You can do that by following this [guide](/docs/current/developer-docs/defi/tokens/ledger/setup/icp_ledger_setup.mdx) or by installing the NNS locally:
``` bash
...
- `dfx ledger --ledger-canister-id <CANISTER_ID> ...`: interact with the [locally deployed](/docs/current/developer-docs/defi/tokens/ledger/setup/icp_ledger_setup.mdx) ICP ledger on your local network.
:::
...
For all other endpoints you can use the request and response structure from the `ledger.did` Candid file. How to retrieve the Candid file is explained in [the guide on deploying an ICP ledger locally](/docs/current/developer-docs/defi/tokens/ledger/setup/icp_ledger_setup.mdx).

### `icrc-ledger-types` Rust crate
As explained in the [token standards](/docs/current/developer-docs/defi/tokens/token-standards), the ICP ledger supports all ICRC-1 endpoints. You will need to define the structures used for these endpoints.
