...
[ICRC](https://github.com/dfinity/ICRC) stands for `Internet Computer Request for Comments'. ICRC standards are proposed by the community working group and are not specific to tokens; they can be created for anything on ICP, such as services, canister calls, or authentication methods. However, this documentation will detail the ICRC standards that are specific to tokens.
...
- Once the propsal is deemed ready for implementation by the working group, it is proposed to the NNS through an NNS proposal.
...
-   Accounts are identified by a pair `(owner, subaccount)`.
...
ICRC-7 is still in the drafting and development phases. It has not yet been implemented on the ICP mainnet.
...
ICRC-37 is still in the drafting and development phases. It has not yet been implemented on the ICP mainnet.
