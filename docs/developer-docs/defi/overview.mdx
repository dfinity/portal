...
[Chain-key](/docs/current/developer-docs/multi-chain/chain-key-tokens/overview) tokens are created using ICP's chain-key cryptography protocols, such as [threshold signing](https://internetcomputer.org/docs/current/concepts/glossary/#chain-key). The chain-key tokens ckBTC, ckETH and ckERC20 use the ICRC-2 standard. Each chain-key token is deployed on ICP and backed 1:1 with the native token, such as BTC, ETH, or ERC20, held 100% on-chain in an ICP smart contract. There are no intermediaries or centralized bridges used for chain-key tokens.
