...
- Digital records corresponding to physical items, such as real estate or luxury goods.
...
[Chain-key](/docs/current/developer-docs/multi-chain/chain-key-tokens/overview) tokens are created using ICP's chain-key cryptography protocols, such as [threshold signing](https://internetcomputer.org/docs/current/concepts/glossary/#chain-key). The chain-key token ckBTC uses the ICRC-1 standard, while ckETH and ckERC20 use the ICRC-2 standard. Each chain-key token is deployed on ICP and backed 1:1 with the native token, such as BTC, ETH, or ERC20, held 100% on-chain in an ICP smart contract. There are no intermediaries or centralized bridges used for chain-key tokens.

An older token standard, [DIP-721](https://github.com/Psychedelic/DIP721), is designed to resemble the Ethereum ERC-721 NFT standard. While some projects on ICP still use DIP-721, the new ICRC-7 standard is designed to provide a newer, alternative NFT standard.
...
- **Principal**: An entity that can be authenticated by ICP. Principals are used to identify canisters, individuals, services, ledger accounts, or other components deployed on ICP. A principal can have multiple accounts.

- **Subaccount**: A 32-byte string representing an account of a principal.
...
- **Index**: Allows for querying a transaction list for a specific account rather than querying a ledger's entire history.
...
ICP utilizes two different types of ledgers:
...
The ICP ledger is a specific implement that follows no official standard. It was created before the ICRC-1 standard had been conceptualized and created.
...
See the documentation on [how to interact with the ICRC ledger for more info](/docs/current/developer-docs/defi/icrc-1/icrc-1ledger-setup).
...
Since there are multiple ICRC-1 compatible ledgers, there will be multiple ICRC-1 index canisters. For example, the ICRC-1 ledger recording ckBTC transactions is running under the canister ID [n5wcd-faaaa-aaaar-qaaea-cai](https://dashboard.internetcomputer.org/canister/n5wcd-faaaa-aaaar-qaaea-cai).
