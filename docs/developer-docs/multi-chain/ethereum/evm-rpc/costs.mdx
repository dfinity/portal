...
In addition to the cost of the RPC request itself, callers must also pass at least 0.0003 TC of "collateral cycles" to account for possible future downstream API price increases. The canister currently refunds all of these cycles, but this may change in the future.
