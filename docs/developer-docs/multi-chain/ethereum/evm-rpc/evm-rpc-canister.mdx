...
Then start the local replica and deploy the canister locally with a specified number of nodes (`28` for the [fiduciary subnet](/docs/current/developer-docs/backend/subnet-types#fiduciary-subnets)):
...
Note that even if an error is returned, you should assume that your transaction is known to the Ethereum network. You can verify whether your transaction was included in a block by querying the transaction count (`eth_getTransactionCount`) at `Latest block height`.

If you receive this error, but your transaction can be seen on Ethereum, then this error is expected. This is because HTTPS outcalls are replicated, and it is expected that only one of the contacted nodes from the JSON-RPC provider should return the `success` reply. Others will return an error indicating the transaction is already known.
...
```
"{\"code\":429,\"message\":\"Your app has exceeded its compute units per second capacity. If you have retries enabled, you can safely ignore this message. If not, check out https://docs.alchemy.com/reference/throughput\"}"
```

The Alchemy JSON-RPC provider has a low rate limit capacity, and is not recommended to be used in production projects for this reason. It is recommended to use another [JSON-RPC provider](/docs/current/developer-docs/multi-chain/ethereum/evm-rpc/overview#supported-json-rpc-providers) for production applications.
