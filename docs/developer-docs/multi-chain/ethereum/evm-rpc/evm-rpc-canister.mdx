...
If you receive this error, but your transaction can be seen on Ethereum, then this error is expected because HTTPS outcalls are replicated meaning only one of the contacted nodes from the JSON-RPC provide should return the success results, while others return an error indicating the transaction is already known.
