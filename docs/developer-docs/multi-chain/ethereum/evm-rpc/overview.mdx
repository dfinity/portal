...
Calls made to the EVM RPC canister cost [cycles](/docs/current/developer-docs/getting-started/tokens-and-cycles). The cost per request is generally similar to making the equivalent [HTTPS outcalls](/docs/current/references/https-outcalls-how-it-works#pricing) in your own canister.
