...
The goal of the EVM RPC canister is to act as a gateway between canisters on ICP and EVM-compatible networks through RPC services such as Alchemy, Ankr, or Cloudflare. The EVM RPC canister provides the necessary facilities to send requests to the Ethereum JSON-RPC API and receive responses that are immediately useful in the context of a canister.
