...
1. [Generate a public key](/docs/current/developer-docs/multi-chain/generating-addresses): To sign and submit transactions, your canister will need a public key. You will also need this public key to generate an Ethereum wallet address.

2. [Sign a transaction](signing-transactions.mdx): Next, a transaction must be signed before it can be submitted to the Ethereum network and executed. Transactions are signed using [threshold ECDSA](/docs/current/references/t-ecdsa-how-it-works), a novel protocol as part of ICP's chain-key signatures suite.

3. [Submit a transaction](submit-transactions.mdx): Signed raw transactions can be submitted using the EVM RPC canister's RPC method `eth_sendRawTransaction`. Code examples for sending transactions can be found in the [EVM RPC canister](/docs/current/developer-docs/multi-chain/ethereum/evm-rpc/evm-rpc-canister) documentation.

4. [Read information from the Ethereum network](/docs/current/developer-docs/multi-chain/ethereum/evm-rpc/evm-rpc-canister): Information from the Ethereum network can be queried using RPC methods such as:
...
Each of these steps can be executed programmatically within a smart contract that is deployed on ICP. View the [EVM RPC canister](/docs/current/developer-docs/multi-chain/ethereum/evm-rpc/evm-rpc-canister) documentation to see code examples for each RPC method.

Additional sample projects can be found in the [EVM RPC](../evm-rpc/samples.mdx) canister documentation.
...
Chain-key Ethereum (ckETH) is a digital twin token of Ethereum that is backed 1:1 with Ethereum held within an ICP smart contract. ckETH tokens comply with the [ICRC token standard](/docs/current/developer-docs/multi-chain/ethereum/cketh/overview).
...
Currently, the ckERC20 token ckUSDCSepolia is available for testing. This token is still under development and there may be bugs. 

You can try ckUSDCSepolia by following the testing [instructions on the developer forum](https://forum.dfinity.org/t/cketh-a-canister-issued-ether-twin-token-on-the-ic/22819/155).

## Resources

- [EVM RPC canister](/docs/current/developer-docs/multi-chain/ethereum/evm-rpc/evm-rpc-canister).

- [Generating addresses](/docs/current/developer-docs/multi-chain/generating-addresses).
