...
**ICP**: Developers use dfx which is a similar development environment. [DFX](../../../../developer-docs/getting-started/install/#installing-dfx-via-dfxvm) is a command-line tool that interacts with the IC SDK and allows developers to compile and deploy their canisters.
...
**ICP**: Currently, testing is done in the local environment. A [playground environment](/docs/current/developer-tools/ide/playground.mdx) on mainnet is available for canisters, but deployment time is limited to 20 minutes. However, the ability to upgrade and delete canisters on the mainnet allows developers greater flexibility to iterate and refine their canisters.
