...
**ICP**: Currently, testing is done in the local environment. A [playground environment](/docs/current/developer-docs/developer-tools/ide/playground/) on mainnet is available for canisters, but deployment time is limited to 20 minutes. However, the ability to upgrade and delete canisters on the mainnet allows developers greater flexibility to iterate and refine their canisters than on Ethereum.
