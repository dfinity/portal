...
To create an ETH address for your canister, first you will need to obtain a public key that will be used to sign your canister's Ethereum transactions using [threshold ECDSA chain-key signatures](docs/current/developer-docs/smart-contracts/encryption/t-ecdsa). You can get this key by calling the `ecdsa_public_key` API method of the management canister:
