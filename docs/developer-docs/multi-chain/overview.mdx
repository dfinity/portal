...
- Full Web 2 integration allows canisters to both expose [HTTPS interfaces](/docs/current/developer-docs/smart-contracts/advanced-features/serving-http-request) and call [HTTPS APIs hosted on the web](/docs/current/developer-docs/smart-contracts/advanced-features/https-outcalls/making-http-requests).
