...
A key feature of the ICP's Chain Fusion capabilities is [threshold signing](/docs/current/developer-docs/smart-contracts/encryption/t-ecdsa/). This feature allows canisters to control accounts on other blockchains. The private keys that are used to sign transactions are never accesible in a single physical place, but securely distributed in form of shares among a group of nodes which perform a threshold signing protocol to generate a signature on behalf of the canister.
...
- **[HTTPS outcalls](/docs/current/developer-docs/security/rust-canister-development-security-best-practices/#http-outcalls)**: Replicated calls into external web services.
