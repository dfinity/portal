---
keywords: [advanced, ethereum, cketh, erc20, ckerc20, chain-key ethereum, overview]
---
...
Just like other chain-key tokens, such as [ckBTC](/docs/current/developer-docs/multi-chain/chain-key-tokens/ckbtc/overview) and [ckETH](/docs/current/developer-docs/multi-chain/chain-key-tokens/cketh/overview), the ckERC20 tokens do not rely on any centralized bridge to facilitate the conversion between the ckERC20 token and the ERC-20 tokenETH and ckETH, which makes the conversion substantially more secure compared to other traditional 'wrapped' tokens.

Moreover, like ckETH, ckERC20 tokens offer the same advantages over their counterparts on Ethereum, which are faster and cheaper transactions.

A ckERC20 token can be created for any ERC-20 token. The process of adding a ckERC20 token involves an NNS proposal as described [here](https://github.com/dfinity/ic/blob/master/rs/ethereum/ledger-suite-orchestrator/README.adoc#add-new-ckerc20).
Note that the [same requirements](/docs/current/developer-docs/multi-chain/chain-key-tokens/overview#requirements-for-chain-key-tokens) as for any other type of chain-key token apply.

## How it works

The ckERC20 functionality is primarily provided by two canisters, the **ckETH minter** and the **ledger suite orchestrator (LSO)**, and a helper smart contract on Ethereum:
...
1. The user submits an Ethereum transaction calling the `approve` function of the ERC-20 smart contract to allow the helper smart contract to withdraw some the user's funds.
...
2. The user calls the `icrc2_approve` function on the ledger of the given ckERC20 token to approve the minter to burn some of the user' ckERC20 tokens.

3. The user calls the `withdraw_erc20` function on the ckETH minter, specifying the canister ID of the ledger of the given ckERC20 token, the amount to be withdrawn, and the Ethereum destination address. The ckETH minter then issues transactions to burn the fee in ckETH and the specififed amount of the ckERC20 token. If both operations succeed, it sends an Ethereum transaction that transfers the specified amount from the helper smart contract to the destination address that the user provided.
...
The ledger and index canisters in any ckERC20 ledger suite are regular ICRC-1 compliant canisters. A guide on how to interact with these canisters can be found in the [using the ICRC-1 ledger](/docs/developer-docs/defi/icrc-1/using-icrc1-ledger.mdx) section.
