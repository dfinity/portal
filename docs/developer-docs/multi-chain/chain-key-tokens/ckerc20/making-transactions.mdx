...
First [download the ckETH repository](https://github.com/dfinity/ic/tree/master/rs/ethereum/cketh). Then, navigate into the subdirectory for either the mainnet(ic/rs/ethereum/cketh/mainnet) or testnet(ic/rs/ethereum/cketh/testnet).

:::note

Note that ERC-20 tokens can only be converted if the ICRC ledger suite for the
corresponding ckERC20 token exists. The list of supported ckERC20 tokens can be obtained by running the following command:

:::

```
dfx canister --ic call minter get_minter_info
```
...
Currently, the ckERC20 helper contracts are the following.
...
Note that the helper smart contract might change. It is therefore recommended to request the (ckETH) minter info and extract the helper smart contract address from the response.

:::

If the targeted ckERC20 token is listed, the first step is to create an Ethereum transaction that calls the `approve` function on the corresponding ERC-20 contract address, allowing the helper smart contract to withdraw some of the user's funds.
...
ckERC20 tokens are standard ICRC-1 tokens and can be transferred by calling the `ircr1_transfer` function.

```
dfx canister --ic call CKERC20_LEDGER "(record { to = DESTINATION_ACCOUNT; amount = AMOUNT:nat})"
```

The placeholders `CKERC20_LEDGER`, `DESTINATION_ACCOUNT`, and `AMOUNT` must be replaced by the canister ID of the targeted ckERC20 ledger, the destination account, and the desired amount, respectively. More information about the `icrc1_tranfer` function can be found in the [ICRC-1 standard specification](https://github.com/dfinity/ICRC-1/tree/main/standards/ICRC-1).
...
The ckERC20 tokens have not been added to the Internet Computer dashboard yet.
The history of transactions can be obtained from the index canister of the ckERC20 ledger suite.
...
`LSO` must be replaced by the canister ID of the ledger suite orchestrator associated with the targeted Ethereum network (for example, it is `vxkom-oyaaa-aaaar-qafda-cai` for Ethereum mainnet), `CHAIN_ID` must be replaced by the chain ID of the targeted Ethereum network (for example, `chain_id = 1` for Ethereum mainnet). Lastly, `ERC_20_CONTRACT_ADDRESS` must be replaced by the corresponding ERC-20 smart contract address (for example, `address = "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"` for USDC).
