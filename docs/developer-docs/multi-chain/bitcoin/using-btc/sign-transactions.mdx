...
The following snippet shows a simplified example of how to sign a Bitcoin transaction for the special case where

- All the inputs are referencing outpoints that are owned by `own_address`.
- `own_address` is a `P2PKH` address.
...
[Learn more about using the Threshold signing API](/docs/current/developer-docs/smart-contracts/encryption/t-ecdsa).
