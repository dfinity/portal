...
The Internet Computer employs [Orthogonal Persistence](../build/languages/motoko/motoko.md#orthogonal-persistence), so developers generally do not need to think a lot about storing their data.
...
beneficial to use a [query call instead of an update call](../../concepts/canisters-code.md#query-and-update-methods).
But, since query calls do not go through consensus, [certified responses](../good-to-know/security/general-security-best-practices.md#certify-query-responses-if-they-are-relevant-for-security)
...
On the Internet Computer, this non-existing principal is called the [Management Canister](../../references/ic-interface-spec.md#the-ic-management-canister).
