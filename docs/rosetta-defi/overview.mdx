...
ICP tokens are the network's native utility token. They are used for different actions on the network, such as being converted into <GlossaryTooltip>[cycles](/docs/current/building-dapps/getting-started/tokens-and-cycles)</GlossaryTooltip> to pay for a <GlossaryTooltip>canister</GlossaryTooltip>'s resources, or being staked in a neuron to receive voting power on the [<GlossaryTooltip>NNS</GlossaryTooltip>](/docs/current/docs/current/move-to-learnhub/nns/overview).
...
[Chain-key](/docs/current/building-dapps/chain-fusion/chain-key-tokens/overview) tokens are created using ICP's chain-key cryptography protocols, such as [threshold signing](https://internetcomputer.org/docs/current/developer-education/glossary/index#chain-key). The chain-key tokens ckBTC, ckETH, and ckERC20 use ICRC-1 ledgers that support the ICRC-2 extension. Each chain-key token is deployed on ICP and backed 1:1 with the native token, such as BTC, ETH, or ERC20, locked and kept on their native chain. There are no intermediaries or centralized bridges used for chain-key tokens.
...
Each token standard is elaborated on in the [token standards documentation](/docs/current/rosetta-defi/tokens/token-standards).
...
- The ICP ledger canister that is running on the [NNS](/docs/current/docs/current/move-to-learnhub/nns/overview).

- ICRC-1 ledgers.

### ICP ledger

The main purpose of the ICP ledger is to record `burn`, `mint`, `approve`, and, most commonly, `transfer` transactions with regards to ICP's native [token](/docs/current/rosetta-defi/tokens/token-standards#icp-tokens) `ICP`. The ICP ledger canister services transaction requests and offers a variety of endpoints to fetch data and information about the state of the ICP ledger.

There is only one ICP ledger on the mainnet.

See the documentation on [how to interact with the ICP ledger for more info](/docs/current/rosetta-defi/token-ledgers/usage/icp_ledger_usage).
...
[Learn more about index canisters](/docs/current/rosetta-defi/tokens/indexes).
...
- [Receiving ICP tokens as governance rewards](/docs/current/move-to-learnhub/nns/concepts/neurons/staking-voting-rewards/).
...
[Learn more about token standards](/docs/current/rosetta-defi/tokens/token-standards).