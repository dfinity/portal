...
[Chain-key](/docs/current/building-dapps/chain-fusion/chain-key-tokens/overview) tokens are created using ICP's chain-key cryptography protocols, such as [threshold signing](https://internetcomputer.org/docs/current/developer-education/glossary#c). The chain-key tokens ckBTC, ckETH, and ckERC20 use ICRC-1 ledgers that support the ICRC-2 extension. Each chain-key token is deployed on ICP and backed 1:1 with the native token, such as BTC, ETH, or ERC20, locked and kept on their native chain. There are no intermediaries or centralized bridges used for chain-key tokens.
...
Each token standard is elaborated on in the [token standards documentation](/docs/current/rosetta-defi/token-standards/index).
...
The main purpose of the ICP ledger is to record `burn`, `mint`, `approve`, and, most commonly, `transfer` transactions with regards to ICP's native [token](/docs/current/rosetta-defi/token-standards/index#icp-tokens) `ICP`. The ICP ledger canister services transaction requests and offers a variety of endpoints to fetch data and information about the state of the ICP ledger.
...
[Learn more about token standards](/docs/current/rosetta-defi/token-standards/index).