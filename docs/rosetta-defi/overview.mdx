...
There are two implementations of the Rosetta-API standard. One for the [ICP Ledger](/docs/current/rosetta-defi/rosetta/icp_rosetta/index) and the other for ledgers that implement the [ICRC-1](/docs/current/rosetta-defi/rosetta/icrc_rosetta/index) standard.

## Types of tokens

ICP tokens are used for different actions on the network, such as being converted into <GlossaryTooltip>[cycles](/docs/current/building-dapps/getting-started/tokens-and-cycles)</GlossaryTooltip> to pay for a <GlossaryTooltip>canister</GlossaryTooltip>'s resources, or being staked in a neuron to receive voting power on the [<GlossaryTooltip>NNS</GlossaryTooltip>](/docs/current/move-to-learnhub/nns/overview).
...
[Chain-key](/docs/current/rosetta-defi/chain-key-tokens/overview) tokens are created using ICP's chain-key cryptography protocols including [threshold signing](/docs/current/building-dapps/using-network-features/signatures/t-ecdsa) and adhere to the ICRC standards such as ICRC-1 and ICRC-2. For example, the chain-key tokens ckBTC, ckETH, and ckERC20 use ICRC-1 ledgers that support the ICRC-2 extension. Each chain-key token is deployed on ICP and backed 1:1 with the native token, such as BTC, ETH, or ERC20, locked and kept on their native chain. There are no intermediaries or centralized bridges used for chain-key tokens.
...
Each token standard is elaborated on in the [token standards documentation](/docs/current/rosetta-defi/token-standards/).
...
- The ICP ledger canister that is running on the [NNS](/docs/current/move-to-learnhub/nns/overview): Used to record `burn`, `mint`, `approve`, and, most commonly, `transfer` transactions with regards to ICP's native [token](/docs/current/rosetta-defi/token-standards/) `ICP`. The ICP ledger canister services transaction requests and offers a variety of endpoints to fetch data and information about the state of the ICP ledger.
...
See the documentation on [how to interact with the ICRC ledger for more info](/docs/current/rosetta-defi/token-ledgers/setup/icrc1_ledger_setup).
