...
:::caution
The cycles ledger is supported by default in [`dfx` version 0.22.0 and newer](/docs/current/building-dapps/getting-started/install).
:::
...
- To create a canister, the fee is the transfer fee (100M cycles), plus the canister creation fee, which scales [linearly with the size of the subnet](/docs/current/developer-education/developer-concepts/gas-cost) that the canister is created on. For example, if the canister is created on a 13-node subnet, the fee is 100B cycles. This means the total fee will be 100_100_000_000 cycles.
