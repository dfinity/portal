...
1. Use [`dfx nns`](/docs/current/building-dapps/developer-tools/dfx/dfx-nns) to deploy the entire NNS locally. Since the ICP ledger is part of the NNS, this command will also install an ICP ledger with canister ID `ryjl3-tyaaa-aaaaa-aaaba-cai`. This solution is fast and straightforward, but also more heavyweight.
...
If you want to deploy your own token, read the [guide on setting up an ICRC-1 ledger](/docs/current/rosetta-defi/token-ledgers/setup/icrc1_ledger_setup).
:::

## Using `dfx nns install`

The [`dfx nns install`](/docs/current/building-dapps/developer-tools/dfx/dfx-nns) command can be used to deploy an instance of the entire NNS locally. The NNS includes the ICP ledger canister with canister ID `ryjl3-tyaaa-aaaaa-aaaba-cai`.
...
You can also pass these init args to the canister [using the command line](/docs/current/building-dapps/developer-tools/advanced-dfx/init-args/#using-init_arg) or by [storing them in a file](/docs/current/building-dapps/developer-tools/advanced-dfx/init-args/#using-an-init_arg_file), then referencing that file in your `dfx.json` configuration:
...
[Learn more about init args](/docs/current/building-dapps/developer-tools/advanced-dfx/init-args/).
