...
`dfx` does not come with an ICP ledger instance installed by default. To be able to use this command, you will need to install the [ICP ledger locally](/docs/current/rosetta-defi/token-ledgers/setup/icp_ledger_setup).
...
:::info
To interact with the ICRC-1 endpoints of the ICP ledger, learn more about [interacting with an ICRC-1 ledger](/docs/current/rosetta-defi/token-indexes/).
:::
...
As explained in [token standards](/docs/current/rosetta-defi/token-standards/), the ICP ledger supports all ICRC-1 endpoints. You will need to define the structures used for these endpoints.
