...
To learn more about ckERC20, please refer to the [ckERC20 docs](/docs/current/rosetta-defi/chain-key-tokens/ckerc20/overview).
...
To find any proposed ckERC20 tokens, view any open governance proposals that intend to upgrade the **ledger suite orchestrator** (LSO), which has the canister ID `vxkom-oyaaa-aaaar-qafda-cai`. Navigate to the [proposals section on the governance page](https://dashboard.internetcomputer.org/governance) and check for any open proposals with the title "Upgrade NNS Canister: vxkom-oyaaa-aaaar-qafda-cai to wasm." Click on the proposal to see the details of the intended ckERC20 tokens. For example, [Proposal 130755](https://dashboard.internetcomputer.org/proposal/130755) is the governance proposal to create ckPEPE.
