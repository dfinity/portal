...
As discussed in [tokens and cycles](/docs/current/building-dapps/getting-started/tokens-and-cycles), ICP tokens can be converted into **cycles** to power <GlossaryTooltip>canister</GlossaryTooltip> operations. Cycles reflect the operational cost of communication, computation, and storage that dapps consume.
...
:::caution
Please note that the cycles wallet will be removed from dfx in a future release. We recommend using the [cycles ledger](/docs/current/rosetta-defi/token-ledgers/cycles-ledger) feature instead, which simplifies the cycles management workflow.
:::
...
In a production environment, however, you need to explicitly register and transfer cycles to new canisters, specify the principals that can act as custodians, and manage the principals with ownership rights. You can perform some of these tasks using the default cycles wallet dapp running in a web browser. Depending on the specific action you want to take, you can also perform these cycle and canister management tasks by running `dfx wallet` commands in a terminal or by calling methods in the default cycles wallet canister directly. You can learn more about [how to use `dfx wallet`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet).
...
There are dapps that can help you convert ICP to cycles and create a new cycles wallet, e.g., [NNS dapp](/docs/current/move-to-learnhub/nns/using-the-nns-dapp/nns-app-quickstart#_deploy_a_canister_with_cycles).
...
For more details, see the [dfx wallet reference](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet).
...
- [`add-controller`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-add-controller): Add a controller using the selected identity's principal.
- [`addresses`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-addresses): Displays the address book of the cycles wallet.
- [`authorize`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-authorize): Authorize a custodian by principal for the selected identity's cycles wallet.
- [`balance`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-balance): Displays the cycles wallet balance of the selected identity.
- [`controllers`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-controllers): Displays a list of the selected identity's cycles wallet controllers.
- [`custodians`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-custodians):    Displays a list of the selected identity's cycles wallet custodians.
- [`deauthorize`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-deauthorize): Deauthorize a cycles wallet custodian using the custodian's principal.
- [`help`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-help): Displays a usage message and the help of the given subcommand(s).
- [`name`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-name): Returns the name of the cycles wallet if you've used the dfx wallet set-name command.
- [`redeem-faucet-coupon`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-redeem-faucet-coupon): Redeem a code at the cycles faucet.
- [`remove-controller`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#remove-controller): Removes a specified controller from the selected identity's cycles wallet.
- [`send`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#send): Sends a specified amount of cycles from the selected identity's cycles wallet to another cycles wallet using the destination wallet canister ID.
- [`set-name`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#set-name): Specify a name for your cycles wallet.
- [`upgrade`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#upgrade): Upgrade the cycles wallet's Wasm module to the current Wasm bundled with `dfx`.
