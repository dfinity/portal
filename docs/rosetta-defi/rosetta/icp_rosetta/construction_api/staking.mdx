...
This section will give you an overview of how how ICP staking on the Internet Computer using Rosetta works.

This guide assumes you are familiar with the [flow of operations](/docs/rosetta-defi/rosetta/icp_rosetta/construction_api/operations-flow).

### Prerequisites

- You have a Rosetta instance running at: `http://127.0.0.1:8081`.

- You are familiar with the concept of staking on the NNS in general. To read up on how staking works on ICP,
...
To derive the account identifier of your public key you can look at how the [derive endpoint works](/docs/rosetta-defi/rosetta/icp_rosetta/construction_api/operations-flow).
...
However, the governance canister must be notified of this transfer to activate the neuron. This process is done in the [next step of the guide](/docs/rosetta-defi/rosetta/icp_rosetta/construction_api/staking).
...
After that, you can fetch the [neuron info](/docs/rosetta-defi/rosetta/icp_rosetta/construction_api/staking) and it should display that it is now in `DISSOLVING` state.
```json
{
...
There are a few variations to this that depend on whether you are the controller of the neuron or whether you are using a [hotkey](/docs/rosetta-defi/rosetta/icp_rosetta/construction_api/staking).
