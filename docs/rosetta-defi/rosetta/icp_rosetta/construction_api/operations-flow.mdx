...
- Request-specific metadata: These are options that can be set for specific request types. For ICP Rosetta, they are relevant for `TRANSACTION` request types.
    - `memo`: See the [section](/docs/current/rosetta-defi/token-standards/) for `memo` for the ICP ledger.
    - `created_at_time`: See the [section](/docs/current/rosetta-defi/token-standards/) for `created_at_time` for the ICP ledger.
