...
For instructions on how to install the ICRC-1 Ledger, follow [ICRC-1 Ledger Deployment](/docs/current/rosetta-defi/token-ledgers/setup/icrc1_ledger_setup).
For instructions on how to interact with the ICRC-1 Ledger, follow [ICRC-1 Ledger Interaction](/docs/current/rosetta-defi/tokens/indexes).
...
However, you might also consider interfacing with SNSess if you work on a project that builds a wallet dapp or DEX, for example, to provide an interface to the SNS swap, the SNS governance, or the SNS ledger.
...
* [SNS wrapper](https://github.com/dfinity/ic-js/blob/main/packages/sns/README.md#factory-snswrapper) which knows all SNSess and can therefore serve as a nice way to access differentSNSeses' data.