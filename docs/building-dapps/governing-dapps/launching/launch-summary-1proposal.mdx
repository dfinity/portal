...
  Therefore defining these parameters usually requires a lot of preparation and community engagement already (see [here](../tokenomics/sns-checklist.mdx) for
  more information).
...
Some frontends, for example the NNS frontend dapp, do not show neurons of SNSess that are not fully launched, and thus neurons controlled by NNS frontend dapp principals will only be visible after a successful launch.
...
As a first step of the automatically triggered SNS creation, [SNS-W](../overview.mdx) creates SNS canisters on the SNS subnet.
...
Once the SNS canisters are deployed, [SNS-W](../overview.mdx) sets the SNS root as the sole controller of the dapp canister(s).
