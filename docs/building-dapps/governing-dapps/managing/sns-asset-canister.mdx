...
:::info
To deploy to the mainnet, you will need a wallet that contains cycles. For more information on cycles wallets, please see [here](/docs/current/rosetta-defi/cycles/cycles-wallet).
:::
...
The [`ExecuteGenericNervousSystemFunction`](/docs/current/building-dapps/governing-dapps/managing/making-proposals#executegenericnervoussystemfunction) SNS proposal is used to execute the newly registered proposal. To submit an `ExecuteGenericNervousSystemFunction` SNS Proposal with the output from `dfx deploy FRONTEND_CANISTER_NAME --network ic --by-proposal`, see the update steps below.
...
Before submitting an SNS proposal to update an asset canister, ensure that the asset canister has been upgraded (by proposal) to use the Wasm bundled with [dfx 0.14.1+](https://github.com/dfinity/sdk/blob/release-0.14.1/src/distributed/assetstorage.wasm.gz).
