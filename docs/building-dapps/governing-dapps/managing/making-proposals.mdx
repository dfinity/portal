...
Any eligible neuron can submit a proposal. Therefore, the command to [submit a proposal](/docs/current/building-dapps/developer-tools/quill/quill-sns-make-proposal)
...
:::warning
Because this proposal requires passing wasm, which is unwieldy to copy/paste into the command line as binary, it is recommended that developers use the specially-made [`make-upgrade-canister-proposal`](/docs/current/building-dapps/developer-tools/quill/quill-sns-make-proposal) command in `quill sns`.
