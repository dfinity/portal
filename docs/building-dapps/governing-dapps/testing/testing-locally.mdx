...
:::info
`sns-testing` repo might not work for any dapp. It is intended to showcase one example dapp, and so one can tweak it for their own dapp.
:::
...
- A canister that can be deployed to a local replica via `dfx deploy`.
