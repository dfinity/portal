...
Once a developer has tested the process of an SNS, it is highly recommended they do [an **"SNS testflight" on mainnet**](./testing-on-mainnet.mdx). An SNS test flight is when a developer deploys their dapp (to mainnet) and hands control of it to a mock SNS (on mainnet).
