...
SNSess are provided as a system functionality by the Internet Computer Protocol in that the code for the SNS canisters is maintained by the Internet Computer Protocol. ([Learn about alternative ways to use the SNS code or how to create a DAO.](#dao-alternatives)) More concretely, this means that the NNS community approved the original SNS canisters' code and continuously approves new improved SNS versions.
...
SNSess are protocol-provided DAOs on the Internet Computer. There are, of course, also other alternatives to getting a DAO.
...
They can then choose to follow the same SNS versions that the protocol-provided SNSess follow, or they can choose to deviate from this path. In this option, the DAO community has to be more active in implementing, testing, and approving code versions. In exchange, they have more flexibility.

One advantage of the protocol-provided SNSess over this option is that the verification of the code is easier, as allSNSeses on the SNS subnet run the same canister versions that are verified by the NNS community.
...
- [quill](/docs/current/building-dapps/developer-tools/quill/quill-parent).
