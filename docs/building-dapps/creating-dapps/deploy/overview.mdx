...
<MarkdownChipRow labels={["Beginner", "Launching dapp"]} />

After you have [created](/docs/current/building-dapps/creating-dapps/create-and-install) a <GlossaryTooltip>canister</GlossaryTooltip>, you can deploy it to:
...
- A custom [local or playground network](/docs/current/building-dapps/creating-dapps/deploy/custom-networks).
...
However, tools such as [ICP Ninja](https://icp.ninja) (a web IDE) and the [playground environment](/docs/current/building-dapps/developer-tools/ide/icp-ninja) (available from the CLI) can be used as testnet-like deployment options. They do not require cycles to deploy and operate a canister, but they do have limitations, and canisters will be automatically removed after 20 minutes.
...
- [Custom playgrounds](/docs/current/building-dapps/creating-dapps/deploy/custom-networks): Developers can deploy their own custom instance of the playground on the mainnet, allowing for full customization of the playground's parameters.

- [Custom `dfx` networks](/docs/current/building-dapps/creating-dapps/deploy/custom-networks): The `dfx` named network feature can be used to create custom networks that can be used for testing segmented from other projects deployed locally.
...
<input type="checkbox"/> <a href="/docs/current/building-dapps/creating-dapps/create-and-install">Write a smart contract.</a>
<div>
</div>
<input type="checkbox"/> <a href="/docs/current/building-dapps/creating-dapps/create-and-install">Create your canister.</a> Optional: <code>dfx deploy</code> will execute this step if not already complete.
<div>
</div>
<input type="checkbox"/> <a href="/docs/current/building-dapps/creating-dapps/create-and-install">Compile your canister code into Wasm.</a> Optional: <code>dfx deploy</code> will execute this step if not already complete.
<div>
</div>
<input type="checkbox"/> <a href="/docs/current/building-dapps/creating-dapps/create-and-install">Install the Wasm module into your canister.</a> Optional: <code>dfx deploy</code> will execute this step if not already complete.
</TabItem>
</Tabs>
...
Deploying canisters is done using the `dfx deploy` command.

:::danger
`dfx deploy` does several processes in the background, including:

- [Creating each canister](/docs/current/building-dapps/creating-dapps/create-and-install) listed in the `dfx.json` file.

- [Compiling the code](/docs/current/building-dapps/creating-dapps/create-and-install) for each canister into a Wasm module.

- [Installing the Wasm module](/docs/current/building-dapps/creating-dapps/create-and-install) into each canister.

- Deploying the canisters to the deployment endpoint of choice (local, playground, or mainnet).
:::
...
If you are sharing a link to a backend canister, the [Candid UI](/docs/current/building-dapps/calling-dapps/candid/candid-concepts) will be displayed.
