...
All projects created with [`dfx new`](building-dapps/developer-tools/dfx/dfx-new) have the option to include a template frontend canister containing code using one of the following frameworks: SvelteKit, React, Vue, or Vanilla JS. These templates are available in `dfx` versions 0.17.0 and newer.
...
[Learn more about the HTTP certification library](/docs/current/building-dapps/dapp-frontends/http-gateways-certification/serving-static-assets-over-http).
