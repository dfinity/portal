...
- Backend canisters that communicate with an external frontend application that isn't hosted on ICP. To facilitate this communication, [the ICP JavaScript agent](/docs/current/building-dapps/developer-tools/off-chain/agents/javascript-agent) can be used. [Learn more about the JS request API](/docs/current/building-dapps/developer-tools/off-chain/agents/javascript-agent).
