...
A simple example of this workflow is a basic 'Who am I?' application that uses Internet Identity for authentication. View the [integrating with Internet Identity](/docs/current/building-dapps/using-network-features/authentication/integrate-internet-identity) documentation for more information.

## Authentication methods

Authentication methods available on ICP include:

- [Internet Identity](/docs/current/building-dapps/using-network-features/authentication/overview): ICP's native identity service that uses a cryptographic public/private key pair for authentication.
