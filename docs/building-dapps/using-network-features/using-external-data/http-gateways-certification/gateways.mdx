...
DFINITY exclusively controls the HTTP gateways that serve canisters on `ic0.app` and `icp0.io`. Developers can then also configure their own [custom domains](/docs/current/building-dapps/dapp-frontends/custom-domains/using-custom-domains) to point at the DFINITY-controlled HTTP gateways.
...
Additional details can be found in the [HTTP gateway specification](/docs/current/references/http-gateway-protocol-spec).
