...
It is recommended to deploy canisters locally or to the temporary [playground](/docs/current/building-dapps/launching-a-dapp/deploy/overview#deploying-to-playground) network to test canister functionality and make adjustments before considering the canister production-ready and deploying it to the mainnet. Deploying canisters locally and to the playground is free and does not cost cycles. Once a canister is deployed to the mainnet, it will consistently consume cycles and will need to be topped up periodically.
...
Now that the canister has been tested, you can deploy it to the mainnet. **Deploying to the mainnet costs [cycles](/docs/current/building-dapps/getting-started/tokens-and-cycles).**
...
After a canister is deployed to the mainnet, it will constantly be consuming resources and cycles. To view information about how many resources and cycles are currently being used, along with other information, view the canister's settings:
...
- **Wasm memory limit:** The maximum size for a canister's Wasm memory. [Learn more about resource limits](/docs/current/building-dapps/launching-a-dapp/resource-limits).
...
[Learn more about canister settings](/docs/current/building-dapps/managing-dapps/settings)
...
[Canister logs](/docs/current/building-dapps/managing-dapps/logs) can be used to gain insight into a canister's behavior and assist with debugging situations such as when a canister traps.
...
To view help information for `dfx`, you can use the `dfx help` command or view the [`dfx` reference documentation](/docs/current/building-dapps/developer-tools/dfx/).