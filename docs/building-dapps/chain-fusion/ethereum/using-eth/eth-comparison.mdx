...
For a feature comparison between the two protocols, refer to the [quick comparison between ICP and Ethereum](/docs/current/developer-education/developer-concepts/network-overview#quick-comparison-with-ethereum).
...
| Node           | Replica  |       |
| Account        | Principal |       |
| DAO            | SNS       | An SNS refers to a DAO using ICP's [unique framework](/docs/current/building-dapps/governing-dapps/) However, ICP DAOs can launch without using this framework.|
...
Both Ethereum and ICP allow developers to deploy programs that run onchain. These programs are referred to as smart contracts on Ethereum and canister smart contracts, or simply just [canisters](/docs/current/developer-education/developer-concepts/what-are-canisters), on ICP.
...
**ICP**: Canisters can be written in any language that compiles into [Wasm](https://webassembly.org/) or has an interpreter that can compile to Wasm, including [Rust](/docs/current/building-dapps/developer-tools/cdks/rust/), [Typescript](https://demergent-labs.github.io/azle/), [Motoko](/docs/current/motoko/main/getting-started/motoko-introduction), and [Python](https://demergent-labs.github.io/kybra/).
...
**ICP**: Developers use `dfx`, which is a similar development environment. [`dfx`](/docs/current/building-dapps/getting-started/install#installing-dfx-via-dfxvm) is a command-line tool that interacts with the IC SDK and allows developers to compile and deploy their canisters.
...
**ICP**: Currently, testing is done in the local environment. A [playground environment](/docs/current/building-dapps/getting-started/deploy-and-manage/) on the mainnet is available for canisters, but deployment time is limited to 20 minutes. However, the ability to upgrade and delete canisters on the mainnet allows developers greater flexibility to iterate and refine their smart contracts than on Ethereum.


#### Paying for gas

**Ethereum**: Users pay for gas fees directly using the crypto wallet they connect to the dapp.
**ICP**: Using the [reverse gas model](/docs/current/developer-education/developer-concepts/gas-cost#the-reverse-gas-model), the dapp developers, not the users, pay for the gas fees. The reverse gas model aims to provide a smoother user experience by eliminating upfront fees.
