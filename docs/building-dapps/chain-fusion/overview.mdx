...
:::info
Threshold signing is also referred to as [chain-key cryptography](/docs/current/developer-education/glossary#c).
:::
...
ICP’s decentralization is rooted in the replicated execution and state of canisters which depends on the honesty of a supermajority of subnet [nodes](/docs/current/developer-education/developer-concepts/network-overview#nodes). These nodes are sourced from various identifiable and DAO-approved providers that operate in globally distributed data centers.
...
- **[Threshold signatures](/docs/current/references/t-sigs-how-it-works)**: An ICP service implementing the threshold ECDSA and threshold Schnorr protocols.
