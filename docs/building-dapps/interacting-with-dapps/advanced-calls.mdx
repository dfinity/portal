...
[Composite queries](/docs/current/building-dapps/interacting-with-dapps/query-calls) are query calls that can call other queries on the same <GlossaryTooltip>subnet</GlossaryTooltip>. They can only be invoked by end users and not by other canisters.
