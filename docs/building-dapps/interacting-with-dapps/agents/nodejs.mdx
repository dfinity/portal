...
You can configure an authentication identity for Node.js through the [ICP JavaScript agent](https://www.npmjs.com/package/@dfinity/agent) using the ECDSA or Ed25519 private keys. [Learn more in the ICP JavaScript agent docs](https://internetcomputer.org/docs/current/building-dapps/interacting-with-dapps/agents/javascript-agent).
