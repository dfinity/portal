...
For example, an idle canister that has compute allocation of 50% for one hour will be charged `base_fee * 50 * 3600`, where `base_fee` is the fee for [one percent per second](/docs/current/developer-education/developer-concepts/gas-cost).
