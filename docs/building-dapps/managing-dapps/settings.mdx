...
1. Command line: [`dfx canister status <canister-name>`](/docs/current/building-dapps/developer-tools/dfx/dfx-canister#dfx-canister-status). Note that this command returns the canister's settings, along with other canister status information.
...
Learn more about [canister control](/docs/current/building-dapps/managing-dapps/control).
...
For example, an idle canister that has a compute allocation of 50% for one hour will be charged `base_fee * 50 * 3600`, where `base_fee` is the fee for [one percent per second](/docs/current/developer-education/developer-concepts/gas-cost).
...
[Learn more about payment for memory](/docs/current/developer-education/developer-concepts/gas-cost).
...
[Learn more about resource reservation](/docs/current/developer-education/developer-concepts/gas-cost).
...
- [Invalid settings](/docs/current/developer-education/message-execution/execution-errors#invalid-settings).