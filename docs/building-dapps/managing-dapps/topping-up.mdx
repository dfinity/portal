...
- Using [`dfx cycles top-up`](/docs/current/building-dapps/developer-tools/dfx/dfx-cycles#dfx-cycles-top-up).

- Through the [NNS frontend dapp](https://nns.ic0.app).

- Through a [third-party service](/docs/current/building-dapps/managing-dapps/topping-up) (e.g., https://cycleops.dev).
...
Canister cycles balances are available through the [`dfx canister status`](/docs/current/building-dapps/developer-tools/dfx/dfx-canister#dfx-canister-status) command. They are not exposed publicly by default; you can only see them if you are the controller of a canister.
...
If you have ICP associated with your [developer identity](/docs/current/building-dapps/getting-started/identities) you can use [`dfx ledger top-up`](/docs/current/building-dapps/developer-tools/dfx/dfx-ledger) to convert some ICP into cycles, then top up a canister with those cycles:

### Option 2: If you have cycles

If you already have cycles, you can top up a canister with the command [`dfx cycles top-up`](/docs/current/building-dapps/developer-tools/dfx/dfx-cycles#dfx-cycles-top-up):
...
Cycles wallets are canisters like any other. If you have a cycles wallet you control via `dfx`, there is another option for sending cycles from your cycles wallet to a canister of your choice: [`dfx wallet send [OPTIONS] <DESTINATION> <AMOUNT>`](/docs/current/building-dapps/developer-tools/dfx/dfx-wallet#dfx-wallet-send).
...
[Learn more about freezing thresholds](/docs/current/building-dapps/managing-dapps/settings#freezing-threshold).
...
Once a <GlossaryTooltip>canister</GlossaryTooltip> is deployed to ICP, the compute and storage it utilizes must be pre-paid in cycles. The process of burning ICP to obtain cycles and transferring them to a canister is referred to as "topping up" a canister.
