...
- Accessing [heap data](/docs/current/building-dapps/managing-dapps/storage) that is out of bounds.
...
Special care should be taken when writing a [`pre_upgrade`](/docs/current/references/ic-interface-spec#system-api-upgrades) hook for your canister, which will run during an upgrade against the current canister's Wasm module. If the `pre_upgrade` hook traps, then the canister upgrade will fail. This is extremely important, as it means your canister can be permanently unrecoverable.
