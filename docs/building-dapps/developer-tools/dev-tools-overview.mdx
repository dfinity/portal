...
[`dfx`](/docs/current/building-dapps/developer-tools/dfx/) is the primary tool used by developers to create, build, deploy, manage, and call canisters. `dfx` is also referred to as the IC SDK, or the Internet Computer Software Development Kit.

`dfx` supports [local development](/docs/current/building-dapps/getting-started/deploy-and-manage) and deployment through the [`dfx start`](/docs/current/building-dapps/developer-tools/dfx/) command, which runs a local instance of the ICP replica that can be used to deploy and test canisters on your local machine.
...
- [Install `dfx`](/docs/current/building-dapps/getting-started/install).

- [`dfx` documentation](/docs/current/building-dapps/developer-tools/dfx/).
...
[`dfxvm`](/docs/current/building-dapps/developer-tools/dfxvm/) is a CLI tool used for installing and switching between different versions of `dfx`. It enables developers to pivot between different projects that are built with different `dfx` versions.

- [Install `dfxvm`](/docs/current/building-dapps/getting-started/install#installing-dfx).

- [`dfxvm` documentation](/docs/current/building-dapps/developer-tools/dfxvm/dfx/).

- [`dfxvm` GitHub repo](https://github.com/dfinity/dfxvm).

### `quill`

[`quill`](/docs/current/building-dapps/developer-tools/quill/ command-line ledger and governance toolkit designed for interacting with the [Network Nervous System (NNS)](/docs/current/move-to-learnhub/nns/overview) canisters through the use of self-custody keys. It supports using keys in an air-gapped computer, which is a computer that has never been connected to the internet. To support this workflow, `quill` first generates and signs messages based on the user input, then the signed messages are transported to a computer with an internet connection (via a USB flash drive or QR code) where they are submitted to ICP to be executed.

- [Install `quill`](https://github.com/dfinity/quill?tab=readme-ov-file#download--install).

- [`quill` documentation](/docs/current/building-dapps/developer-tools/quill/).
...
- [JavaScript/TypeScript agent (@dfinity/agent)](/docs/current/building-dapps/interacting-with-dapps/agents/javascript-agent)

- [Node.js, supported through the JavaScript agent](/docs/current/building-dapps/interacting-with-dapps/agents/nodejs)

- [Rust agent (ic-agent)](/docs/current/building-dapps/interacting-with-dapps/agents/rust-agent)
...
- [Canpack](/docs/current/building-dapps/developer-tools/canpack): A tool used to facilitate communication between canisters written in different languages; currently supports calling Rust crates from Motoko canisters.

### Canbench

- [Canbench](/docs/current/building-dapps/developer-tools/canbench): A Rust crate that provides canister benchmarking tooling so developers can gain an insight into how their canister uses memory, instructions, and other resources.
...
The [Rust CDK](/docs/current/building-dapps/developer-tools/cdks/rust/) is developed and maintained by DFINITY for Rust development.
...
- [Playground documentation](/docs/current/building-dapps/developer-tools/ide/playground).
...
- [VS Code documentation](/docs/current/building-dapps/developer-tools/ide/vs-code).
...
- [canister-tools](https://github.com/levifeldman/canister-tools): A series of tools for Rust canisters, such as snapshots and simple upgrades.
