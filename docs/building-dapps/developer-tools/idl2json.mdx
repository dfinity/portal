...
<input type="checkbox"/> <a href="https://doc.rust-lang.org/cargo/getting-started/installation.html">Install cargo.</a>
...
In `dfx` versions `0.19.0` and newer, the flag `--output json` can be used with a `dfx canister call` command. This removes the need to pipe the output to `idl2json`. [Learn more in the `dfx` documentation](/docs/current/building-dapps/developer-tools/dfx/er#options).

## Resources

- [`idl2json`](https://github.com/dfinity/idl2json/tree/main)

- [`dfx` documentation](/docs/current/building-dapps/developer-tools/dfx/er#options)