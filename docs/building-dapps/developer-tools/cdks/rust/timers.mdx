...
:::info
Note: As described in the [periodic tasks and timers](/docs/current/building-dapps/using-network-features/periodic-tasks-timers) page, the timers library does not handle canister upgrades. It is up to the canister developer to serialize the timers in the `canister_pre_upgrade` and reactivate the timers in the `canister_post_upgrade` method if needed.
