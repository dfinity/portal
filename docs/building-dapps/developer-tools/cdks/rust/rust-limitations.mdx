...
- You cannot use crates that make or serve HTTP requests. Instead, use [HTTP outcalls](/docs/current/building-dapps/using-network-features/using-external-data/http-outcalls/overview) or the [gateway API](/docs/current/references/http-gateway-protocol-spec/).
