...
Candid is an interface description language (IDL) for interacting with canisters running on ICP. To see details about the Candid interface description language syntax, see the [Candid guide](/docs/current/building-dapps/calling-dapps/candid/candid-concepts) or the [Candid crate documentation](https://docs.rs/candid/).
