...
After you have [created](/docs/current/building-dapps/launching-a-dapp/create-and-install) a <GlossaryTooltip>canister</GlossaryTooltip>, you can deploy it to:
...
- A custom [local or playground network](/docs/current/building-dapps/launching-a-dapp/deploy/custom-networks).
...
However, tools such as [ICP Ninja](https://icp.ninja) (a web IDE) and the [playground environment](/docs/current/building-dapps/developer-tools/ide/playground) (available from the CLI) can be used as testnet-like deployment options. They do not require cycles to deploy and operate a canister, but they do have limitations, and canisters will be automatically removed after 20 minutes.
...
- [Custom playgrounds](/docs/current/building-dapps/launching-a-dapp/deploy/custom-networks): Developers can deploy their own custom instance of the playground on the mainnet, allowing for full customization of the playground's parameters.

- [Custom `dfx` networks](/docs/current/building-dapps/launching-a-dapp/deploy/custom-networks): The `dfx` named network feature can be used to create custom networks that can be used for testing segmented from other projects deployed locally.
...
If you are sharing a link to a backend canister, the [Candid UI](/docs/current/building-dapps/interacting-with-dapps/candid/candid-concepts) will be displayed.
:::
