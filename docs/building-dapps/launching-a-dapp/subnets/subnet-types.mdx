...
The subnet [`uzr34`](https://dashboard.internetcomputer.org/subnet/uzr34-akd3s-xrdag-3ql62-ocgoh-ld2ao-tamcv-54e7j-krwgb-2gm4z-oqe) is a system subnet that contains 28 nodes. It hosts the canisters for [Internet Identity](/docs/current/building-dapps/using-network-features/authentication/overview), the [cycles ledger](/docs/current/rosetta-defi/token-ledgers/cycles-ledger), the [exchange rate canister](/docs/current/rosetta-defi/exchange-rate-canister/) and dashboard, and stores the backup threshold signature keys.
...
Adjust the `--amount 0.5` to the number of ICP tokens that you want to send the new canister. More information can be found in the [documentation](/docs/current/building-dapps/developer-tools/dfx/dfx-ledger/#options).
