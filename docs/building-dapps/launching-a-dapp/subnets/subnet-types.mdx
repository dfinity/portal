...
You can learn more about subnets and how they work [here](/docs/current/building-dapps/launching-dapps/subnets/overview).
...
- [`uzr34`](https://dashboard.internetcomputer.org/subnet/uzr34-akd3s-xrdag-3ql62-ocgoh-ld2ao-tamcv-54e7j-krwgb-2gm4z-oqe) hosts the canisters for [Internet Identity](/docs/current/building-dapps/using-network-features/authentication/overview), the [cycles ledger](/docs/current/rosetta-defi/token-ledgers/cycles-ledger), the [exchange rate canister](/docs/current/rosetta-defi/exchange-rate-canister/) and the dashboard, and stores the backup threshold signature keys.
...
There is currently one fiduciary subnet, [`pzp6e`](https://dashboard.internetcomputer.org/subnet/pzp6e-ekpqk-3c5x7-2h6so-njoeq-mt45d-h3h6c-q3mxf-vpeq5-fk5o7-yae), which contains 34 nodes, hosts the EVM RPC canister, and stores the threshold signature (tECDSA and tSchnorr) signing keys.
