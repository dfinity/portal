...
| [Canister-controlled wallet](/docs/current/concepts/what-are-canisters) | Using [Chain Fusion](/docs/current/building-apps/chain-fusion/overview), canisters on ICP can hold, sign, and transact with assets on various chains including ICP, Bitcoin, Ethereum, EVMs, and more. | [Cycles wallet](/docs/current/defi/cycles/cycles-wallet) (will be depreciated from dfx). Holds cycles needed to deploy, upgrade, and perform other operations on canisters. The cycles wallet is still supported, but has been replaced by the [cycles ledger](/docs/current/defi/token-ledgers/cycles-ledger). It is recommended to use the cycles ledger instead. |
...
The [NNS dapp](https://nns.ic0.app/) provides users the same functionality in addition to generating neurons to stake ICP and participate in governance. The Internet Identity integration is often referred to as the [NNS wallet](/docs/current/defi/overview#nns-dapp).
...
- [Quill](/docs/current/building-apps/developer-tools/quill/).
