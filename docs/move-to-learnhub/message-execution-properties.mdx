...
ICP relies on an asynchronous messaging model. Compared to synchronous messaging like on Ethereum, this provides performance advantages because individual messages can be executed in parallel. However, asynchrounous message execution can also lead to sometimes unexpected or unintuitive behavior. Therefore, it is important to understand the properties of message execution. Potential security issues that arise in this model, such as reentrancy bugs, are discussed in the [security best practices on inter-canister calls](/docs/building-apps/best-practices/security/inter-canister-calls).
...
To interact with a canister's methods, there are two primary types of **calls** that can be used: [update](/docs/references/ic-interface-spec#http-call) calls and [query](/docs/references/ic-interface-spec#http-query) calls.

If the Rust CDK is used, these are usually annotated with `#[update]` or `#[query]`, respectively. In Motoko, updates are declared as `public func`, and queries us the dedicated keyword: `public query func`. ICP also supports additional [entry points](/docs/references/ic-interface-spec#entry-points) such as heartbeats, timers, initialization or upgrade hooks.
