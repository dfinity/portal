...
All proposals to manage [protocol canisters](/docs/current/concepts/system-canisters//), which are considered part of the Internet Computer protocol and are essential for its proper functioning.
...
* `AddSnsWasm`: Add a new SNS canister Wasm to SNS-W. All SNS DAOs can then upgrade to new versions along the upgrade path defined in [SNS-W](/docs/current/building-apps/governing-dapps/overview#SNS-W).
...
This topic includes proposals that concern [SNS DAO launches](/docs/current/building-apps/governing-dapps/launching/) and the [Neurons' Fund](/docs/current/move-to-learnhub/nns/concepts/neurons-fund) (formerly called Community Fund).
