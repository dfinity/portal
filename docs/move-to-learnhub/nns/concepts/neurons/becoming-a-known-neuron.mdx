...
within the NNS. This information can be used by other neurons to find known neurons and [follow](/docs/current/move-to-learnhub/nns/concepts/neurons/neuron-following) them.
...
* [Create the neuron on the NNS dapp](/docs/current/move-to-learnhub/nns/overview#stake-icp-utility-tokens-in-a-neuron).
...
To install `dfx`, follow the [installation instructions](/docs/current/building-dapps/getting-started/install#installing-dfx).
...
[**hot key**](/docs/current/move-to-learnhub/nns/concepts/neurons/neuron-management#neuron-attributes)
of the neuron. Hot keys have the permission to vote and submit proposals for the neuron.

Run `dfx identity get-principal` to get the principal ID that corresponds to your current [developer identity](/docs/current/building-dapps/getting-started/identities). The following steps will refer to this as your "dfx principal".
