...
ICP also supports dynamic NFTs that can be curated to evolve based on both data stored onchain or obtained through external APIs. NFT assets can be modified in real time by making external API requests using [HTTPS outcalls](/docs/current/building-dapps/using-network-features/using-external-data/https-outcalls/overview).
