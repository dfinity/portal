...
* The trust model of ICP assumes that at least 2/3 of the replicas are honest. This trust in the protocol extends to the IC-managed infrastructure that proxies certain requests (e.g., to IPv4 addresses). This ensures that honest replicas can reach a consensus on the response to be returned to the canister.
