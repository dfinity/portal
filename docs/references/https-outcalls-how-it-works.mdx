...
* It is assumed that the HTTP service is honest; otherwise, it can provide any responses to any of the calling replicas of the subnet. This is also the case for calls to the service from a Web 2.0 service or an oracle; thus, the situation is the same as in our scenario of a blockchain making the call to the service.
...
* **IPv4 support:** IPv4 support would help canisters reach servers that are not available on IPv6 but only on IPv4.
