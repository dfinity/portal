...
### General availability of Ed25519

A single threshold Ed25519 production key according to curve `ed25519` has been deployed ahead of the Helium milestone release.
...
An example of the Ed25519 API can be found below:

```
  ed25519_public_key : (record {
    canister_id : opt canister_id;
    derivation_path : vec blob;
    key_id : record { algorithm: schnorr_algorithm; name: text };
  }) -> (record { public_key : blob; chain_code : blob; });
  sign_with_ed25519 : (record {
    message : blob;
    derivation_path : vec blob;
    key_id : record { algorithm: schnorr_algorithm; name: text };
  }) -> (record { signature : blob });
...
### Fees for the t-Ed25519 test key

| Transaction                          | Description                                                                                                    | Cycles (test key)                     | USD                         |
|--------------------------------------|----------------------------------------------------------------------------------------------------------------|-----------------------------|-----------------------------|
| Threshold Ed25519 signing              | For computing one threshold Ed25519 signature (`sign_with_ed25519`)                                                | 10_000_000_000              | $0.0130886                  |

### Fees for the t-Ed25519 production key

| Transaction                          | Description                                                                                                    | Cycles (production key)                     | USD                         |
|--------------------------------------|----------------------------------------------------------------------------------------------------------------|-----------------------------|-----------------------------|
| Threshold Ed25519 signing              | For computing one threshold Ed25519 signature (`sign_with_ed25519`)                                                | 26_153_846_153              | $0.0346672                  |
