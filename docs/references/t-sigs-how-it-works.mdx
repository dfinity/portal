...
The Schnorr API comprises two methods: `schnorr_public_key` for retrieving threshold Schnorr public keys and `sign_with_schnorr` for requesting threshold Schnorr signatures to be computed from the subnet holding the secret-shared private threshold Schnorr key. Note that, in some cases, a signature may be computed when requested with a `sign_with_*` method, but not returned to the requesting canister. Thus, canisters should not rely on the signature not existing even if the signing request fails.
