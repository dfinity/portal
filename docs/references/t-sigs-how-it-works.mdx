...
A single threshold Schnorr production key according to the `bip340` specification for curve `secp256k1` has been deployed with the Deuterium Schnorr release.
The key with id `(bip340secp256k1, key_1)` will be maintained in same manner that the [threshold ECDSA](#general-availability-release-of-ECDSA) key is mainted.
