...
This method returns a vetKD key (aka vetKey) encrypted under `encryption_public_key` derived from the master key with ID `key_id` for the given derivation information consisting of `derivation_id` and `public_key_derivation_path`.

The `derivation_id` is used to derive different encrypted keys. The `public_key_derivation_path` is a vector of variable length byte strings, containing at most 255 byte strings, and acts (together with the calling canister's ID) as domain separator to derive different public (verification) keys with the IC method `vetkd_public_key`. The `key_id` is a struct specifying both a curve and a name. The availability of a particular `key_id` depends on the implementation.
