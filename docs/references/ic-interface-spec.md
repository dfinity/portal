...
-   In status `stopped`, calls to the canister are rejected by the IC with reject code `CANISTER_ERROR` (5), and there are no outstanding responses. This status contains an additional boolean, `ready_for_migration` which indicates whether the canister has empty queues and flushed streams and thus if it is ready to be migrated. 
