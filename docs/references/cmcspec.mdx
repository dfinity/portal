...
-   The **destination account** of the ICP transfer must be the CMC's account with a subaccount derived from the target canister’s principal (see the section on [Encoding a principal into a subaccount](#encoding-a-principal-into-a-subaccount)).
...
-   Other fields: Use system-wide default configuration values set by the ICP replica.
