...
For further information on calling canisters, see the [smart contract docs](../building-dapps/calling-dapps/advanced-calls) and the [IC spec](./ic-interface-spec#system-api-requests).
...
[Installing](/docs/current/building-dapps/creating-dapps/create-and-install) a Wasm module on a canister is meant to be performed only when the canister has no existing state when it has first been created and therefore fails if the canister already has an installed Wasm module.
...
[limits]:/docs/current/building-dapps/creating-dapps/resource-limits#resource-constraints-and-limits
