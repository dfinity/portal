...
When encountering an error, canisters may choose to fail with an error message by calling the [`ic0.trap` API](/docs/current/references/ic-interface-spec#debugging-aids). The Rust and Motoko CDKs will insert calls to this API when panicking.
...
Certain system APIs are only available in certain types of messages. For an overview of when each system API can be used, see [the ICP spec](/docs/current/references/ic-interface-spec#system-api-imports).
...
Each canister has a number of [controllers](/docs/current/building-dapps/managing-dapps/control) and certain actions (e.g., updating the canister's Wasm code) can only be performed by controllers. To fix this error, consider performing the action from a [principal](/docs/current/developer-education/glossary/index#principal) that is already a controller, or have an existing controller add additional principals to the canister's list of controllers.
