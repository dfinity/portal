...
To fix this error, consider using tools such as the [performance counter API](/references/samples/rust/performance_counters/) or [canbench][canbench] to determine which sections of code are using significant instructions and optimize them.
...
When encountering an error, canisters may choose to fail with an error message by calling the [`ic0.trap` API](/references/ic-interface-spec#debugging-aids). The Rust and Motoko CDKs will insert calls to this API when panicking.
...
Canisters can [exist without having Wasm code installed](/references/ic-interface-spec#canister-lifecycle). A canister will have no Wasm code if it has never been installed or if it has been [uninstalled](/references/ic-interface-spec#ic-uninstall_code).
...
To fix this error, consider installing code using [`dfx deploy`](/building-apps/developer-tools/dfx/dfx-deploy), or the APIs for [`install_code`](/references/ic-interface-spec#ic-install_code) or [`install_chunked_code`](/references/ic-interface-spec#ic-install_chunked_code).
...
Newer versions of Motoko don't require these reserved pages, so upgrading the version of Motoko used in the canister will fix the issue. `dfx` stores the Motoko compiler (`moc`) in the directory returned by [`dfx cache show`](/building-apps/developer-tools/dfx/dfx-cache#dfx-cache-show). So the version of Motoko can be shown by running `$(dfx cache show)/moc --version`. This issue only occurs for versions `0.6.20` and older.
...
Certain system APIs are only available in certain types of messages. For an overview of when each system API can be used, see [the ICP spec](/references/ic-interface-spec#system-api-imports).
...
ICP [limits][limits] the length of each function name in a Wasm module to 1 MiB. This error indicates that a function name exceeds this limit.

To fix this error, try using shorter function names.
...
Each canister has a number of [controllers](/building-apps/canister-management/control) and certain actions (e.g., updating the canister's Wasm code) can only be performed by controllers. To fix this error, consider performing the action from a [principal](/references/glossary#p) that is already a controller, or have an existing controller add additional principals to the canister's list of controllers.
...
[Installing](/building-apps/essentials/canisters) a Wasm module on a canister is meant to be performed only when the canister has no existing state when it has first been created and therefore fails if the canister already has an installed Wasm module.

If the intention is to update the Wasm module without wiping the state, then an [upgrade](/building-apps/canister-management/upgrade) should be used instead. If the intention is actually to overwrite the canister state, then the [reinstall](/building-apps/canister-management/settings#reinstall-a-canister) mode can be used.
...
There was an error while executing a method to manipulate the canister's [Wasm chunk store](/references/ic-interface-spec#ic-upload_chunk) (e.g., `upload_chunk`, `clear_chunk_store`, `install_chunked_code`, etc.).
...
To fix this error, consider [topping up][topping-up] the canister if it doesn't have enough cycles or using the [`stored_chunks` API](/references/ic-interface-spec#ic-stored_chunks) to see which chunks have been uploaded if you get errors about mismatching or missing hashes.
...
Snapshot is not loadable on the target canister in case the canister you're trying to load the snapshot onto is different than the one that was used to create the snapshot.
...
To fix this error, try resending the message after waiting for at least 10 minutes.


### Canister snapshot not controller
This indicates that you are not a controller of the canister that a snapshot belongs to and so you cannot load it (on this or another canister).
...
[canbench]: /building-apps/advanced/benchmarking

[canister-status-api]: /references/ic-interface-spec#ic-canister_status

[compute-allocation]:/building-apps/developer-tools/dfx/dfx-canister/#allocating-message-processing

[dashboard-subnets]: https://dashboard.internetcomputer.org/subnets

[dfx-canister-status]:/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-status

[dfx-canister-stop]:/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-stop

[ic-wasm]: https://github.com/dfinity/ic-wasm

[limits]:/building-apps/canister-management/resource-limits#resource-constraints-and-limits

[memory-allocation-setting]:/building-apps/developer-tools/dfx/dfx-canister/#options-8

[setting-freezing-threshold]:/tutorials/hackathon-prep-course/managing-canisters/#setting-the-canisters-freezing-threshold

[topping-up]:/building-apps/canister-management/topping-up/
