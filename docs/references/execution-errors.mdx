...
[Installing](/docs/building-apps/developing-canisters/what-are-canisters) a Wasm module on a canister is meant to be performed only when the canister has no existing state when it has first been created and therefore fails if the canister already has an installed Wasm module.
