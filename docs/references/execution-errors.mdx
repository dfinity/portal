...
Each canister has a number of [controllers](/docs/current/developer-docs/smart-contracts/maintain/control) and certain actions (e.g. updating the canister's Wasm code) can only be performed by controllers. To fix this error, consider performing the action from a [principal](/docs/current/concepts/glossary/#principal) which is already a controller, or have an existing controller add additional principals to the canister's list of controllers.
...
There was an attempt to update the canister's settings and change the reserved cycles limit field to a value that is
lower than the already reserved cycles balance of the canister.
