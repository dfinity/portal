...
For further information on calling canisters see the [smart contract docs](../developer-education/message-execution/types-of-canister-calls) and the [IC spec](./ic-interface-spec#system-api-requests).
...
To fix this error, consider installing code using [`dfx deploy`](/docs/current/building-dapps/developer-tools/dfx/dfx-deploy), or the APIs for [`install_code`](/docs/current/references/ic-interface-spec#ic-install_code) or [`install_chunked_code`](/docs/current/references/ic-interface-spec#ic-install_chunked_code).
...
Newer versions of Motoko don't require these reserved pages, so upgrading the version of Motoko used in the canister will fix the issue. `dfx` stores the Motoko compiler (`moc`) in the directory returned by [`dfx cache show`](/docs/current/building-dapps/developer-tools/dfx/dfx-cache#dfx-cache-show). So the version of Motoko can be shown by running `$(dfx cache show)/moc --version`. This issue only occurs for versions `0.6.20` and older.
...
Each canister has a number of [controllers](/docs/current/building-dapps/managing-dapps/control) and certain actions (e.g. updating the canister's Wasm code) can only be performed by controllers. To fix this error, consider performing the action from a [principal](/docs/current/developer-education/glossary/index#principal) which is already a controller, or have an existing controller add additional principals to the canister's list of controllers.
...
[Installing](/docs/current/building-dapps/launching-a-dapp/create-and-install) a Wasm module on a canister is meant to be performed only when the canister has no existing state when it has first been created, and therefore fails if the canister already has an installed Wasm module.

If the intention is to update the Wasm module without wiping the state, then an [upgrade](/docs/current/building-dapps/managing-dapps/upgrade) should be used instead. If the intention is actually to overwrite the canister state, then the [reinstall](https://internetcomputer.org/docs/current/building-dapps/managing-dapps/settings#reinstall-a-canister) mode can be used.
...
[canbench]: /docs/current/building-dapps/developer-tools/canbench

[canister-status-api]: /docs/current/references/ic-interface-spec#ic-canister_status

[compute-allocation]:/docs/current/building-dapps/developer-tools/dfx/dfx-canister/#allocating-message-processing

[dashboard-subnets]: https://dashboard.internetcomputer.org/subnets

[dfx-canister-status]:/docs/current/building-dapps/developer-tools/dfx/dfx-canister#dfx-canister-status

[dfx-canister-stop]:/docs/current/building-dapps/developer-tools/dfx/dfx-canister#dfx-canister-stop

[ic-wasm]: https://github.com/dfinity/ic-wasm

[limits]:/docs/current/building-dapps/launching-a-dapp/resource-limits#resource-constraints-and-limits

[memory-allocation-setting]:/docs/current/building-dapps/developer-tools/dfx/dfx-canister/#options-8
...
[topping-up]:/docs/current/building-dapps/managing-dapps/topping-up/
