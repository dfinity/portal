...
Each canister has a number of [controllers](/docs/current/building-dapps/managing-dapps/control) and certain actions (e.g., updating the canister's Wasm code) can only be performed by controllers. To fix this error, consider performing the action from a [principal](/docs/current/developer-education/glossary#p) that is already a controller, or have an existing controller add additional principals to the canister's list of controllers.
