...
[Installing](docs/current/developer-docs/smart-contracts/install) a Wasm module on a canister is meant to be performed only when the canister has no existing state when it has first been created, and therefore fails if the canister already has an installed Wasm module.
...
Canisters have a [compute allocation][compute-allocation] setting which can be used to guarantee the canister will run periodically. This error occurs when the requested allocation cannot be met because other canisters on the subnet have already requested all of the currently available compute resources.
