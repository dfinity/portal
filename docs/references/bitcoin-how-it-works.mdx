...
In the typical canister development workflow, canisters on ICP are compiled and run in the local environment using the [IC SDK](/building-apps/getting-started/install) during their development. Thus, the IC SDK is the first stage, or environment, of the development workflow. The IC SDK has been enabled to support both the Bitcoin integration and threshold ECDSA APIs.
...
- [Local development](/build-on-btc/using-regtest).
