...
In the typical canister development workflow, canisters on ICP are compiled and run in the local environment using the [IC SDK](/docs/current/building-dapps/getting-started/install) during their development. Thus, the IC SDK is the first stage, or environment, of the development workflow. The IC SDK has been enabled to support both the Bitcoin integration and threshold ECDSA management canister APIs.
...
- [Local development](/docs/current/building-dapps/chain-fusion/bitcoin/using-btc/local-development).
