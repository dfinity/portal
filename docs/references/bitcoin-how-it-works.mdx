...
The Bitcoin adapter of the single replica running the local SDK environment connects to the local `bitcoind` node instead of multiple nodes on the Bitcoin testnet or mainnet. Please look at the output of `dfx start --help` to see the relevant flags on dfx.
...
This is the same setting as used in local development.
