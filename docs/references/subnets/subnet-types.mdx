...
You can learn more about subnets and how they work [here](/building-apps/developing-canisters/create).
...
The subnet [`uzr34`](https://dashboard.internetcomputer.org/subnet/uzr34-akd3s-xrdag-3ql62-ocgoh-ld2ao-tamcv-54e7j-krwgb-2gm4z-oqe) is a system subnet that contains 28 nodes. It hosts the canisters for [Internet Identity](/building-apps/authentication/overview), the [cycles ledger](/defi/token-ledgers/cycles-ledger), the [exchange rate canister](/references/system-canisters/) and dashboard, and stores the backup threshold signature keys.
...
To top up your principal's balance, you can send cycles to the principal using the [NNS dapp](https://nns.ic0.app/wallet/), or [learn how to convert ICP tokens into cycles](/tutorials/developer-liftoff/level-1/1.4-using-cycles#converting-icp-tokens-to-cycles).
...
Adjust the `--amount 0.5` to the number of ICP tokens that you want to send the new canister. More information can be found in the [documentation](/building-apps/developer-tools/dfx/dfx-ledger/#options).
