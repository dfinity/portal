...
The Internet Computer integrates the vetKD protocol as a threshold key derivation service operated by vetkD-enabled subnets. Each such subnet maintains a threshold-shared vetKD master key. The protocol involves three main actors:
...
2. **Access Control & Routing:** The canister authenticates the user and enforces any access control policies. If the request is authorized, the canister invokes the [vetKD system API](/docs/building-apps/network-features/vetkeys/api), specifying:  
...
3. **Key Derivation:** All the nodes in the designated vetKD subnet run the threshold key derivation protocol:  
    ...
    * **Share Combination**: Once a quorum (2f + 1 shares in a subnet of 3f + 1 nodes) is available, any node can combine the shares into a single **encrypted derived key**. The final result remains encrypted under the user’s transport key.  
4. **Response Delivery:** The combined encrypted key is returned to the canister, which delivers it to the user.  
5. **Verification and Decryption:** The user verifies that:  
    * The key corresponds to the provided **context** and **input**  
    * It was encrypted with their **transport public key**  

  If verification passes, the user decrypts the result using their private key. The derived key can then be used, for instance, as a seed for generating user keys, session keys, or for application-specific cryptographic operations.
...
Instead, the system derives a single canister-level master key from the subnet’s vetKD master key using an additive key derivation scheme, similar to those used for [threshold ECDSA and Schnorr](/docs/references/t-sigs-how-it-works#key-derivation). Each canister can then derive additional subkeys by specifying a *context*, such as a user identifier or key purpose. This context acts as a domain separator, ensuring that derived subkeys are isolated and application-specific, even though they originate from the same canister master key.

Each canister-level key can then be used in the vetKD protocol to derive user-requested keys. To do so, the canister supplies an input, typically user-generated or application-specific data, that serves as the basis for the derived key. This input does not need to be random or secret. Because the key derivation process is deterministic (given the same canister, context, and input), the input can effectively serve as an identifier for the derived key within the application.
