...
The HTTP Gateway Protocol is an extension of the Internet Computer Protocol that allows conventional HTTP clients to interact with the Internet Computer network. This is important for software such as web browsers to be able to fetch and render client-side canister code, including HTML, CSS, and JavaScript as well as other static assets such as images or videos. The HTTP Gateway does this by translating between standard HTTP requests and [API canister calls](/references/ic-interface-spec/#http-interface) that the Internet Computer Protocol will understand.
...
- `certificate`: [Base64 encoded](https://www.rfc-editor.org/rfc/rfc4648#section-4) string of self-describing, [CBOR-encoded](https://www.rfc-editor.org/rfc/rfc8949.html) bytes that decode into a valid [certificate](/references/ic-interface-spec/#certification).
- `tree`: [Base64 encoded](https://www.rfc-editor.org/rfc/rfc4648#section-4) string of self-describing, [CBOR-encoded](https://www.rfc-editor.org/rfc/rfc8949.html) bytes that decode into a valid hash tree as per [certificate encoding](/references/ic-interface-spec/#certification-encoding).
...
- The path must be the most specific path for the current request URL in the tree, i.e. a lookup of more specific paths must return `Absent` as per [lookup](/references/ic-interface-spec/#lookup).
...
Certificate validation is performed as part of [response verification](#response-verification) as per [Canister Signatures](/references/ic-interface-spec/#canister-signatures) and [Certification](/references/ic-interface-spec/#certificate). It is expanded on here concerning [response verification](#response-verification) for completeness:
...
3. The decoded `certificate` must pass the following validations:
   ...
   - The subnet state tree in the certificate must reveal the canister's [certified data](/references/ic-interface-spec/#system-api-certified-data).
4. The root hash of the decoded `tree` must match the canister's [certified data](/references/ic-interface-spec/#system-api-certified-data).
