...
* Submit the `call` once, and poll `read_state` while at least one of the following conditions is met: 
  * `ingress_expiry` is not reached.
  * The call status is `processing`.

  Evaluating the first condition may be hard if the IC time and local time differ. Currently, agent-js and agent-rs by default poll until `ingress_expiry` is reached. 
* Do the same as above, but resubmit the initiall `call` (same `request_id`) e.g. every `30s`. This can be beneficial because a call that didn’t make it is retried.
