...
* Only poll `read_state` until `ingress_expiry` is reached. Note that this may be hard if the IC time and local time differ. This is currently the behaviour of agent-js and agent-rs.
...
If the expiry is reached, there is no way for the client to determine if the call was successful unless the canister’s API provides this information. In this case, after the message has expired, the user must be instructed to manually check the status of the call using application-specific APIs. There can be (hopefully uncommon) cases where the APIs don't allow checking this, in which case the user simply can’t tell if the message has been executed. Ideally, applications set expiry values such that under typical load timeouts are rare.