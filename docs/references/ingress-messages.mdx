...
* _No retries, only poll `read_state`:_ Submit the `call` once, and poll `read_state` until at least one of the following conditions is met:
  ...
  * `ingress_expiry` is reached, and the call status is not `received` or `processing`.
...
* _Retry the call while polling `read_state`_ Do the same as above, but resubmit the initial `call` (same `request_id`) periodically (e.g. every `30s`) until at least one of the following conditions is met: 
  * The call is processing or a certified response was produced. That is, the call status is `received`, `processing`, `replied`, `rejected`, or `done`.
...
At the time of writing, both agent-js (v2.1.3) and agent-rs (v0.39.1) don't retry `call`, so they follow the first option above. By default, agent-js uses `ingress_expiry` of five minutes and also polls for five minutes. agent-rs sets a default `ingress_expiry` of four minutes and polls for five minutes, but whenever it observes a `received` or `processing` status, it resets the timer to allow for an additional four minutes of polling. 
