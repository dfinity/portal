...
Type cd.. And hit enter to go up a folder in the directory
Repeat as necessary until you are at the C:\> prompt
Go to the folder that the OpenSC tools are in which should be in your Program Files folder. If it is, then you would go there by typing this: 
cd Program Files/OpenSC Project/OpenSC/tools
Continue with the following directions. (You can copy and paste with Control-V into the command prompt if you wish)
