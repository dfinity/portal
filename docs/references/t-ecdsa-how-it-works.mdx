...
-   [`sign_with_schnorr`](/docs/current/references/ic-interface-spec#ic-ecdsa_public_key): this method returns a new Schnorr signature of the given `message_hash` that can be separately verified against a derived Schnorr public key. 
...
{/* Something about encoding */}
This call requires that the Schnorr feature is enabled, the caller is a canister, and `message_hash` is 32 bytes long. Otherwise it will be rejected.
...
```
  schnorr_public_key : (record {
    canister_id : opt canister_id;
    derivation_path : vec blob;
    key_id : record { curve: schnorr_curve; name: text };
  }) -> (record { public_key : blob; chain_code : blob; });
