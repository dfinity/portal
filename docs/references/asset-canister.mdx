...
A canister configured as `"type": "asset"` already contains the boilerplate code to facilitate these HTTP requests, making serving static content simple. Once the asset canister is deployed on ICP, the frontend interface can be accessed at http://<canister id>.ic0.app and http://<canister id>.raw.ic0.app.
...
- [mops server](https://mops.one/server)

- [mops certified-http](https://mops.one/certified-http)
...
Dynamic URLs are currently not supported. The asset canister is only for static assets.
