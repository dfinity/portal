...
- **Signing and submitting Bitcoin transactions**: One of the special features of ICP is its direct integration with the [Bitcoin](/docs/build-on-btc/index) network. This is exposed to users through a set of API methods on the management canister that allow users to sign transactions with [threshold ECDSA](/docs/building-apps/network-features/signatures/t-ecdsa) and [threshold Schnorr](/docs/building-apps/network-features/signatures/t-schnorr) signatures and submit transactions to the Bitcoin network, all done through canisters on ICP. The relevant APIs are [`sign_with_ecdsa`](/docs/references/ic-interface-spec#ic-sign_with_ecdsa), [`sign_with_schnorr`](/docs/references/ic-interface-spec#ic-sign_with_schnorr) and [`bitcoin_send_transaction`](/docs/references/ic-interface-spec#ic-bitcoin_send_transaction). [Read more about ICP's direct integration with Bitcoin.](/docs/build-on-btc/index)
