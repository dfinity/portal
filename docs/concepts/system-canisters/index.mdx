...
The [Bitcoin mainnet](/docs/current/building-apps/chain-fusion/bitcoin/overview) system canister is used to facilitate connection from the ICP mainnet to the Bitcoin mainnet.
...
The [Bitcoin testnet](/docs/current/building-apps/chain-fusion/bitcoin/overview) system canister is used to facilitate connection from the ICP mainnet to the Bitcoin testnet.
...
The [cycles minting](/docs/current/concepts/gas-cost) canister is used to mint cycles on the mainnet.
...
The [ICP index](/docs/current/defi/token-indexes/) canister is the corresponding index canister for the ICP ledger.
...
The [ICP ledger](/docs/current/defi/token-indexes/) canister is used for sending and receiving ICP tokens on the mainnet. It can also be used to obtain account information, such as the account's current balance of ICP.
...
The [ICP ledger archive](/docs/current/defi/token-indexes/) canisters are used to store historic data about ICP token transactions. There are two ICP ledger archive canisters.
...
The [Internet Identity](/docs/current/building-apps/authentication/overview) canister is used to provide an authentication method used across ICP applications, such as the NNS dapp.
...
View the [Internet Identity specification](/docs/current/specifications/ii-spec).
...
The [SNS Wasm](/docs/current/building-apps/governing-dapps/overview) canister is used for deploying and updating SNSes.
...
For example, Internet Identity can be integrated into a dapp to provide user authentication service to the project. Internet Identity is commonly integrated into a dapp through the [`dfx deps`](/docs/current/building-apps/creating-dapps/using-third-party-canisters) command. `dfx deps` allows you to pull a system canister into your local project for testing.
