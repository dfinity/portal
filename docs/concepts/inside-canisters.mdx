...
## Canister storage
There are two types of [storage](/docs/current/building-apps/managing-dapps/storage) available to the canister: the Wasm memory and the stable memory.
...
The function can access the stable memory by calling special [system functions](/docs/current/specifications/ic-interface-spec#system-api-stable-memory) such as `stable64_read` and `stable64_write`.
...
## Cycles balance
Canisters pay for execution in [cycles](/docs/current/concepts/gas-cost).
...
## Controllers
Only [controllers](/docs/current/building-apps/managing-dapps/control) of the canister can do certain system-level operations on the canister, such as stopping it, starting it, upgrading it, etc.
...
[View the full list of canister settings](/docs/current/specifications/ic-interface-spec#ic-create_canister).