...
Canister smart contracts pay for consumed resources (storage, messaging, execution, and special features) by burning **cycles**. Developers can obtain cycles by converting ICP tokens into cycles using [`dfx`](/docs/current/defi/cycles/converting_icp_tokens_into_cycles).

For pricing estimations, you can use the [pricing calculator](/docs/current/concepts/cost-estimations-and-examples).
...
Learn how to query a [canister's cycles balance](/docs/current/building-apps/managing-dapps/topping-up#checking-the-cycles-balance-of-a-canister).

Learn how to [top up a canister](/docs/current/building-apps/managing-dapps/topping-up).

A [freezing threshold](/docs/current/building-apps/managing-dapps/settings/#freezing-threshold) can be set that pauses a canister's executions if the cycles amount is expected to fall below a certain amount. There are several community tools that have been developed to automate managing a canister's cycles, such as [CycleOps](https://cycleops.dev/).

## Units and fiat value

The price of cycles is fixed against the price of [XDR](/docs/current/concepts/glossary#x), where **1 trillion cycles equals 1 XDR**. As of December 18, 2023, the exchange rate for 1 XDR = $1.336610. The exchange rate for USD/XDR may vary. Learn more about [XDR exchange rates](https://www.imf.org/external/np/fin/data/rms_sdrv.aspx).
...
### Storage
Canisters pay for storage consumed by their [Wasm memory](/docs/current/building-apps/managing-dapps/storage#heap-memory) and [stable memory](/docs/current/building-apps/managing-dapps/storage#stable-memory) per time. Storing 1 GiB for 1 second costs 127k cycles, which amounts to ~4T cycles (or $5.35 USD) for storing 1 GiB for 1 year.
...
- **Bitcoin API**: Pricing for the **Bitcoin API** is available in the [Bitcoin API documentation](/docs/current/specifications/bitcoin-how-it-works).

- **Chain-key signing API**: Pricing for the **threshold signatures**, such as threshold ECDSA and threshold Schnorr, is available in the [threshold signatures documentation](/docs/current/specifications/t-sigs-how-it-works/).

- **EVM RPC** Pricing for the **EVM RPC** canister is available in the [EVM RPC canister documentation](/docs/current/building-apps/chain-fusion/ethereum/overview).
...
- [Insufficient cycles in memory grow](/docs/current/specifications/execution-errors#insufficient-cycles-in-memory-grow).
- [Reserved cycles limit exceeded in memory grow](/docs/current/specifications/execution-errors#reserved-cycles-limit-exceeded-in-memory-grow).
- [Insufficient cycles in message memory grow](/docs/current/specifications/execution-errors#insufficient-cycles-in-message-memory-grow).
- [Insufficient cycles in compute allocation](/docs/current/specifications/execution-errors#insufficient-cycles-in-compute-allocation).
- [Insufficient cycles in memory allocation](/docs/current/specifications/execution-errors#insufficient-cycles-in-memory-allocation).
- [Insufficient cycles in memory grow](/docs/current/specifications/execution-errors#insufficient-cycles-in-memory-grow-1).
- [Reserved cycles limit exceeded in memory allocation](/docs/current/specifications/execution-errors#reserved-cycles-limit-exceeded-in-memory-allocation).
- [Install code not enough cycles](/docs/current/specifications/execution-errors#install-code-not-enough-cycles).
- [Create canister not enough cycles](/docs/current/specifications/execution-errors#create-canister-not-enough-cycles).

## Further readings

### Monitoring cycles usage
- [Check your canister's cycles balance](/docs/current/defi/cycles/cycles-wallet#check-the-cycle-balance).
- [Monitor your canister's cycles usage](/docs/current/building-apps/managing-dapps/topping-up).
...
- [Top up your canisters](/docs/current/building-apps/managing-dapps/topping-up).
- [Cycles management services](/docs/current/building-apps/managing-dapps/topping-up).
...
- [IC interface specification](/docs/current/specifications/ic-interface-spec#system-api-performance-counter).