...
[the respective system APIs](/docs/current/specifications/ic-interface-spec#system-api-call) to make the call and will become ready once the response from the calling canister is received. The language runtime can poll the future once it's ready and retrieve the result of the inter-canister call.
...
When canister A calls B, a future that represents the result of the call is created. The future calls [the system API](/docs/current/specifications/ic-interface-spec#system-api-call)
