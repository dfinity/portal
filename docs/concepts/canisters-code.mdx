...
Since query calls do not go through consensus, [certified responses](/docs/current/developer-docs/security/security-best-practices/data-integrity-and-authenticity) should be used wherever possible when data only needs to be read, not updated.
