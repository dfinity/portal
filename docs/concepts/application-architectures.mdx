...
The default project structure that [`dfx new`](/docs/current/building-apps/developer-tools/dfx/dfx-new) generates can be viewed as the simplest microservice architecture, with the frontend canister being responsible for serving web assets and the backend canister being responsible for the core logic of the application.
