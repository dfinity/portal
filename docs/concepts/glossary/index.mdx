...
blockchain with blocks containing messages for the [canisters](#cs)
...
allow users to seamlessly access the [canister](#cs) smart contracts
running on ICP.
...
at a specific place in its state tree. Details can be found in the [Internet Computer interface specification](/docs/current/specifications/ic-interface-spec/).
...
software. The consensus layer selects [messages](#m)
...
The **governance canister** is a [system canister](#s) that implements the
...
[The Internet Computer interface specification](/docs/current/specifications/ic-interface-spec) has more
detail.
...
The **message routing** layer receives [batches](#b) from
...
account](#c) whose
[beneficiary](#beneficiary) is a [neuron](#n)
...
[principal](#p) who has the authority to add/remove
[nodes](#n) to/from the
[ICP](#i), by delegation of the
the corresponding [node providers](#n).
...
    routing](#m) and the [execution
    environment](#e) operates on.
- Converting back and forth between the replicated state and its
    canonical version (the latter can be understood independent of the
    concrete implementation).
- Obtaining certifications of parts of the canonical state, which
    allow other stakeholders, such as other [subnets](#s)
...
    [replicas](#r) in the same subnet so that replicas
