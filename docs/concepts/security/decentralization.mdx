...
Furthermore, the controller of canisters serving web content (such as e.g., the [asset canister](/docs/current/specifications/asset-canister)) could maliciously modify the web application to e.g., steal user funds or perform security-sensitive actions on the user's behalf. For example, if [Internet Identity](/docs/current/building-apps/authentication/overview) is used, the user principal's private key for the given origin is stored in the browser storage, and a malicious app can therefore fully control the private key, the user's session, and any assets controlled by that key.
...
3. **Full decentralization using a DAO**: The dapp is controlled by a decentralized governance system such as ICP's [Service Nervous System (SNS)](/docs/current/building-apps/governing-dapps/overview), so that any security-sensitive changes to the canisters are only executed if the SNS community approves them collectively through a proposal voting mechanism. If an SNS is used:
    - Make sure voting power is distributed over many independent entities such that there is not one single or a few entities that can decide by themselves how the [DAO evolves](/docs/current/building-apps/governing-dapps/tokenomics/tokenomics-intro#voting-power-and-decentralization).
    - Ensure all components of the dapp are under SNS control, including the canisters serving the web frontends; see [SNS asset canisters](/docs/current/building-apps/governing-dapps/managing/sns-asset-canister).
    - Consider the [SNS preparation checklist](/docs/current/building-apps/governing-dapps/tokenomics/sns-checklist). Important points from a security perspective are [tokenomics](/docs/current/building-apps/governing-dapps/tokenomics/sns-checklist#11-tokenomics-specification), [disclosing dependencies to off-chain components](/docs/current/building-apps/governing-dapps/tokenomics/sns-checklist#13-disclosure-of-dependencies) and [performing security reviews](/docs/current/building-apps/governing-dapps/tokenomics/sns-checklist#21-security-review).
    - Rather than [self-deploying the SNS code](/docs/current/building-apps/governing-dapps/overview#self-deploy-the-sns-code) or [building your own DAO](/docs/current/building-apps/governing-dapps/overview#build-your-own-dao--use-other-dao-frameworks), consider using the official SNS on the SNS subnet, as this guarantees that the SNS is running an NNS-blessed version and maintained as part of ICP.
...
- Use a [content security policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) to prevent scripts and other content from other origins from being loaded at all. See also [define security headers, including a content security policy (CSP)](/docs/current/concepts/security/resources#web-security).