...
ICP offers three modes of operation for canisters: `update`, `query`, and `composite_query`. For the sake of simplicity, we will club `composite_query` under queries for the rest of this section. For more information, view the [detailed overview between `update` and `query` calls](/docs/current/building-apps/calling-dapps/query-calls).
