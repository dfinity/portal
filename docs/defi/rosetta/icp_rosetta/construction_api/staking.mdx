...
This guide assumes you are familiar with the [flow of operations](/docs/current/defi/rosetta/icp_rosetta/construction_api/operations-flow).
...
To derive the account identifier of your public key, you can look at how the [derive endpoint works](/docs/current/defi/rosetta/icp_rosetta/construction_api/operations-flow).
...
However, the governance canister must be notified of this transfer to activate the neuron. This process is done in the [next step of the guide](/docs/current/defi/rosetta/icp_rosetta/construction_api/staking).
...
After that, you can fetch the [neuron info](/docs/current/defi/rosetta/icp_rosetta/construction_api/staking) and it should display that it is now in `DISSOLVING` state.
...
The command has two forms: one form accepts an [ICP principal](/docs/current/specifications/ic-interface-spec) as a hotkey, and another form accepts a [public key](https://www.rosetta-api.org/docs/models/PublicKey.html).
...
There are a few variations to this that depend on whether you are the controller of the neuron or whether you are using a [hotkey](/docs/current/defi/rosetta/icp_rosetta/construction_api/staking).
