...
For testing and development, you can connect to the test ICP ledger (`xafvr-biaaa-aaaai-aql5q-cai`) that runs on mainnet but uses test tokens (TESTICP). Get free test tokens from [the faucet](https://nqoci-rqaaa-aaaap-qp53q-cai.icp0.io/) to start experimenting without using real ICP.
...
Use the [ICP faucet](https://nqoci-rqaaa-aaaap-qp53q-cai.icp0.io/) to get TESTICP tokens from the test ICP ledger (`xafvr-biaaa-aaaai-aql5q-cai`). This allows you to test ICP Rosetta functionality without using real ICP tokens.
