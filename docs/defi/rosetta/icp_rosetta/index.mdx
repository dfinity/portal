...
For detailed setup instructions, see the [Running Rosetta](/defi/rosetta/icp_rosetta/running-rosetta) guide.
...
The easiest way is to use the [`dfinity/rosetta-api`](https://hub.docker.com/r/dfinity/rosetta-api/tags?page=1&ordering=last_updated) Docker image. See the [Running Rosetta](/defi/rosetta/icp_rosetta/running-rosetta) guide for detailed instructions on all deployment methods.
...
Use the [faucet](https://faucet.internetcomputer.org/) to get TESTICP tokens from the test ICP ledger (`xafvr-biaaa-aaaai-aql5q-cai`). This allows you to test ICP Rosetta functionality without using real ICP tokens.
