...
Better metadata results in a better user experience and wallet integration. [Learn more about metadata](/tutorials/hackathon-prep-course/integrating-with-tokens#icrc-1-tokens).

## Transaction deduplication

#### Recommendation: Use the `created_at_time` and `memo` parameters to make transaction calls [idempotent](/building-apps/best-practices/idempotency#id-deduplication).
...
There is a limit for the number of deduplicated transactions that can be stored in the [deduplication store](/building-apps/best-practices/idempotency#id-deduplication). Transactions are rejected once this store is full until older transactions expire at the end of the time window.
...
Issues with [inter-canister calls](/building-apps/security/overview) can lead to time-of-check, time-of-use, or double spending security bugs.
