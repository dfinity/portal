...
For a feature comparison between the two protocols, refer to the [quick comparison between ICP and Ethereum](/building-apps/essentials/network-overview#quick-comparison-with-ethereum).
...
| Message        | Memo      | On Ethereum, you can encode data, often referred to as a message, as a hexadecimal in the optional `data` field of any transaction. Some tools allow you to input text, which they then convert to hexadecimal in the background. On ICP, you can encode data as an arbitrary blob, provided it is less than 32 bytes, in the optional `memo` field of any [ICRC-1 transaction](https://github.com/dfinity/ICRC-1/tree/main/standards/ICRC-1#icrc1_transfer-). |

In addition, ICP has implemented similar token standards as Ethereum:

| Ethereum standard | ICP standard |
|-------------------|--------------|
| ERC-20            | [ICRC-1](/defi/token-standards/#icrc-1)/[ICRC-2](/defi/token-standards/#icrc-2) |
| ERC-721           | [ICRC-7](/defi/token-standards/#icrc-7) |


## Smart contracts

Both Ethereum and ICP allow developers to deploy programs that run onchain. These programs are referred to as smart contracts on Ethereum and canister smart contracts, or simply just [canisters](/building-apps/essentials/canisters), on ICP.
...
**ICP**: Canisters can be written in any language that compiles into [Wasm](https://webassembly.org/) or has an interpreter that can compile to Wasm, including [Rust](/building-apps/developer-tools/cdks/rust/intro-to-rust), [Typescript](https://demergent-labs.github.io/azle/), [Motoko](/motoko/home), and [Python](https://demergent-labs.github.io/kybra/).
...
**ICP**: Developers use `dfx`, which is a similar development environment. [`dfx`](/building-apps/getting-started/install#installing-dfx-via-dfxvm) is a command-line tool that interacts with the IC SDK and allows developers to compile and deploy their canisters.
...
**ICP**: Currently, testing is done in the local environment. A [playground environment](/building-apps/developing-canisters/deploy/) on the mainnet is available for canisters, but deployment time is limited to 20 minutes. However, the ability to upgrade and delete canisters on the mainnet allows developers greater flexibility to iterate and refine their smart contracts than on Ethereum.


#### Paying for gas

**Ethereum**: Users pay for gas fees directly using the crypto wallet they connect to the dapp.
**ICP**: Using the [reverse gas model](/building-apps/essentials/gas-cost#the-reverse-gas-model), the dapp developers, not the users, pay for the gas fees. The reverse gas model aims to provide a smoother user experience by eliminating upfront fees.
