...
ICP tokens are used for different actions on the network, such as being converted into <GlossaryTooltip>[cycles](/docs/building-apps/getting-started/tokens-and-cycles)</GlossaryTooltip> to pay for a <GlossaryTooltip>canister</GlossaryTooltip>'s resources, or being staked in a neuron to receive voting power on the [<GlossaryTooltip>NNS</GlossaryTooltip>](/docs/building-apps/governing-apps/nns/overview).
...
- The ICP ledger canister that is running on the [NNS](/docs/building-apps/governing-apps/nns/overview): Used to record `burn`, `mint`, `approve`, and, most commonly, `transfer` transactions with regards to ICP's native [token](/docs/defi/token-standards/) `ICP`. The ICP ledger canister services transaction requests and offers a variety of endpoints to fetch data and information about the state of the ICP ledger.
