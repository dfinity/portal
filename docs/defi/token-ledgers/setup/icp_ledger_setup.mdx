...
For example, to use the `secp256k1` key, create a new file called `ident-1.pem` that contains the following content:
...
Go to the [releases overview](https://dashboard.internetcomputer.org/releases) and copy the latest replica binary release hash.

The URL for the ledger Wasm module is `https://download.dfinity.systems/ic/<RELEASE_HASH>/canisters/ledger-canister.wasm.gz`.

The URL for the ledger's Candid file is `https://raw.githubusercontent.com/dfinity/ic/<RELEASE_HASH>/rs/ledger_suite/icp/ledger.did`.
...
Replace <RELEASE_HASH> with the latest replica release hash obtained from the dashboard.
