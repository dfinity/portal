...
- **Approving ICRC tokens** to be spent by some other `Account`, and/or **transferring ICRC tokens from** a previously approved `Account`. Approve and transfer workflows are supported in ICRC-2 compatible ledgers. [Learn more about token standards](/docs/defi/token-standards).
