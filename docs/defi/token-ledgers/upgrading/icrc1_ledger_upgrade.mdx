...
In such cases, this is documented in the “Upgrade and Downgrade Notes” section of the release.
...
3. For each canister in the ledger suite, in [the order specified earlier](https://c4y23-xqaaa-aaaam-abayq-cai.icp0.io/docs/defi/token-ledgers/upgrading/icrc1_ledger_upgrade#component-upgrade-order/docs/defi/token-ledgers/upgrading/icrc1_ledger_upgrade#component-upgrade-order), test the upgrade, take a snapshot, perform the upgrade, and [verify the upgrade](/docs/defi/token-ledgers/upgrading/icrc1_ledger_upgrade#verifying-upgrades).
...
5. Don’t upgrade the production ledger suite canisters on a Friday 😉
