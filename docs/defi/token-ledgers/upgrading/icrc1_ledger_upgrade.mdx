...
Development of ledger implementations have prioritized the ability to downgrade from any version of the ledger to its immediately preceding version until the migration of the ledgers to use stable storage for key data structures (balances, approval information) caused breaking changes. Additionally, migrating from heap-based data storage to stable storage requires a series of carefully orchestrated steps, each of which introduce critical changes that subsequent upgrades depend upon.
