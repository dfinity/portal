...
:::caution
The cycles ledger is supported by default in [`dfx` version 0.22.0 and newer](/building-apps/getting-started/install).
:::

### Obtaining ICP

To use the cycles ledger, first, you also need to have [a developer identity](/building-apps/getting-started/identities), then you need to [obtain ICP tokens](/building-apps/getting-started/tokens-and-cycles). These steps are necessary to create cycles and canisters.
...
- To create a canister, the fee is the transfer fee (100M cycles), plus the canister creation fee, which scales [linearly with the size of the subnet](/building-apps/essentials/gas-cost) that the canister is created on. For example, if the canister is created on a 13-node subnet, the fee is 500B cycles. This means the total fee will be 500_100_000_000 cycles.
