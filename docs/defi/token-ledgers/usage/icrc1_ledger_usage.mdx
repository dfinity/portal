...
View the [`dfx canister call` documentation](/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-call) for more information on calling canister methods.
...
When calling into arbitrary ICRC-1 ledgers, it is recommended that you use [bounded wait (aka best-effort response) calls](/references/async-code). These calls ensure that your canister does not get stuck waiting for a response from the ledger.
