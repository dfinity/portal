...
[`dfx ledger`](/building-apps/developer-tools/dfx/dfx-ledger) is a convenience command to interact with the ICP ledger canister and related functionality from the CLI. It only exposes a subset of the ICP ledger functionality, namely `balance` and `transfer`.

`dfx` does not come with an ICP ledger instance installed locally by default. To use this command, you will need to install the [ICP ledger locally](/defi/token-ledgers/setup/icp_ledger_setup).

View the [`dfx ledger`](/building-apps/developer-tools/dfx/dfx-ledger) documentation for all available arguments and flags.
...
[View a more detailed description of the data types used in these commands](/references/ledger#_getting_ledger_blocks).
...
View the [inter-canister call documentation](/building-apps/developer-tools/cdks/rust/intercanister) to learn more about inter-canister calls.

## The ICP ledger's ICRC endpoints

As explained in the [token standards documentation](/defi/token-standards/), the ICP ledger supports all ICRC-1 endpoints. You will need to define the structures used for these endpoints.
...
The sender notifies the receiver about the payment. However, the receiver needs to verify the payment by using the [`query_blocks` interface](/references/ledger#_getting_ledger_blocks) of the ledger.
