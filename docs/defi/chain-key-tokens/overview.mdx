...
ICP introduces a more secure alternative to wrapped tokens known as **chain-key** tokens. Chain-key tokens are created using ICP's chain-key cryptography protocols, such as [threshold signing](/references/glossary#c). Each chain-key token is deployed on ICP and backed 1:1 with the native token held 100% onchain by an ICP smart contract. There are no intermediaries or centralized bridges used for chain-key tokens.
...
- Must adhere to an [ICRC](/defi/token-standards/#icrc-1) token standard.
