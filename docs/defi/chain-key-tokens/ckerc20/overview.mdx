...
Note that the [same requirements](/docs/current/defi/chain-key-tokens/overview#requirements-for-chain-key-tokens) as for any other type of chain-key token apply.
...
The ledger and index canisters in any ckERC20 ledger suite are regular ICRC-1 compliant canisters. A guide on how to interact with these canisters can be found in the [using the ICRC-1 ledger](/docs/current/defi/token-ledgers/usage/icrc1_ledger_usage) section.
