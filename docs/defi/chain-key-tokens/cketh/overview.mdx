...
Like [ckBTC](/docs/current/defi/chain-key-tokens/ckbtc/overview), ckETH does not rely on a centralized bridge to facilitate the conversion between ETH and ckETH, which makes it substantially more secure when compared to other traditional 'wrapped' tokens.
...
The **ckETH ledger** and **ckETH index** canisters are regular ICRC-1 compliant canisters. A guide on how to interact with these canisters can be found in the [using the ICRC-1 ledger](/docs/current/defi/token-ledgers/usage/icrc1_ledger_usage) section.
