...
A client application (such as a frontend, CLI tool, or script) can use an **[agent](/docs/building-apps/interact-with-canisters/agents/overview)** to interact directly with the ckBTC ledger canister.

An **agent** is the API client layer used to call ICP canisters. For example, the [ICP JavaScript agent](/docs/building-apps/interact-with-canisters/agents/javascript-agent) can create an actor from a canister’s Candid interface and provides methods like `call`, `query`, and `readState`.
...
In this model, all transactions are executed from the user’s own [account](/docs/defi/token-standards/#accounts-principals-and-subaccounts) (typically the principal without a subaccount).
...
- **Motoko:** [icrc1-types](https://mops.one/icrc1-types), [icrc2-types](https://mops.one/icrc2-types)
...
- **Client calls (via agent)**
  ...
  - **TypeScript:** [@dfinity/ckbtc](https://www.npmjs.com/package/@dfinity/ckbtc)

- **Inter-canister calls**
  - A backend canister can call the ckBTC minter canister as part of its logic, either for its own use or on behalf of users.

Learn more about implementing this conversion flow in the [ckBTC best practices](/docs/defi/defi-best-practices).
...
## Examples applications for ckBTC
