...
It can also be run as a standalone binary on macOS and Linux systems that doesn't require additional containers or virtual machines.
...
Currently, it supports Rust, Python, and JavaScript/TypeScript and can support integration with any language that is written against the [PocketIC REST API](https://docs.rs/pocket-ic/latest/pocket_ic/).

## How to use PocketIC

There are two ways to use PocketIC:

1. PocketIC can be used to create a local development network with `dfx start`. Canisters deployed to this local network can be interacted with via `dfx` commands or through their Candid UI.
...
2. You can create automated tests through a PocketIC client library. In this workflow, the client library starts the PocketIC server, which the file path must be configured as the `POCKET_IC_BIN` environment variable.
...
## Using PocketIC for automated tests
