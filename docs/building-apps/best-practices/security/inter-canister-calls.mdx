...
To understand the issues around async inter-canister calls, one needs to understand the [properties of message execution on ICP](/docs/move-to-learnhub/message-execution-properties). Understanding these properties is a prerequisite for understanding the security issues discussed below.
