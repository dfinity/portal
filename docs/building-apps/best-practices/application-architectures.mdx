...
This means that developers have a lot of freedom with regards to how they can organize and store data. The recommended practice is to use already existing libraries, such as the Motoko [stable regions](/motoko/main/stable-memory/stable-regions) library or the Rust [`ic-stable-structures`](https://github.com/dfinity/stable-structures) library, to store data in the stable memory.
...
The default project structure that [`dfx new`](/building-apps/developer-tools/dfx/dfx-new) generates can be viewed as the simplest microservice architecture, with the frontend canister being responsible for serving web assets and the backend canister being responsible for the core logic of the application.
