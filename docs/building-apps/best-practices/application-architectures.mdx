...
The canister-per-user architecture is based on the vision that Web3 users should have full control over their data, with the idea that a new canister should be created for each user, and the user should be a controller of the canister. This architecture provides user's sovereignty over apps, that is that nobody can delete or update his/her app without user's consent.
...
A package manager is implemented e.g. in IC Pack project ([MVP beta](https://app.package-manager.com), [business site](https://docs.package-manager.com), [dev site](https://dev.package-manager.com)) - a software for canister-per-user architecture.
