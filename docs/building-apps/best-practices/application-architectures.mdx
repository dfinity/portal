...
It is implemented in IC Pack package manager ([MVP preliminary alpha](https://app.package-manager.com), [business site](https://docs.package-manager.com), [dev site](https://dev.package-manager.com)) - the software that allows to do canister-per-user architecture easily, making the user self-sovereign over their software. So, you will use IC Pack to create your canister-per-user apps. This architecture provides user's sovereignty, so that nobody can delete or update his/her app without user's consent.
