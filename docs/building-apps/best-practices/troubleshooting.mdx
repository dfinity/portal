...
For troubleshooting specific to the IC SDK, see [IC SDK troubleshooting](/building-apps/getting-started/troubleshooting).

For troubleshooting specific to using the IC SDK with WSL, see [WSL troubleshooting](/building-apps/getting-started/wsl-troubleshoot).
...
Requesting data from [heap memory](/building-apps/canister-management/storage) instead of [stable memory](/building-apps/canister-management/storage) can reduce latency in some applications.
...
To ensure that your canister is optimized, both for latency and cycles cost, be sure to follow the [canister general best practices](/building-apps/best-practices/general), which include ways to implement efficient Motoko and Rust code.
