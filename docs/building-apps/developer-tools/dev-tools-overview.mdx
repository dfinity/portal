...
`dfx` supports [local development](/docs/building-apps/developing-canisters/deploy) and deployment through the [`dfx start`](/docs/building-apps/developer-tools/dfx/) command, which runs a local development environment using [PocketIC](/docs/building-apps/test/pocket-ic) that can be used to deploy and test canisters on your local machine.
