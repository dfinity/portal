...
This guide provides a simple example of how you can use the Rust CDK interfaces and macros to simplify writing dapps in Rust for ICP.
...
dfx new rust_profile --type=rust
```
...
Now that you have the files in place for your Rust dapp, you can replace the template `lib.rs` dapp with the Rust dapp you want to deploy on ICP.
...
``` toml title="src/rust_profile_backend/Cargo.toml"
[dependencies]
candid = "0.8.2"
ic-cdk = "0.6.0"
serde = "1.0"
...
Candid is an interface description language (IDL) for interacting with canisters running on ICP. To see details about the Candid interface description language syntax, see the [Candid guide](/docs/current/building-apps/calling-dapps/candid/candid-concepts) or the [Candid crate documentation](https://docs.rs/candid/).

To update the Candid file for this guide, open the `src/rust_profile_backend/rust_profile_backend.did` file in a text editor.
...
```did title="src/rust_profile_backend/rust_profile_backend.did"
type Profile = record {
    "name": text;
    "description": text;
    "keywords": vec text;
...
Next, register, build, and deploy your project locally.
...
Call the `search` function using part of the description or a keyword to further test whether the correct profile is returned.
...
Call the `search` function to further test whether the correct profile is returned.
