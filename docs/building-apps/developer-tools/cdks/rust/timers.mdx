...
```toml title="src/my_timers_backend/Cargo.toml"
[package]
...
```candid title="src/my_timers_backend/my_timers_backend.did"
service : (nat64) -> {
      "counter": () -> (nat64) query;
}
```
...
```rust title="src/my_timers_backend/src/lib.rs"
...
:::info
Note: As described in the [periodic tasks and timers](/docs/current/building-apps/using-network-features/periodic-tasks-timers) page, the timers library does not handle canister upgrades. It is up to the canister developer to serialize the timers in the `canister_pre_upgrade` and reactivate the timers in the `canister_post_upgrade` method if needed.
