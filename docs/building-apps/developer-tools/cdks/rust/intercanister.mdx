...
To follow along, clone the [Git repository](https://github.com/dfinity/examples) of the example repo. The code for this tutorial is in the `rust/inter-canister-calls` directory, where you can find the completed code in the `caller` package. Note that this example uses the alpha version of the Rust CDK.
...
As the example notes, panicking when handling call errors is generally a bad idea past trivial examples such as this one. This is due to the nature of the [Internet Computer](https://learn.internetcomputer.org/hc/en-us/articles/33152818663444-What-is-ICP)'s messaging model.
...
:::info
To summarize, the Internet Computer has an asynchronous messaging model that is  different from Ethereum and many other blockchains. This improves [scalability](https://learn.internetcomputer.org/hc/en-us/articles/34576974172692-Evolution-Scaling), but you need to understand the implications, especially around when and how state is persisted.
:::
...
Let's test this by calling the `sign_message` method. The exact signature in your response will differ, as it will based on a secret key from your local replica.
