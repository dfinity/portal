...
**Description**: Define custom local networks. [Learn more about custom network definitions and configurations](/building-apps/developing-canisters/custom-networks).
...
There are several workflow-specific `dfx.json` configuration options detailed in the full [`dfx.json` schema](/building-apps/developer-tools/dfx-json-reference).
