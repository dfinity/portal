...
In `dfx` versions `0.19.0` and newer, the flag `--output json` can be used with a `dfx canister call` command. This removes the need to pipe the output to `idl2json`. [Learn more in the `dfx` documentation](/docs/current/building-apps/developer-tools/dfx/).
