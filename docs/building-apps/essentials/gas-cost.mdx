...
Canister smart contracts consume resources to execute methods, respond to or send messages, store data, and use special features like [HTTPS outcalls](/building-apps/network-features/using-http/https-outcalls/overview). Each resource costs **cycles**. Developers obtain cycles by converting ICP tokens into cycles using [`dfx`](/building-apps/getting-started/tokens-and-cycles).

The price of cycles is fixed against the price of [XDR](/references/glossary#x), where 1 trillion cycles equals 1 XDR. As of May 22, 2025, the [exchange rate for 1 XDR = $1.35](https://www.imf.org/external/np/fin/data/rms_sdrv.aspx). The exchange rate for USD/XDR may vary.
...
To prevent a canister from running out of cycles, a [freezing threshold](/building-apps/canister-management/settings/#freezing-threshold) can be set that pauses a canister's execution if the cycles balance is expected to fall below a certain amount, or[ topping up a canister](/building-apps/canister-management/topping-up) can be automated using tools like [CycleOps](https://cycleops.dev/).
...
More information about responsible canisters can be found on the [cycles calculation reference page.](/references/cycles-cost-formulas)
...
- [Inter-canister calls](/building-apps/interact-with-canisters/advanced-calls#inter-canister-calls): 260K base fee, plus 1K cycles per byte, paid by the sending canister.

- [Ingress messages](/building-apps/essentials/message-execution): 1.2M base fee, plus 2K cycles per byte, paid by the receiving canister.
...
[View the cost calculation formula for more information about how costs are calculated](/references/cycles-cost-formulas).

### Storage

Canisters pay for the storage used by both their [Wasm memory](/building-apps/canister-management/storage#heap-memory) and [stable memory](/building-apps/canister-management/storage#stable-memory). Cost is calculated based on the amount of storage and time duration, such that storing 1 GiB for 1 second costs 127k cycles, which amounts to \~4T cycles (approximately $5.35 USD) for storing 1 GiB for 1 year.
...
- [**HTTPS outcalls**](/references/cycles-cost-formulas#https-outcalls)

- [**Bitcoin API**](/references/bitcoin-how-it-works)
...
- [Insufficient cycles in memory grow](/references/execution-errors#insufficient-cycles-in-memory-grow).

- [Reserved cycles limit exceeded in memory grow](/references/execution-errors#reserved-cycles-limit-exceeded-in-memory-grow).

- [Insufficient cycles in message memory grow](/references/execution-errors#insufficient-cycles-in-message-memory-grow).

- [Insufficient cycles in compute allocation](/references/execution-errors#insufficient-cycles-in-compute-allocation).

- [Insufficient cycles in memory allocation](/references/execution-errors#insufficient-cycles-in-memory-allocation).

- [Insufficient cycles in memory grow](/references/execution-errors#insufficient-cycles-in-memory-grow-1).

- [Reserved cycles limit exceeded in memory allocation](/references/execution-errors#reserved-cycles-limit-exceeded-in-memory-allocation).

- [Install code not enough cycles](/references/execution-errors#install-code-not-enough-cycles).

- [Create canister not enough cycles](/references/execution-errors#create-canister-not-enough-cycles).
...
- [Check your canister's cycles balance](/building-apps/canister-management/topping-up#checking-the-cycles-balance-of-a-canister).
...
- To withdraw cycles from a canister, the [canister must be deleted](/tutorials/developer-liftoff/level-1/1.6-managing-canisters#getting-cycles-back-from-a-canister).


### Topping up canisters

- [Top up your canisters](/building-apps/canister-management/topping-up).
