...
Canister smart contracts pay for consumed resources (storage, messaging, execution, and special features) by burning **cycles**. Developers can obtain cycles by converting ICP tokens into cycles using [`dfx`](/docs/defi/cycles/converting_icp_tokens_into_cycles).
...
The price of cycles is fixed against the price of [XDR](/docs/concepts/glossary#x), where **1 trillion cycles equals 1 XDR**. As of December 18, 2023, the exchange rate for 1 XDR = $1.336610. The exchange rate for USD/XDR may vary. Learn more about [XDR exchange rates](https://www.imf.org/external/np/fin/data/rms_sdrv.aspx).
...
- [Check your canister's cycles balance](/docs/defi/cycles/cycles-wallet#check-the-cycle-balance).
...
- To withdraw cycles from a canister, the [canister must be deleted](/docs/tutorials/developer-journey/level-1/1.6-managing-canisters#getting-cycles-back-from-a-canister).
