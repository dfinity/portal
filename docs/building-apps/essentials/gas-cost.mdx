...
The cycles cost for an HTTPS outcall request has a fixed and variable component.
The fixed part accounts for the overheads associated with an HTTPS outcall, whereas the variable part charges for the resources consumed during the requests.
Just like with other functions, the cost is scaled to account for larger subnets.
...
The `request_bytes` include not only the http body, but the entire request:
