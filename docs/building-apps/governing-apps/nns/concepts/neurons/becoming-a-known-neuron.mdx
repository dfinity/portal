...
To update a known neuron, follow the same process for registering a known neuron (the RegisterKnownNeuron proposal type). Note a given known neuron name cannot be transferred to another neuron id through this proposal (for example, if there is an existing known neuron with name `n1` and id 123, a `RegisterKnownNeuron` proposal with name `n1` and id 456 would fail). Otherwise, the known neuron name can still be updated through this type of proposal.
