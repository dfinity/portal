...
To update a known neuron, follow the same process for registering a known neuron (the RegisterKnownNeuron proposal type). Note that both the neuron id and the known neuron name should be the same as the existing known neuron. More specifically, if there is an existing known neuron with name `n1` and id 123, a `RegisterKnownNeuron` proposal with name `n1` and id 456 would fail.
