...
To remove the legacy following, there are two options:

1. Since you cannot remove the catch-all in just one topic, there is an option to remove it for all topics, called "Deactivate catch-all." You can find it next to the non-critical topics, as the catch-all is only applied to non-critical proposals.  
