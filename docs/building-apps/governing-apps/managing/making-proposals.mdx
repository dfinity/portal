...
This allows an SNS to register an extension. An extension is a canister that need not be part of each SNS, so it is not an SNS framework canister, but it is an NNS-approved canister that can be added to those SNSs that need it.
...
To make a concrete example, assume we want to upgrade the canister under the ID `$TARGET` using a large Wasm (more than 2 MiB).

*1. Give the store canister a name* 
```
touch dfx.json
jq '.canisters += { "store": {} }' dfx.json > dfx.json
```

*2. Create the store canister using the current DFX identity* 
```
...
dfx canister create \
    ...
    --controller "$(dfx identity get-principal)" \
         store
```
Set the output canister id into the variable `STORE`.
        
*3. Prepare the Wasm chunks*
```
...
*4.  Upload the Wasm chunks, one-by-one.*
```
...
*5. Submit the `UpgradeSnsControlledCanister` proposal to upgrade the SNS-controlled canister using the chunks.*
...
As explained on in [Framework and Architecture](https://learn.internetcomputer.org/hc/en-us/articles/34140764336788-Framework-and-Architecture), all approved SNS canister versions are stored on the NNS canister SNS-W.
...
For these cases, SNSes have so-called "custom proposals" that each SNS community can define itself.
...
Typically a generic proposal will have the following structure: a developer sends a proposal to add, execute, remove, or assign a topic to a custom proposal, i.e., a "generic nervous system function". So even though the proposal types below are technically "native proposal types," they are used to manage generic proposals.
