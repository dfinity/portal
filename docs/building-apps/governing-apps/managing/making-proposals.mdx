...
In an SNS DAO, all decisions and changes happy by proposals.
Therefore, to manage an SNS, the SNS community needs to understand how proposals work, how they can be submitted, voted on, and what effect they have.

The [Learn Hub page on SNS proposals](https://learn.internetcomputer.org/hc/en-us/articles/34146571133204-Proposals) contains more information on the SNS proposals' lifecycle, the concept of are *native proposals* that all SNSs have and *custom proposals* that are SNS-specific, and an introduction to proposal topics. 
...
On a high level, a proposals can be submitted by any eligible neuron. 
Practically, there are different tools that provide an interface to users to do so.
Any tool needs to access the neuron to be able to submit a proposal - this can for example be achieved by adding the principal in the tool as a hotkey to an eligible neurons. 
We give a brief overview of some options here, but later focus on submitting proposals via the command line.
...
Each SNS also has the following native proposals for handling, that is adding and removing, custom proposals:
...
Recall that all approved SNS canister versions are stored on the NNS canister SNS-W.
...
:::warning
Because this proposal requires passing wasm, which is unwieldy to copy/paste into the command line as binary, it is recommended that developers use the specially made [`make-upgrade-canister-proposal`](https://github.com/dfinity/quill/) command in `quill sns`.
