...
See more [security best practices](/building-apps/security/overview).
...
The [SNS asset canister](/building-apps/governing-apps/managing/sns-asset-canister) is a canister used to store and retrieve static assets. A dapp controlled by an SNS may have its own associated asset canister.
...
To do this, the SNS governance needed to add a new proposal type, one that would execute the custom [`commit_proposed_batch` function](/building-apps/governing-apps/managing/sns-asset-canister#sns-genericnervoussystemfunctions).
