...
Any eligible neuron can submit a proposal. Therefore, the command to [submit a proposal](/docs/building-apps/developer-tools/quill/sns/quill-sns-make-proposal)
...
- `<PATH_TO_PEM_FILE>`: The path to the PEM file of the identity that owns the neuron that is submitting the proposal. [Learn how to generate a PEM file](/docs/building-apps/developer-tools/quill/quill-generate).
...
:::warning
Because this proposal requires passing wasm, which is unwieldy to copy/paste into the command line as binary, it is recommended that developers use the specially made [`make-upgrade-canister-proposal`](/docs/building-apps/developer-tools/quill/sns/quill-sns-make-proposal) command in `quill sns`.
...
See more [security best practices](/docs/building-apps/best-practices/security/overview).
