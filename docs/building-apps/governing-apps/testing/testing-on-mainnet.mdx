...
If you would like to deploy the test flight locally before trying the test flight on the mainnet (which requires cycles), first start a local replica:
...
When running the test flight locally, export the environment variable `IC_URL` to point to your local replica instance, e.g.,
