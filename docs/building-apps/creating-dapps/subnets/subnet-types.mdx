...
[Learn more about subnets and how they work](/docs/current/building-apps/creating-dapps/subnets/overview).
...
- [`uzr34`](https://dashboard.internetcomputer.org/subnet/uzr34-akd3s-xrdag-3ql62-ocgoh-ld2ao-tamcv-54e7j-krwgb-2gm4z-oqe) hosts the canisters for [Internet Identity](/docs/current/building-apps/authentication/overview), the [cycles ledger](/docs/current/defi/token-ledgers/cycles-ledger), the [exchange rate canister](/docs/current/concepts/system-canisters/) and the dashboard, and stores the backup threshold signature keys.
