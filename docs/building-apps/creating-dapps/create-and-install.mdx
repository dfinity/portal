...
- [What are canisters?](/docs/current/concepts/what-are-canisters)

- [Canister components](/docs/current/concepts/inside-canisters).

- [Canister lifecycle](/docs/current/concepts/canister-lifecycle).
...
The [`dfx deploy`](/docs/current/building-apps/creating-dapps/deploy/overview) command does several of these steps in the background; this guide will demonstrate how to execute each step separately. However, this is not necessary for all workflows.
...
Examples of production canisters that use Motoko include [ICDex](https://github.com/iclighthouse/ICDex) and [CycleOps](https://github.com/CycleOperators/cycles-manager). [Learn more about using Motoko](/docs/current/motoko/main/getting-started/motoko-introduction).

- **[Rust](/docs/current/building-apps/developer-tools/cdks/rust/intro-to-rust)**: Supported by [DFINITY](https://github.com/dfinity/cdk-rs). Currently, Rust is the language with the most production coverage for ICP applications.
...
[Learn more about using Rust](/docs/current/building-apps/developer-tools/cdks/rust/intro-to-rust).
...
The [HTTP Gateway protocol](/docs/current/specifications/http-gateway-protocol-spec) of ICP allows browsers to load web assets such as JS, HTML, and CSS from a canister via HTTP.
This means that [web assets can be stored fully onchain](/docs/current/building-apps/dapp-frontends/using-an-asset-canister) and developers don’t need to use traditional centralized web hosting to serve the UI of their application.
...
Having no frontend at all is also a valid option for canisters that don’t have a UI and are callable only by users or other canisters. [Learn more](/docs/current/building-apps/dapp-frontends/using-an-asset-canister).
...
The [`dfx new`](/docs/current/building-apps/developer-tools/dfx/dfx-new) command creates a new project directory containing template files and a new `<project_name>` Git repository for your project.
...
Canisters are created with [`dfx canister create`](/docs/current/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-create). They are initially empty and do not contain program code. The code must be [compiled](/docs/current/building-apps/creating-dapps/create-and-install) into Wasm and [installed](/docs/current/building-apps/creating-dapps/create-and-install) into the empty canister before it can be deployed.
...
- `dfx canister create <canister-name> --network=playground`: Create a canister on the [playground](/docs/current/building-apps/creating-dapps/deploy/overview#testnets). Creating a canister on the playground is free, but canisters are temporary and will be removed after 20 minutes.

- `dfx canister create <canister-name> --network=ic`: Create a canister on the mainnet. Creating a canister on the mainnet will cost [cycles](/docs/current/concepts/gas-cost).

- `dfx canister create --all --network=ic`: Create all canisters in the project's `dfx.json` file on the mainnet.

Settings can be configured while creating a canister using optional flags. [View the full list of settings](/docs/current/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-create).
...
- [Canister not found](/docs/current/specifications/execution-errors#canister-not-found).
- [Maximum number of canisters reached](/docs/current/specifications/execution-errors#maximum-number-of-canisters-reached).
...
[`dfx build`](/docs/current/building-apps/developer-tools/dfx/dfx-build) looks for source code to compile for each canister configured under the `canisters` section in the [`dfx.json`](/docs/current/building-apps/developer-tools/dfx-json-reference) file. `dfx build` can be used to compile a specific canister or all canisters defined in the project.
...
Compiled Wasm modules must be installed into a canister using the [`dfx canister install`](/docs/current/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-install) command from the project's directory:
...
- [Install code rate limited](/docs/current/specifications/execution-errors#install-code-rate-limited).
- [Canister not empty](/docs/current/specifications/execution-errors#canister-not-empty).
- [Wasm module not found](/docs/current/specifications/execution-errors#wasm-module-not-found).
- [Wasm module too large](/docs/current/specifications/execution-errors#wasm-module-too-large).
- [Wasm module duplicate exports](/docs/current/specifications/execution-errors#wasm-module-duplicate-exports).
- [Wasm module exports too many methods](/docs/current/specifications/execution-errors#wasm-module-exports-too-many-methods).
- [Wasm module sum of exported name lengths too large](/docs/current/specifications/execution-errors#wasm-module-sum-of-exported-name-lengths-too-large).
- [Wasm module too many functions](/docs/current/specifications/execution-errors#wasm-module-too-many-functions).
- [Wasm module too many globals](/docs/current/specifications/execution-errors#wasm-module-too-many-globals).
- [Wasm module function complexity too high](/docs/current/specifications/execution-errors#wasm-module-function-complexity-too-high).
- [Wasm module function too large](/docs/current/specifications/execution-errors#wasm-module-function-too-large).
- [Wasm module code section too large](/docs/current/specifications/execution-errors#wasm-module-code-section-too-large).