...
After you have [created](/docs/current/building-apps/creating-dapps/create-and-install) a <GlossaryTooltip>canister</GlossaryTooltip>, you can deploy it to:
...
- A custom [local or playground network](/docs/current/building-apps/creating-dapps/deploy/custom-networks).

- The mainnet, designed for production use. This deployment method will cost [cycles](/docs/current/concepts/gas-cost).
...
<input type="checkbox"/> <a href="/docs/current/building-apps/creating-dapps/create-and-install">Write code.</a>
<div>
</div>
<input type="checkbox"/> <a href="/docs/current/building-apps/creating-dapps/create-and-install">Create your canister.</a> Optional: <code>dfx deploy</code> will execute this step if not already complete.
<div>
</div>
<input type="checkbox"/> <a href="/docs/current/building-apps/creating-dapps/create-and-install">Compile your canister code into Wasm.</a> Optional: <code>dfx deploy</code> will execute this step if not already complete.
<div>
</div>
<input type="checkbox"/> <a href="/docs/current/building-apps/creating-dapps/create-and-install">Install the Wasm module into your canister.</a> Optional: <code>dfx deploy</code> will execute this step if not already complete.
</TabItem>
</Tabs>
...
If you are sharing a link to a backend canister, the [Candid UI](/docs/current/building-apps/calling-dapps/candid/candid-concepts) will be displayed.
...
However, tools such as [ICP Ninja](https://icp.ninja) (a web IDE) and the [playground environment](/docs/current/building-apps/developer-tools/ide/icp-ninja) (available from the CLI) can be used as testnet-like deployment options. They do not require cycles to deploy and operate a canister, but they do have limitations, and canisters will be automatically removed after 20 minutes.
...
- [Custom playgrounds](/docs/current/building-apps/creating-dapps/deploy/custom-networks): Developers can deploy their own custom instance of the playground on the mainnet, allowing for full customization of the playground's parameters.

- [Custom `dfx` networks](/docs/current/building-apps/creating-dapps/deploy/custom-networks): The `dfx` named network feature can be used to create custom networks that can be used for testing segmented from other projects deployed locally.
