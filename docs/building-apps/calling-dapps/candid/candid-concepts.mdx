...
-   Candid is inherently a higher-order language. With Candid, you can pass more than plain data, including specifications to services and methods. Candid support for safe upgrades takes such higher-order use into account.

-   Candid has built-in support for specific Internet Computer features, such as the `query` annotation.

## Candid types and values

Candid is a strongly typed system with a set of types that canonically cover most uses. All types are described in detail in the [reference](/docs/current/specifications/candid-ref) documentation.

The philosophy behind this set of types is that they are sufficient to describe the **structure** of data so that information can be encoded, passed around, and decoded, but they intentionally do not describe **semantic** constraints beyond what’s needed to describe the representation. For example, there’s no way to express that a number should be even, that a vector has a certain length, or that the elements of a vector are sorted.
...
Depending on the language you use to implement your service, you can get the Candid service description generated from your code.
...
For examples of how to write Candid service descriptions for Rust programs, see the [Rust CDK examples](https://github.com/dfinity/cdk-rs/tree/main/examples) and the [Rust tutorials](/docs/current/building-apps/developer-tools/cdks/rust/intro-to-rust). [Learn how to generate Candid files for Rust canisters](/docs/current/building-apps/developer-tools/cdks/rust/generating-candid).

Regardless of the host language you use, it is important to know the mapping between host language types and Candid types. In the [supported types](/docs/current/specifications/candid-ref) reference section, you’ll find Candid type mapping described for Motoko, Rust, and JavaScript.
...
This annotation indicates that the `get_address` method can be invoked as an ICP **query call**. As discussed in [query and update methods](/docs/current/building-apps/calling-dapps/advanced-calls), a query provides an efficient way to retrieve information from a canister without going through consensus, so being able to identify a method as a query is one of the key benefits of using Candid to interact with ICP.
...
For information about the supertypes and subtypes of a given type, see the corresponding [reference](/docs/current/specifications/candid-ref) section for that type.
