...
See the reference on [ingress messages](/docs/current/concepts/message-execution/ingress-messages) for a more technical discussion of this topic.

## Making query calls with `dfx`

To make a query call to a canister, use the [`dfx canister call`](/docs/current/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-call) command with the `--query` flag:
...
- `dfx canister call --query <canister-name> <method_name> --network=playground`: Make a query call to a canister deployed on the [playground](/docs/current/building-apps/creating-dapps/deploy/overview#testnets). Query calls are free, but canisters are temporary and will be removed after 20 minutes.
...
```motoko no-repl file=../../specifications/samples/motoko/counter/src/Main.mo#L5-L9
```
</TabItem>

<TabItem value="rust" label="Rust">

```rust file=../../specifications/samples/rust/counter/src/lib.rs#L8-L12
```
</TabItem>
...
```motoko no-repl file=../../specifications/samples/motoko/composite_query/src/map/Map.mo
```
</TabItem>
<TabItem value="rust" label="Rust">

```rust file=../../specifications/samples/rust/composite_query/src/kv_frontend/lib.rs
```
</TabItem>
