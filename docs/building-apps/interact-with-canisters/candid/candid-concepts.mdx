...
Candid is a strongly typed system with a set of types that canonically cover most uses. All types are described in detail in the [reference](/docs/references/candid-ref) documentation.
...
For examples of how to write Candid service descriptions for Rust programs, see the [Rust CDK examples](https://github.com/dfinity/cdk-rs/tree/main/examples) and the [Rust tutorials](/docs/building-apps/developer-tools/cdks/rust/intro-to-rust). [Learn how to generate Candid files for Rust canisters](/docs/building-apps/developer-tools/cdks/rust/generating-candid).

Regardless of the host language you use, it is important to know the mapping between host language types and Candid types. In the [supported types](/docs/references/candid-ref) reference section, you’ll find Candid type mapping described for Motoko, Rust, and JavaScript.
