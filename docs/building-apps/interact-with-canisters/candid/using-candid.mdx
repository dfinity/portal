...
[Learn more in the Candid reference documentation](/references/candid-ref).
:::
...
For canisters written in Rust, the Candid extractor tool can be used with the Rust [IC CDK](https://crates.io/crates/ic-cdk) versions `0.11.0` and newer. View the full instructions for using the [Candid extractor](/building-apps/developer-tools/cdks/rust/generating-candid).
...
[Learn more about supported data types in the Candid reference documentation](/references/candid-ref#supported-types).
...
To interact with a service, use [`dfx canister call`](/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-call).

To pass Candid arguments to the `dfx canister call` command, please refer to the [Candid reference](/references/candid-ref), or, for Candid arguments too long to fit the command line, please use the `--argument-file` flag.
...
The Rust CDK then translates the Candid type into the appropriate native Rust type. This translation enables you to call the canister's method natively. For additional information on the type mapping between Candid and Rust, you can consult the [supported types](/references/candid-ref) reference section.
...
By default, `dfx` makes the `candid:service` metadata information public to promote interoperability and make it convenient to use the Candid UI. The Candid UI reads this metadata to determine the interface of your canister. It doesn’t publish any more information than what is already public. This metadata can be returned through the [`dfx canister metadata`](/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-metadata) command:
