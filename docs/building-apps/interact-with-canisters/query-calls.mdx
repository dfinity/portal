...
- `dfx canister call --query <canister-name> <method_name>`: Make a query call to a canister deployed locally. The local development environment must be running to create a canister locally. Start it with `dfx start --background`.
