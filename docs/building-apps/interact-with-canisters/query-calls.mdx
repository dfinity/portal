...
These endpoints correspond to exported Wasm functions that are annotated with [`canister_update`](/docs/building-apps/interact-with-canisters/update-calls) or `canister_query` in their names.

<div class="text--center">
<img src="/img/docs/call-canister.png" alt="Call canister" width="600"/>
</div>

ICP supports two types of calls: updates and queries. Query calls, also referred to as non-replicated queries, are executed on a single node and return a synchronous response. They do not go through consensus.
