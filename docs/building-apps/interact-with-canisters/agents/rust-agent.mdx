...
By default, the agent uses static routing, sending all API calls to a single, fixed URL. This URL can either point to:
...
In the example above, we instantiate a `DynamicRouteProvider`, an implementation of `RouteProvider`, and inject it into the agent to handle dynamic request routing. 
