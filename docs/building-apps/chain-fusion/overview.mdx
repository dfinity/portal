...
:::info
Threshold signing is also referred to as [chain-key cryptography](/docs/concepts/glossary#c).
:::
...
ICP’s decentralization is rooted in the replicated execution and state of canisters which depends on the honesty of a supermajority of subnet [nodes](/docs/building-apps/essentials/network-overview#nodes). These nodes are sourced from various identifiable and DAO-approved providers that operate in globally distributed data centers.
...
    // https://internetcomputer.org/docs/references/ic-interface-spec/#ic-bitcoin_send_transaction
