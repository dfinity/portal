---
keywords: [advanced, Chain Fusion, overview, what is chain fusion, chain, fusion, ethereum, bitcoin, multi-chain]
---
...
- Canisters can request [randomness](/docs/current/concepts/system-canisters/management-canister/#retrieving-randomness) from the protocol, which is useful in applications where fairness or unpredictability is required.

- [Timers](/docs/current/building-apps/using-network-features/periodic-tasks-timers) let canisters manage their tasks by initiating specific actions periodically without depending on any external actors.

- Full Web 2 integration allows canisters to both expose [HTTP interfaces](/docs/current/building-apps/using-network-features/using-http/https-outcalls/overview) and call [HTTP APIs hosted on the web](/docs/current/building-apps/using-network-features/using-http/https-outcalls/overview).

### Key management and signing

A key feature of the ICP’s Chain Fusion capabilities is [threshold signing](/docs/current/building-apps/using-network-features/signatures/t-ecdsa/). This feature allows canisters to control accounts on other blockchains. The private keys that are used to sign transactions are never accessible in a single physical place but are securely distributed as shares among a group of nodes performing a threshold signing protocol to generate signatures on behalf of a canister.

:::info
Threshold signing is also referred to as [chain-key cryptography](/docs/current/concepts/glossary#c).
:::
...
    // https://internetcomputer.org/docs/current/specifications/ic-interface-spec/#ic-bitcoin_send_transaction
...
- **Chain-key tokens**: Using the primitives described above, DFINITY has built canisters implementing the [ICRC-2](https://github.com/dfinity/ICRC) token standard to represent native Bitcoin, Ethereum, and ERC-20 tokens on ICP: [ckBTC](/docs/current/defi/chain-key-tokens/ckbtc/overview), [ckETH](/docs/current/defi/chain-key-tokens/cketh/overview), and [ckERC20](/docs/current/defi/chain-key-tokens/ckerc20/overview). These tokens are digital twins of their corresponding native token and are backed 1:1 by the native assets which are managed 100% onchain through a canister smart contract without any intermediaries.

- **[EVM RPC canister](/docs/current/building-apps/chain-fusion/ethereum/evm-rpc/evm-rpc-canister)**: A canister smart contract providing an onchain API for communicating with [Ethereum](https://ethereum.org/en/) or any other [EVM blockchain](https://chainlist.org/?testnets=true).

- **[Threshold signatures](/docs/current/specifications/t-sigs-how-it-works)**: An ICP service implementing the threshold ECDSA and threshold Schnorr protocols.

- **[HTTPS outcalls](/docs/current/building-apps/using-network-features/using-http/https-outcalls/overview)**: Replicated calls into external web services.

- **[Bitcoin API](/docs/current/specifications/bitcoin-how-it-works)**: A protocol API that allows canisters to interact with the Bitcoin blockchain directly.
