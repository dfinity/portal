...
1. [Generate a Bitcoin address](generate-addresses.mdx): To receive BTC, your canister must generate a Bitcoin address using the `ecdsa_public_key` API endpoint of the [management canister](/docs/current/concepts/system-canisters/management-canister#signing-and-submitting-bitcoin-transactions).
...
Chain-key Bitcoin (ckBTC) is a digital twin token of Bitcoin that is backed 1:1 with Bitcoin held within an ICP smart contract. ckBTC tokens comply with the [ICRC token standard](/docs/current/defi/chain-key-tokens/ckbtc/overview).
...
[Learn more about ckBTC](/docs/current/defi/chain-key-tokens/ckbtc/overview).
