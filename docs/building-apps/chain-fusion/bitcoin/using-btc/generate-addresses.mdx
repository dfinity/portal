...
An ECDSA public key can be retrieved using the [`ecdsa_public_key`](/docs/current/specifications/ic-interface-spec#ic-ecdsa_public_key) API. The [basic Bitcoin example](https://github.com/dfinity/examples/tree/master/motoko/basic_bitcoin)
...
A Schnorr public key can be retrieved using the [`schnorr_public_key`](/docs/current/specifications/ic-interface-spec#ic-schnorr_public_key) API. The [basic Bitcoin example](https://github.com/dfinity/examples/tree/master/rust/basic_bitcoin) also demonstrates how to generate two different types of `P2TR` addresses, an untweaked key path address and a script spend address, from a canister's public key.
...
API](/docs/current/specifications/ic-interface-spec#ic-ecdsa_public_key).

[Learn more about the `schnorr_public_key` API](/docs/current/specifications/ic-interface-spec#ic-schnorr_public_key).