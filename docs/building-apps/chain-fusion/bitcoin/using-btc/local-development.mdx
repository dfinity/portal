...
`dfx` includes support for both the [ECDSA API](/docs/current/specifications/ic-interface-spec#ic-ecdsa_public_key) and
the [Bitcoin API](/docs/current/specifications/ic-interface-spec#ic-bitcoin-api) so that you can locally test out your Bitcoin dapp before deploying it to the Internet Computer.
...
Bitcoin has different types of addresses (e.g., P2PKH, P2SH). Most of these addresses can be generated from an ECDSA public key. The example code showcases how your canister can generate a P2PKH address using the [ecdsa_public_key](/docs/current/specifications/ic-interface-spec#ic-ecdsa_public_key) API.
