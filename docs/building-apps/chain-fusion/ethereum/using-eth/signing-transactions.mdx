...
Before a transaction can be sent to the Ethereum network, it must be signed and formatted into a raw ETH transaction. Transactions are signed with [threshold ECDSA](/docs/current/building-apps/using-network-features/signatures/t-ecdsa). For this example, the transaction standard EIP1559 will be used.
...
Additional examples of signing transactions with threshold ECDSA can be found in the [threshold ECDSA documentation](/docs/current/building-apps/using-network-features/signatures/t-ecdsa).
