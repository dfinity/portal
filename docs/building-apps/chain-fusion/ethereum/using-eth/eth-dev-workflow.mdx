...
3. [Submit a transaction](./submit-transactions.mdx): Signed raw transactions can be submitted using [`ic-alloy`](https://ic-alloy.dev/getting-started.html) (recommended) or the EVM RPC canister's RPC method `eth_sendRawTransaction`. Code examples for sending transactions can be found in the [EVM RPC canister](/docs/building-apps/chain-fusion/ethereum/evm-rpc/evm-rpc-canister) documentation.
