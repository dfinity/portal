...
Calls made to the EVM RPC canister cost [cycles](/building-apps/getting-started/tokens-and-cycles). The cost per request is generally similar to making the equivalent [HTTPS outcalls](/references/https-outcalls-how-it-works#pricing) in your own canister.

Learn more about the [costs of EVM RPC requests](costs.mdx).

## Deployed canister

The EVM RPC canister runs on the 34-node [fiduciary subnet](/building-apps/developing-canisters/create) with the following principal: [`7hfb6-caaaa-aaaar-qadga-cai`](https://dashboard.internetcomputer.org/canister/7hfb6-caaaa-aaaar-qadga-cai).
