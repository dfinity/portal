...
Other RPC methods, including those specific to non-Ethereum networks, may be accessed using the canister's `multi_request` method.
...
Then start the local development environment and deploy the canister locally with a specified number of nodes (`31` for the [fiduciary subnet](/docs/building-apps/developing-canisters/create)):
...
The [`evm_rpc_client`](https://crates.io/crates/evm_rpc_client) Rust crate can be used to interact with the EVM RPC canister from Rust code. It simplifies the code necessary to perform common EVM operations and allows working with the [Alloy](https://alloy.rs/) library.
