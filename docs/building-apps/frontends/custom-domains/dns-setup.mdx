...
When using Cloudflare as DNS provider, you need to apply the following settings such that the
custom domain works as intended and does not fail to renew:

* Under SSL/TLS > Edge Certificates at the very bottom, you have to disable "Universal SSL".
* You should not proxy traffic through Cloudflare ("DNS only").
