...
By default, all canisters on the Internet Computer are accessible through the domain `icp0.io` and their canister ID. In addition to that default domain, one can also make a canister accessible under a custom domain by register the domain with the custom domains service of HTTP gateways. All you need, is to acquire a domain through any registry, such as NameCheap or GoDaddy. Then, you simply have to configure the DNS records of your domain, and the custom domains service takes care of obtaining a certificate, renewing the certificate before its expiration, and SEO.
...
```sh
curl -sL -X POST \
    https://icp0.io/custom-domains/v1/foo.bar.com | jq
```
