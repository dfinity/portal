...
- #### Step 2: Create a file named `ic-domains` in your canister under the `.well-known` directory containing the custom domain.
  ...
  Create a file named `ic-domains` within your canister's frontend assets. The `.well-known` directory must end up in your build output so it's included when the canister is deployed.

  :::caution

  **If you created your project with `dfx new`**, place the `.well-known` directory in `public/`, not `src/`. The default template uses Vite, which compiles files in `src/` during the build — they won't be served as-is. Only files in `public/` are copied unchanged to `dist/`, which is the directory the asset canister serves.

  :::

  For projects created with `dfx new`, the structure should look like this:
  ...
  For projects with a custom build setup where the asset canister serves directly from the source directory:
  ...
  By default, `dfx` ignores hidden files and directories (those starting with a dot). To ensure the `.well-known` directory and `ic-domains` file are deployed, you need a `.ic-assets.json5` file containing the following configuration:
  ...
  Place this file alongside the `.well-known` directory. If you used `dfx new`, the template already includes a `.ic-assets.json5` in the `public/` directory — add the rule above to it.
...
- #### Step 2: Create the `.ic-assets.json5` file alongside the `.well-known` directory (see [Step 2](#step-2-create-a-file-named-ic-domains-in-your-canister-under-the-well-known-directory-containing-the-custom-domain) for placement details):
...
You may need to specify a `host` in your frontend code when you are using a custom domain, as the `HttpAgent` may not be able to automatically infer the host like it can on `icp0.io` and `icp0.app`. To configure your agent, it will look something like this:
