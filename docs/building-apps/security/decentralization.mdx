...
3. **Full decentralization using a DAO**: The dapp is controlled by a decentralized governance system such as ICP's [Service Nervous System (SNS)](https://learn.internetcomputer.org/hc/en-us/articles/34084394684564-SNS-Service-Nervous-System), so that any security-sensitive changes to the canisters are only executed if the SNS community approves them collectively through a proposal voting mechanism. If an SNS is used:
    ...
    - Ensure all components of the dapp are under SNS control, including the canisters serving the web frontends; see [SNS asset canisters](/building-apps/governing-apps/managing/sns-asset-canister).
    - Consider the [SNS preparation checklist](/building-apps/governing-apps/tokenomics/sns-checklist). Important points from a security perspective are [tokenomics](/building-apps/governing-apps/tokenomics/sns-checklist#11-tokenomics-specification), [disclosing dependencies to off-chain components](/building-apps/governing-apps/tokenomics/sns-checklist#13-disclosure-of-dependencies) and [performing security reviews](/building-apps/governing-apps/tokenomics/sns-checklist#21-security-review).
...
- Use a [content security policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) to prevent scripts and other content from other origins from being loaded at all. See also [define security headers, including a content security policy (CSP)](/building-apps/security/resources#web-security).
