...
Learn more about managing memory and compute resources in the [storage](/building-apps/essentials/gas-cost#storage) and [compute](/building-apps/essentials/gas-cost#execution-and-compute-allocation) guides.
...
Some calls (update or query) might be expensive in terms of the memory or cycles they consume. For example, any function using chain-key signing or HTTPS outcalls is relatively expensive. See the [additional documentation](/building-apps/essentials/gas-cost) on cycles cost details and for other examples.
