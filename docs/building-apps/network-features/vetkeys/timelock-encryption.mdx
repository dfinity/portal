...
A special variant of [identity-based encryption (IBE)](/building-apps/network-features/vetkeys/identity-based-encryption) is called timelock encryption.  
...
Journalists or whistleblowers could ensure that compromising information in their possession is automatically published if they were to become incapacitated. The information is IBE-encrypted and stored in a dapp, which is configured to automatically decrypt and publish the information if it doesn't receive a periodic ping from the authorized user.
...
The first category is essentially the same as [IBE](/building-apps/network-features/vetkeys/identity-based-encryption), with additional restrictions on when the decryption key can be obtained by the authorized users.
...
4. Once the deadline has passed, the auction canister retrieves the decryption key, decrypts all the submitted bids, and determines the winner. From that moment on, the bids become public and the canister announces the winner.
