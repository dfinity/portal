...
Therefore, we focus on explaining how to integrate the second category of timelock encryption into a dapp. 

The code in the descriptions below is taken from the [secret-bid auction example](https://github.com/dfinity/vetkeys/tree/dd255c8fa1ec0356f9448f1728ed4d6a5b736308/examples/basic_timelock_ibe), which works as follows: 
...
4. Once the deadline is passed, the auction canister retrieves the decryption key, decrypts all the submitted bids, and determine the highest. From that moment on, the bids become public and the canister announces the winner.
...
To encrypt bids in the example, the bidder uses the auction lot id as the encryption identity (see in the `input` variable of the `vetkd_derive_key` arguments).
...
In the example, it is checked by simply comparing the stored timestamp with the timestamp of the current time.
...
```rust no-repl reference
https://github.com/dfinity/vetkeys/blob/dd255c8fa1ec0356f9448f1728ed4d6a5b736308/examples/basic_timelock_ibe/backend/src/lib.rs#L223-L299
```
