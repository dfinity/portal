...
In other example, the condition may change with time (e.g., in dead man's switch the deadline may be extended) or be more complex and depend on external factors, such as allowing decryption only when the price of a stock gets higher than some threshold, determined via HTTPS outcalls.
