...
DKMS is a higher-level library based on vetKeys. Its main goal is to provide APIs that are simple to use, especially for developers without extensive knowledge in cryptography. DKMS enables multiple users to securely generate, manage, and share cryptographic keys. Those keys can be used for encryption, decryption, or signing of data.

DKMS provides two types of APIs:
- **KeyManager** - Key derivation and sharing.
- **EncryptedMaps** - **Efficient** encryption and decryption of data using **cleartext frontend APIs** and storing the encrypted data in a **key-value store** in a canister (ICP smart contract).
