...
In the example, users authenticate using their principals, encrypt messages, and then store the ciphertexts in the canister. Afterwards, the recipient retrieves their messages and their encrypted IBE private key, decrypts and verifies the private (decryption) key, and finally decrypts the encrypted messages. In this scenario, communication and key retrieval happen in the same canister.
...
For a detailed overview of the basic flow for obtaining `vetKeys`, refer to the [API documentation](/building-apps/network-features/vetkeys/api).
