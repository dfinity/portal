...
The vetKeys feature enables IBE, allowing data to be encrypted directly to an identity, such as a principal, an Internet Identity, an email address, or even an Ethereum address.

At a high-level IBE works as follows:
...
The underlying design of the vetKD protocol is based on a threshold-variant of [Boneh-Franklin IBE](https://eprint.iacr.org/2001/090), and it incorporates the following features:
- The master public key is generated and maintained in a distributed way by a subnet of the Internet Computer.
...
- The `vetkd_derive_key` API allows to derive the user decryption key. The decryption key is securely encrypted using the user-provided transport key. 
- Canister can enforce access control, ensuring that users are only able to request keys for their own identities. 
...
In the example, the users authenticate using their principals, encrypt their messages and store the ciphertexts in the canister. 
Then, the recepient retrieves their messages and encrypted IBE private key, decrypt and verify the private key, and finally decrypt the encrypted messages.
In this setting, the communication and key retrieval happen in the same canister.
Note that it is possible to encrypt messages to recipients in another canister, e.g., if in the communication dapp the user controls their own canister with their own inbox.
...
To encrypt messages, the sender uses recipient's principal as the encryption identity (set in the `input` variable of the `vetkd_derive_key` arguments).
