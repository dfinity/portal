...
Here we use the encoding of the caller as the identity. Since this value is already implicitly authenticated using II, no further verification by the canister is required. In constrast if the canister wished to support some other sort of identity (for example an Ethereum address or an email address) it would have to validate that the caller was the valid owner of the relevant identity type. Likely this would be accomplished using HTTP outcalls.
