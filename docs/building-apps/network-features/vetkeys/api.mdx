...
In this step, the frontend generates a transport private key and passes the corresponding transport public key to the canister in order to obtain encrypted vetKeys that can only be decrypted by the frontend.
...
Once retrieved, then you must decrypt and verify the received encrypted vetKey.
