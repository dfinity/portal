...
The DKMS has two primary components: **KeyManager** for access control and VetKey derivation and **EncryptedMaps** - that buils on top of the key manager - for efficient data encryption in a key-value store. EncryptedMaps also provides a canister storage to store the encrypted data.

## Benefits of the DKMS

DKMS provides several advantages:
- Deterministic key generation from vetKeys from strong randomness
- Access sharing for keys and encrypted data
