...
# Decentralized Key Management Service (DKMS) from vetKeys
...
The derived vetKeys can be used for different purposes such as encryption, signing, and more. These keys can be shared with other users by adding corresponding user permissions in the canister. The DKMS has two primary components: access control and VetKey derivation.

## Benefits of the DKMS

DKMS provides several advantages:
- Deterministic key generation from strong randomness
- Access sharing without peer-to-peer interaction
- Access sharing without requiring the receiving party to be online
- Fetching keys on demand, reducing the risk of key compromise by eliminating the need for key persistence
