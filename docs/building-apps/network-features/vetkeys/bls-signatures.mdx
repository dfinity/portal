...
The vetKeys feature supports distributed signing using the BLS algorithm, which is widely adopted across ICP for its short, unique signatures that are easy to use in distributed systems and fast to compute.
...
In the frontend of your application, you must implement functionality that can sign messages for a caller using the signing BLS key defined by the caller's principal in the backend.
