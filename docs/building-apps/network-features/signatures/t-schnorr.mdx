...
Canisters can sign messages with threshold Schnorr without holding the Schnorr keys themselves. Each key is derived from a master key held by the dedicated threshold Schnorr [subnet](https://learn.internetcomputer.org/hc/en-us/articles/34209955782420-Subnet-Creation). When a canister needs a signature, a request is sent to the threshold Schnorr subnet that computes the signature based on the requesting canister's root key and then returns the signature to the canister. Note that, in some cases, a signature may be computed when requested and known to some (potentially malicious replicas), even though the requesting canister receives an error. Canisters should not rely on the signature not existing in these cases. The table below shows these cases when using different types of inter-canister calls.

| Response                | Bounded-wait call                                     | Unbounded-wait call                                   |
|-------------------------|-------------------------------------------------------|-------------------------------------------------------|
| Reply (no error)        | Signature returned                                    | Signature returned                                    |
| `SYS_UNKNOWN` reject    | Signature not returned, may be known to some replicas | Not possible                                          |
| `CANISTER_ERROR` reject | Signature not returned, may be known to some replicas | Signature not returned, may be known to some replicas |
| Other reject            | Signature not returned, guaranteed not to exist       | Signature not returned, guaranteed not to exist       |

Furthermore, signatures (including Ed25519) are **non-deterministic** and a different signature may be computed every time it is requested.
...
The `sign_with_schnorr` method requires cycles to be attached. [Learn more about threshold Schnorr costs](/building-apps/essentials/gas-cost).
...
The first method is to perform *offline* derivation, using one of the available libraries which implement the relevant derivation scheme. This allows derivation without having to invoke any services running on the Internet Computer. The alternative is to use *online* derivation, using a call to the management canister interface.

The following snippets demonstrate deriving Schnorr public keys without requiring management canister calls, using the `ic-pub-key` Rust and TypeScript libraries.
...
### Online Key Derivation

As an alternative to offline derivation, you can call the `schnorr_public_key` method of the IC management canister (`aaaaa-aa`). You can deploy the threshold Schnorr sample [locally](https://github.com/dfinity/examples/tree/master/motoko/threshold-schnorr) and use the Candid UI to call this method, or you can call this method programmatically using Motoko or Rust.
...
On the mainnet, the master public keys used for BIP-340 are:
...
On the mainnet, the master public keys used for Ed25519 are:
...
PocketIC uses hardcoded keys for testing purposes. For BIP-340 these are the same as those
used for ECDSA:
...
For Ed25519 the PocketIC test keys are:

- `key_1`: `db415b8eb85bd5127b0984723e0448054042cf40e7a9c262ed0cc87ecea98349`
- `test_key_1`: `6ed9121ecf701b9e301fce17d8a65214888984e8211225691b089d6b219ec144`
- `dfx_test_key`: `7124afcb1be5927cac0397a7447b9c3cda2a4099af62d9bc0a2c2fe42d33efe1`
...
- [Threshold Schnorr API](/references/ic-interface-spec#ic-schnorr_public_key).
