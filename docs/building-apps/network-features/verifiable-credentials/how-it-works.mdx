...
Verifiable credentials (VCs) on ICP rely on Internet Identity (II), which provides distinct identities that preserve unlinkability for each dapp. A dapp utilizing verifiable credentials can’t ask for a user's credentials from the [issuer](/building-apps/network-features/verifiable-credentials/overview#terminology) of the verifiable credential directly. When a [relying party](/building-apps/network-features/verifiable-credentials/overview#terminology) wants to obtain a credential from an issuer, it must request it through an [identity provider](/building-apps/network-features/verifiable-credentials/overview#terminology) such as Internet Identity.
