...
8. If the response size [exceeds the maximum response size](/building-apps/canister-management/resource-limits), the HTTP gateway fetches additional response body data through query calls.
...
DFINITY exclusively controls the HTTP gateways that serve canisters on `ic0.app` and `icp0.io`. Developers can then also configure their own [custom domains](/building-apps/frontends/custom-domains/using-custom-domains) to point at the DFINITY-controlled HTTP gateways.
...
For a more secure solution, it is possible to run your own HTTP gateway instance locally. Additional details can be found in the [HTTP gateway specification](/references/http-gateway-protocol-spec).
...
For outgoing HTTP requests, the [HTTPS outcalls](/building-apps/network-features/using-http/https-outcalls/overview) feature should be used.
...
View the [HTTP Candid interface reference](/references/http-gateway-protocol-spec#canister-http-interface) for more information.
...
- [HTTP gateway specification](/references/http-gateway-protocol-spec)

- [HTTP gateway Learn Hub](https://learn.internetcomputer.org/hc/en-us/articles/34211943471892-HTTP-Gateway-Protocol)

- [HTTPS outcalls: `GET`](/building-apps/network-features/using-http/https-outcalls/get)

- [HTTPS outcalls: `POST`](/building-apps/network-features/using-http/https-outcalls/post)

- [HTTPS outcalls: technology overview](/references/https-outcalls-how-it-works)
