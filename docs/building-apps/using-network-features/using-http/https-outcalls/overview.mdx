...
Some examples of onchain oracle-like services that obtain data from off-chain sources through HTTP outcalls include the [EVM RPC canister](/docs/current/building-apps/chain-fusion/ethereum/evm-rpc/overview), the [exchange rate canister](/docs/current/concepts/system-canisters/) and the [Orally dapp](https://kclew-uiaaa-aaaal-qbuiq-cai.ic0.app/sybil).

## How it works

The feature currently supports `GET`, `HEAD`, and `POST` methods for HTTP requests.

In order for a canister to use HTTPS outcalls, it needs to call the system API of ICP. Canisters can call into the system API by sending messages to [the ICP management canister](/docs/current/specifications/ic-interface-spec#ic-management-canister) using the identifier `"aaaaa-aa"`.

[Cycles](/docs/current/concepts/gas-cost) used to pay for the call must be explicitly transferred with the call. They are not deducted from the caller's balance implicitly.

For detailed information on how it works, please refer to the [HTTPS outcalls reference documentation](/docs/current/specifications/https-outcalls-how-it-works).

## The API for sending HTTPS outcalls

As per the [Internet Computer interface specification](/docs/current/specifications/ic-interface-spec), a canister can use the `http_request` method by [following specification](/docs/current/specifications/ic-interface-spec#ic-http_request):
...
- [Minimal sample code of making a `GET` request](/docs/current/building-apps/using-network-features/using-http/https-outcalls/get).
- [Minimal sample code of making a `POST` request](/docs/current/building-apps/using-network-features/using-http/https-outcalls/post).