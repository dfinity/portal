...
DFINITY exclusively controls the HTTP gateways that serve canisters on `ic0.app` and `icp0.io`. Developers can then also configure their own [custom domains](/docs/current/building-apps/dapp-frontends/custom-domains/using-custom-domains) to point at the DFINITY-controlled HTTP gateways.
...
8. If the response size [exceeds the maximum response size](/docs/current/building-apps/managing-dapps/resource-limits), the HTTP gateway fetches additional response body data through query calls.
...
Additional details can be found in the [HTTP gateway specification](/docs/current/specifications/http-gateway-protocol-spec).
...
For outgoing HTTP requests, the [HTTPS outcalls](/docs/current/building-apps/using-network-features/using-http/https-outcalls/overview) feature should be used.
...
View the [HTTP Candid interface reference](/docs/current/specifications/http-gateway-protocol-spec#canister-http-interface) for more information.
...
HTTP `POST` requests are used to send data to an endpoint with the intention of retaining that data. You cannot make direct `POST` calls to a canister's `http_request_update` method with HTTP clients such as curl. Instead, you can make a `POST` call to a canister's HTTP endpoint, then configure the canister's `http_request` method to [upgrade the call to `http_request_update`](/docs/current/specifications/http-gateway-protocol-spec#upgrade-to-update-calls).
...
- [HTTP gateway specification](https://internetcomputer.org/docs/current/specifications/http-gateway-protocol-spec)

- [HTTPS outcalls: `GET`](/docs/current/building-apps/using-network-features/using-http/https-outcalls/get)

- [HTTPS outcalls: `POST`](/docs/current/building-apps/using-network-features/using-http/https-outcalls/post)

- [HTTPS outcalls: technology overview](/docs/current/specifications/https-outcalls-how-it-works)
