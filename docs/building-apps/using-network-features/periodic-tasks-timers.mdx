...
- **The protocol level implementation:** The Internet Computer protocol supports one minimalistic global timer per canister via the `ic0.global_timer_set()` system API call and `canister_global_timer` handler (see the [Internet Computer interface specification](/docs/current/specifications/ic-interface-spec#timer)).
...
3. Implements the `canister_global_timer` method with the following logic:
    - For each expired task, the handler initiates a self-canister call to isolate the tasks from each other and from the library code. Note, the [normal inter-canister call costs](/docs/current/concepts/gas-cost) apply.
...
- **Self canister calls:** To isolate the tasks from each other and from the scheduling logic, the CDK timers library initiates a self canister call to execute each task. There are a few implications:
    ...
    - Normal [inter-canister call costs](/docs/current/concepts/gas-cost) apply to execute each task. Note, timers are [still more cost effective](https://github.com/dfinity/examples/tree/master/rust/periodic_tasks) than the heartbeats.
...
Once an Internet Computer canister exports the `canister_heartbeat` function, it will be called every subnet heartbeat interval (see the [Internet Computer interface specification](/docs/current/specifications/ic-interface-spec#heartbeat)).
...
Because of those limitations, in most cases, the CDK timers library for [Rust](https://crates.io/crates/ic-cdk-timers) or [Motoko](/docs/current/motoko/main/writing-motoko/timers) is a better option to schedule periodic tasks.

## Frequently asked questions

- #### Do timers support deterministic time slicing (DTS)?
Yes, as the CDK timers library initiates a self-canister call to execute each task, normal [update message instruction limits](/docs/current/building-apps/managing-dapps/resource-limits) apply with DTS enabled.
...
- **Backend tutorial:** [Using timers](/docs/current/building-apps/developer-tools/cdks/rust/timers).
