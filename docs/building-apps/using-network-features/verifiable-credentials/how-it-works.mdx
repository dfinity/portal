...
Verifiable credentials (VCs) on ICP rely on Internet Identity (II), which provides distinct identities that preserve unlinkability for each dapp. A dapp utilizing verifiable credentials can’t ask for a user's credentials from the [issuer](/docs/current/building-apps/using-network-features/verifiable-credentials/overview#terminology) of the verifiable credential directly. When a [relying party](/docs/current/building-apps/using-network-features/verifiable-credentials/overview#terminology) wants to obtain a credential from an issuer, it must request it through an [identity provider](/docs/current/building-apps/using-network-features/verifiable-credentials/overview#terminology) such as Internet Identity.
