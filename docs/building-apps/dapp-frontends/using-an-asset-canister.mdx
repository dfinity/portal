...
- Backend canisters that communicate with an external frontend application that isn't hosted on ICP. To facilitate this communication, [the ICP JavaScript agent](/docs/current/building-apps/calling-dapps/agents/javascript-agent) can be used. [Learn more about the JS request API](/docs/current/building-apps/calling-dapps/agents/javascript-agent).
...
4. Users open the application in the browser by navigating to the URL `https://<canister-id>.icp0.io` or a [custom domain](/docs/current/building-apps/dapp-frontends/custom-domains/using-custom-domains) if one has been registered for the canister.
...
6. When the JS code runs in the browser, it can call the backend canister endpoints using the [ICP JavaScript agent](/docs/current/building-apps/calling-dapps/agents/javascript-agent) library, which is analogous to `web3.js` and `ethers.js` of Ethereum.
...
All projects created with [`dfx new`](/docs/current/building-apps/developer-tools/dfx/dfx-new) have the option to include a template frontend canister containing code using one of the following frameworks: SvelteKit, React, Vue, or Vanilla JS. These templates are available in `dfx` versions 0.17.0 and newer.
...
[Learn more about the HTTP certification library](/docs/current/building-apps/using-network-features/using-http/http-certification/serving-static-assets-over-http).

## Resources

- [Asset canister architecture reference](/docs/current/specifications/asset-canister).
...
- Deploying an [existing Next.js application](/docs/current/building-apps/dapp-frontends/existing-frontend) as a frontend canister.