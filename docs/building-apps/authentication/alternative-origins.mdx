...
For this example, you will have two domains, **A** and **B**. **A** will be the derivation origin, and **B** will be the alternative domain. To help illustrate this model, consider this website, which is hosted both at `https://yourcustomdomain.com` and `https://www.yourdomain.com`.
...
On the code for the **B** origin, you need to set the `derivationOrigin` to **A**.
...
The response of this endpoint must be a valid JSON object with a `alternativeOrigins` key, containing an array of strings:
...
#### Using the asset canister

If you are using the assets canister. You'll be placing the config files in the `src/assets` directory of your frontend canister.
...
Now, all you need to do is deploy your canister.
