...
Most developers implement their frontend applications using the [Asset Canister](https://internetcomputer.org/docs/references/asset-canister). Therefore, we provide a tutorial on how to set up the alternative origins endpoint with it.

#### Adding the `ii-alternative-origins` config file

Place the config file in the `src/assets` directory of your frontend canister.
