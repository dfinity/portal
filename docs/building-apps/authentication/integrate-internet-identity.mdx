...
Note to those using a custom dfx.json configuration file. In this example, the frontend assumes that the Internet Identity canister is installed with ID [rdmx6-jaaaa-aaaaa-aaadq-cai](https://dashboard.internetcomputer.org/canister/rdmx6-jaaaa-aaaaa-aaadq-cai),
the same ID as this canister has on the mainnet. To ensure the correct Internet Identity canister ID, be sure to add `"specified_id": "rdmx6-jaaaa-aaaaa-aaadq-cai"` to your `.canisters.internet_identity` section in dfx.json.
...
```bash
# Check that all versions of @dfinity packages are the same
VERSIONS=$(npm ls | grep '@dfinity/' | awk -F@ '{print $3}')
echo "Your project has npm packages from @dfinity with the following versions (should all be the same):"
echo $VERSIONS

# Install the right version of @dfinity/auth-client
VERSION="$(echo $VERSIONS | head -n 1)"
npm install @dfinity/auth-client@^"$VERSION"
```
...
Make sure you have a local network up and running:

```
dfx start --clean --background
```

If you see the error `Error: dfx is already running`, that's fine, it just means that your local network is ready.
