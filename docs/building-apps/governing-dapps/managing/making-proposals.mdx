...
Any eligible neuron can submit a proposal. Therefore, the command to [submit a proposal](/docs/current/building-apps/developer-tools/quill/sns/quill-sns-make-proposal)
...
- `<PATH_TO_PEM_FILE>`: The path to the PEM file of the identity that owns the neuron that is submitting the proposal. [Learn how to generate a PEM file](/docs/current/building-apps/developer-tools/quill/quill-generate).
...
:::warning
Because this proposal requires passing wasm, which is unwieldy to copy/paste into the command line as binary, it is recommended that developers use the specially made [`make-upgrade-canister-proposal`](/docs/current/building-apps/developer-tools/quill/sns/quill-sns-make-proposal) command in `quill sns`.
...
See more [security best practices](/docs/current/concepts/security/overview).
...
The [SNS asset canister](/docs/current/building-apps/governing-dapps/managing/sns-asset-canister) is a canister used to store and retrieve static assets. A dapp controlled by an SNS may have its own associated asset canister.
...
To do this, the SNS governance needed to add a new proposal type, one that would execute the custom [`commit_proposed_batch` function](/docs/current/building-apps/governing-dapps/managing/sns-asset-canister#sns-genericnervoussystemfunctions).
