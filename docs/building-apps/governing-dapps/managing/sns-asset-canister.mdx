...
:::info
To deploy to the mainnet, you will need a wallet that contains cycles. [Learn more about cycles wallets](/docs/current/defi/cycles/cycles-wallet).
:::
...
The [`ExecuteGenericNervousSystemFunction`](/docs/current/building-apps/governing-dapps/managing/making-proposals#executegenericnervoussystemfunction) SNS proposal is used to execute the newly registered proposal. To submit an `ExecuteGenericNervousSystemFunction` SNS Proposal with the output from `dfx deploy FRONTEND_CANISTER_NAME --network ic --by-proposal`, see the update steps below.
