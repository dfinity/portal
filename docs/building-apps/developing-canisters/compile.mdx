...
[`dfx build`](/building-apps/developer-tools/dfx/dfx-build) looks in a project's [`dfx.json`](/building-apps/developer-tools/dfx-json-reference) to determine the source code file(s) that should be compiled for a specific canister or all canisters configured within the project.
