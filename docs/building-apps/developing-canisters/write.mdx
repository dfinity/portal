...
ICP accepts and executes canister code in the WebAssembly (Wasm) binary format. Usually, developers write code in a higher-level language and then compile the code into a Wasm binary that can be deployed into a canister.
...
A key feature of ICP is its ability to generate [onchain randomness](/docs/building-apps/network-features/randomness) using a verifiable random function (VRF) and [chain-key cryptography](https://learn.internetcomputer.org/hc/en-us/articles/34209486239252-Chain-Key-Cryptography). In each round of [consensus](https://learn.internetcomputer.org/hc/en-us/articles/34207558615956-Consensus) on a [subnet](https://learn.internetcomputer.org/hc/en-us/articles/34209955782420-Subnet-Creation), the VRF is evaluated using the number of the round as input, producing a fresh set of random bytes. Then, the bytes are used as the seed for a pseudorandom number generator (PRNG), called random tape, which uses chain-key cryptography to create a random, unique value for each canister that requests it. Through this process, it is impossible to predict future outputs.
