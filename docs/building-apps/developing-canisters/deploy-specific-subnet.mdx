...
Each subnet can communicate with other subnets and is controlled by the **root subnet**, which uses [chain-key](/references/glossary#c) cryptography to delegate its authority to the various subnets.
...
- [`uzr34`](https://dashboard.internetcomputer.org/subnet/uzr34-akd3s-xrdag-3ql62-ocgoh-ld2ao-tamcv-54e7j-krwgb-2gm4z-oqe) hosts the canisters for [Internet Identity](/building-apps/authentication/overview), the [cycles ledger](/defi/token-ledgers/cycles-ledger), the [exchange rate canister](/references/system-canisters/) and the dashboard, and stores the backup threshold signature keys.
