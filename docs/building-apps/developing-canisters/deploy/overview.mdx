...
After you have [created](/docs/building-apps/developing-canisters/what-is-a-canister) a <GlossaryTooltip>canister</GlossaryTooltip>, you can deploy it to:
...
<input type="checkbox"/> <a href="/docs/building-apps/developing-canisters/what-is-a-canister">Write code.</a>
<div>
</div>
<input type="checkbox"/> <a href="/docs/building-apps/developing-canisters/what-is-a-canister">Create your canister.</a> Optional: <code>dfx deploy</code> will execute this step if not already complete.
<div>
</div>
<input type="checkbox"/> <a href="/docs/building-apps/developing-canisters/what-is-a-canister">Compile your canister code into Wasm.</a> Optional: <code>dfx deploy</code> will execute this step if not already complete.
<div>
</div>
<input type="checkbox"/> <a href="/docs/building-apps/developing-canisters/what-is-a-canister">Install the Wasm module into your canister.</a> Optional: <code>dfx deploy</code> will execute this step if not already complete.
</TabItem>
</Tabs>
...
However, tools such as [ICP Ninja](https://icp.ninja) (a web IDE) and the [playground environment](/docs/building-apps/developer-tools/ide/icp-ninja) (available from the CLI) can be used as testnet-like deployment options. They do not require cycles to deploy and operate a canister, but they do have limitations, and canisters will be automatically removed after 20 minutes.
...
- [Custom playgrounds](/docs/building-apps/developing-canisters/create): Developers can deploy their own custom instance of the playground on the mainnet, allowing for full customization of the playground's parameters.

- [Custom `dfx` networks](/docs/building-apps/developing-canisters/create): The `dfx` named network feature can be used to create custom networks that can be used for testing segmented from other projects deployed locally.
