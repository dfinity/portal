...
The expiration time of an ingress message is specified by the [agent](/docs/current/building-apps/calling-dapps/agents/overview) making the request and can be up to 5 minutes.
...
More information regarding these restrictions can be found in the [Internet Computer interface specification](/docs/current/specifications/ic-interface-spec#system-api-module).