...
1. Command line: [`dfx canister status <canister-name>`](/docs/current/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-status). Note that this command returns the canister's settings, along with other canister status information.
2. Programmatically: Call the [`canister_status`](/docs/current/specifications/ic-interface-spec) endpoint of the IC management canister. The canister's settings will be in the `settings` field of the result.
...
```motoko no-repl file=../../specifications/samples/motoko/query_stats/src/query_stats_backend/main.mo
```

</TabItem>
<TabItem value="rust" label="Rust">

```rust file=../../specifications/samples/rust/query_stats/src/lib.rs
```
</TabItem>
...
1. Programmatically: Call the [`update_settings`](/docs/current/specifications/ic-interface-spec#ic-update_settings) endpoint of the IC management canister.
...
From within your canister, make a call to the [`update_settings`](/docs/current/specifications/ic-interface-spec#ic-update_settings) endpoint of the IC management canister.

## Controllers

The `controllers` field in a canister's settings contains a list of principals that control the canister.
Learn more about [canister control](/docs/current/building-apps/managing-dapps/control).
...
For example, an idle canister that has a compute allocation of 50% for one hour will be charged `base_fee * 50 * 3600`, where `base_fee` is the fee for [one percent per second](/docs/current/concepts/gas-cost).
...
[Learn more about payment for memory](/docs/current/concepts/gas-cost).
...
[Learn more about resource reservation](/docs/current/concepts/gas-cost).
...
- [Invalid settings](/docs/current/specifications/execution-errors#invalid-settings).