...
- Dividing by zero.
- Calling a system API in a context that is not permitted by ICP ([see a list of APIs and which contexts they are available in](/docs/current/specifications/ic-interface-spec#system-api-imports)).
- Accessing [heap data](/docs/current/building-apps/managing-dapps/storage) that is out of bounds.
...
Special care should be taken when writing a [`pre_upgrade`](/docs/current/specifications/ic-interface-spec#system-api-upgrades) hook for your canister, which will run during an upgrade against the current canister's Wasm module. If the `pre_upgrade` hook traps, then the canister upgrade will fail. This is extremely important, as it means your canister can be permanently unrecoverable.
...
- *“Unreachable”*: Typically produced when the canister panics, e.g., for Rust canisters. It's recommended to use the [panic hook](https://github.com/dfinity/cdk-rs/blob/08cbe397fcbaf62dd1edbec218492f967e80dd00/src/ic-cdk/src/printer.rs#L5) so that panics are converted to an explicit call to [ic0.trap](/docs/current/specifications/ic-interface-spec#debugging-aids) which will append an error message that will be more useful for debugging.

## Errors related to canister trapping

Common errors related to canister trapping include:

- [Trapped](/docs/current/specifications/execution-errors#trapped).
- [Trapped explicitly](/docs/current/specifications/execution-errors#trapped-explicitly).