...
Currently, there are ten canister setting fields:
...
8. `snapshot_visibility`: Controls who can read the canister's snapshots.
9. `wasm_memory_threshold`: A threshold that triggers the "on low wasm memory" hook when the canister's remaining Wasm memory falls below the provided value.
10. `environment_variables`: A set of key-value pairs that configure canister behavior without code changes.
...
The `log_visibility` field in a canister's settings controls who can access the canister's logs through the `fetch_canister_logs` endpoint of the management canister. It can take one of three possible values:
- `controllers`: Only the controllers of the canister can fetch its logs.
...
The `snapshot_visibility` field in a canister's settings controls who can access the canister's snapshots through the following management canister endpoints:
- `read_canister_snapshot_metadata`
- `read_canister_snapshot_data`
- `list_canister_snapshots`

It can take one of three possible values:
- `controllers`: Only the controllers of the canister can read its snapshots.
...
The `wasm_memory_threshold` field in a canister's settings sets a threshold on the canister's remaining Wasm memory in bytes. When the remaining Wasm memory (i.e., `wasm_memory_limit` minus current Wasm memory usage) falls below this threshold, the system schedules the execution of the ["on low wasm memory"](/references/ic-interface-spec#on-low-wasm-memory) hook.

The default value of the field is 0 bytes, which means the hook is never scheduled.

## Environment variables

The `environment_variables` field in a canister's settings is a set of key-value pairs where both keys and values are UTF-8 encoded strings. These variables are accessible to the canister during execution and can be used to configure canister behavior without code changes. Each key must be unique.
