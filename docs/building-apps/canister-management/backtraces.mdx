...
When canisters trap, ICP reports backtraces that contain the names of functions in the call stack. Backtraces are added to the canister logs and if the caller has [access to canister logs](/docs/building-apps/canister-management/logs#log-visibility) they will also be included in the message error response.
