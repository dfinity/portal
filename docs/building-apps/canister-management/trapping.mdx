...
- Dividing by zero.
- Calling a system API in a context that is not permitted by ICP ([see a list of APIs and which contexts they are available in](/references/ic-interface-spec#system-api-imports)).
- Accessing [heap data](/building-apps/canister-management/storage) that is out of bounds.
...
Special care should be taken when writing a [`pre_upgrade`](/references/ic-interface-spec#system-api-upgrades) hook for your canister, which will run during an upgrade against the current canister's Wasm module. If the `pre_upgrade` hook traps, then the canister upgrade will fail. This is extremely important, as it means your canister can be permanently unrecoverable.
