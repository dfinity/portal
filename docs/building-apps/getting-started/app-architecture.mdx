...
A default application, such as the [“Hello, world!” example](https://github.com/dfinity/portal/pull/5188) available on ICP Ninja, uses the following project composition:
...
The `dfx.json` file within the `hello` project directory defines two canisters, their types, and their source files:
...
If you are using [ICP Ninja](https://icp.ninja/), you can click the deploy button, then access the backend canister’s URL, which will provide an API interface to call the canister’s methods from your web browser.

If you are using the IC SDK from the command line, you can run the commands:
...
The `dfx canister call` command sent a call to the `hello_backend` canister that is deployed on the `--playground`. The call was sent to the `greet` method, which was given the input of "ICP." The backend canister processed the call and returned the response defined in the method.
...
3. When the project is deployed, `dfx` uses an implementation of the [asset canister](https://github.com/dfinity/sdk/blob/master/docs/design/asset-canister-interface.md) interface to compile the frontend canister. Under the hood, dfx generates boilerplate code using an API client called the [ICP JavaScript agent](/docs/building-apps/interact-with-canisters/agents/overview) that will facilitate communication between the user interface and the backend. ICP agents are libraries used to make calls to a canister's public interface.

## Backend and frontend communication​

The frontend canister is essentially a webserver hosting a website. When a user loads the application, the web browser fetches the user interface from the frontend canister. Once the frontend is loaded into the browser, the user can interact with it, triggering messages to be sent to the backend.
