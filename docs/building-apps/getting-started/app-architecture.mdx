...
1. The code first defines an [actor](/docs/motoko/fundamentals/actors-async) named `HelloWorld`. An actor is an object that can hold state and interact with the world through message passing.
