...
1. The code first defines an [actor](/motoko/actors-async) named `HelloWorld`. An actor is an object that can hold state and interact with the world through message passing.

2. Then, it defines a [stable variable](/building-apps/canister-management/upgrade) called `greeting`. Stable variables are used to store data that will persist across canister upgrades. [Learn more about upgrading canisters.](/building-apps/canister-management/upgrade)

3. Next, an [update method](/building-apps/interact-with-canisters/update-calls) called `setGreeting` is defined. Update methods alter the state of the canister. This method specifically will update the text stored in the `greeting` variable.

4. Lastly, a [query method](/building-apps/interact-with-canisters/query-calls) called `greet` is defined. Query methods are read-only operations and simply return data from the canister. They cannot alter the canister’s state. This query method will return the text stored in `greeting`, followed by the text passed to the method. This method’s syntax can be broken down and explained further:
