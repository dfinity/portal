...
ICP tokens are important because they can be converted into cycles and used to pay for a canister's resource consumption. Cycles reflect the real cost of resources such as physical hardware, rack space, energy, storage devices, and bandwidth.

## ICP tokens

ICP is the Internet Computer Protocol's native utility token. Its value is determined on the open market. ICP tokens play a key role in the governance and economics of the network.
...
- Stake ICP in a [neuron](/docs/move-to-learnhub/nns/concepts/neurons/neuron-overview) in return for network governance participation and rewards.

## Cycles

The Internet Computer Protocol runs on a network of nodes owned and operated by a community of independent node providers distributed across the globe. Node providers spend money to run and maintain nodes, such that they purchase hardware and pay for the electricity and network bandwidth used by the node. The network compensates these node providers on a monthly basis by minting and distributing rewards in the form of ICP tokens.
...
To get cycles, ICP tokens must be converted.

### Why are cycles different from ICP tokens?

While the value of an ICP token is volatile, cycles are not. Cycles are pegged to [XDR](https://en.wikipedia.org/wiki/Special_drawing_rights), a group of fiat currencies, where 1 trillion cycles equals 1 XDR. This allows resource consumption and canister execution on ICP to be a rather stable price.

## Other token types

ICP supports creating and deploying custom fungible or non-fungible tokens through the [ICRC standards](/docs/defi/token-standards/). Each ICRC standard defines a set of parameters that must be met. Not all ICRC standards are for tokens; some are for notifications, architectures, or other network functions.

## What is a ledger?

A [**ledger**](/docs/defi/overview) is a system canister used to store accounts and their transactions. Each token has its own ledger; there are separate ledgers for ICP tokens, cycles, and every ICRC token that has been deployed on the network.

[Learn more about ledgers](/docs/defi/overview).

## Obtaining cycles

You need to obtain [ICP tokens](https://internetcomputer.org/icp-tokens), then convert them into cycles using `dfx`. However, when participating at DFINITY events like [hackathons](https://dfinity.org/hackathons) or working on a [developer grant](https://dfinity.org/grants), you can receive a coupon that you can redeem to claim free cycles from the [cycles faucet](https://anv4y-qiaaa-aaaal-qaqxq-cai.ic0.app/).
...
This command must be run with the `--network=ic` flag because the cycles ledger is not part of the local replica.
