...
If you followed a [quick start](/docs/current/building-apps/getting-started/quickstart/first-smart-contract) guide, you can skip this step.
...
To customize your IC SDK installation, you can use the interactive installation prompt or set [environment variables](/docs/current/building-apps/developer-tools/dfx/dfx-envars) for a terminal session.
...
- [`dfxvm`](/docs/current/building-apps/developer-tools/dev-tools-overview#dfxvm): A version manager for `dfx` used to download a specific version of `dfx`, set a default version, and update to the latest version.
...
- [**Rust CDK**](/docs/current/building-apps/developer-tools/cdks/rust/intro-to-rust): A set of tools for developing Rust canisters.

- [**Candid**](/docs/current/building-apps/calling-dapps/candid/candid-concepts): An interface description language (IDL) used to interact with a canister's methods. A **method** is a function exposed by the canister that can be called by a user, another canister, or the application's frontend.
