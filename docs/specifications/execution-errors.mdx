...
For further information on calling canisters, see the [smart contract docs](../building-apps/calling-dapps/advanced-calls) and the [IC spec](./ic-interface-spec#system-api-requests).
...
To fix this error, consider using tools such as the [performance counter API](/docs/current/specifications/samples/rust/performance_counters/) or [canbench][canbench] to determine which sections of code are using significant instructions and optimize them.
...
When encountering an error, canisters may choose to fail with an error message by calling the [`ic0.trap` API](/docs/current/specifications/ic-interface-spec#debugging-aids). The Rust and Motoko CDKs will insert calls to this API when panicking.
...
Canisters can [exist without having Wasm code installed](/docs/current/specifications/ic-interface-spec#canister-lifecycle). A canister will have no Wasm code if it has never been installed or if it has been [uninstalled](/docs/current/specifications/ic-interface-spec#ic-uninstall_code).
...
To fix this error, consider installing code using [`dfx deploy`](/docs/current/building-apps/developer-tools/dfx/dfx-deploy), or the APIs for [`install_code`](/docs/current/specifications/ic-interface-spec#ic-install_code) or [`install_chunked_code`](/docs/current/specifications/ic-interface-spec#ic-install_chunked_code).
...
Newer versions of Motoko don't require these reserved pages, so upgrading the version of Motoko used in the canister will fix the issue. `dfx` stores the Motoko compiler (`moc`) in the directory returned by [`dfx cache show`](/docs/current/building-apps/developer-tools/dfx/dfx-cache#dfx-cache-show). So the version of Motoko can be shown by running `$(dfx cache show)/moc --version`. This issue only occurs for versions `0.6.20` and older.
...
Certain system APIs are only available in certain types of messages. For an overview of when each system API can be used, see [the ICP spec](/docs/current/specifications/ic-interface-spec#system-api-imports).
...
Method names are generally short (similar to function names), so it is likely a bug in the calling canister that creates a name that is too long. Consider running the canister locally and using debug printing ([using `Debug` in Motoko](/docs/current/motoko/main/base/Debug/) or [`println!` in Rust](https://docs.rs/ic-cdk/latest/ic_cdk/macro.println.html)) to verify that the correct method name is being called.
...
Each canister has a number of [controllers](/docs/current/building-apps/managing-dapps/control) and certain actions (e.g., updating the canister's Wasm code) can only be performed by controllers. To fix this error, consider performing the action from a [principal](/docs/current/concepts/glossary#p) that is already a controller, or have an existing controller add additional principals to the canister's list of controllers.
...
[Installing](/docs/current/building-apps/creating-dapps/create-and-install) a Wasm module on a canister is meant to be performed only when the canister has no existing state when it has first been created and therefore fails if the canister already has an installed Wasm module.

If the intention is to update the Wasm module without wiping the state, then an [upgrade](/docs/current/building-apps/managing-dapps/upgrade) should be used instead. If the intention is actually to overwrite the canister state, then the [reinstall](https://internetcomputer.org/docs/current/building-apps/managing-dapps/settings#reinstall-a-canister) mode can be used.
...
There was an error while executing a method to manipulate the canister's [Wasm chunk store](/docs/current/specifications/ic-interface-spec#ic-upload_chunk) (e.g., `upload_chunk`, `clear_chunk_store`, `install_chunked_code`, etc.).
...
To fix this error, consider [topping up][topping-up] the canister if it doesn't have enough cycles or using the [`stored_chunks` API](/docs/current/specifications/ic-interface-spec#ic-stored_chunks) to see which chunks have been uploaded if you get errors about mismatching or missing hashes.
...
[canbench]: /docs/current/building-apps/developer-tools/canbench

[canister-status-api]: /docs/current/specifications/ic-interface-spec#ic-canister_status

[compute-allocation]:/docs/current/building-apps/developer-tools/dfx/dfx-canister/#allocating-message-processing

[dashboard-subnets]: https://dashboard.internetcomputer.org/subnets

[dfx-canister-status]:/docs/current/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-status

[dfx-canister-stop]:/docs/current/building-apps/developer-tools/dfx/dfx-canister#dfx-canister-stop

[ic-wasm]: https://github.com/dfinity/ic-wasm

[limits]:/docs/current/building-apps/managing-dapps/resource-limits#resource-constraints-and-limits

[memory-allocation-setting]:/docs/current/building-apps/developer-tools/dfx/dfx-canister/#options-8
...
[topping-up]:/docs/current/building-apps/managing-dapps/topping-up/