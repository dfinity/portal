...
Candid is designed to support higher-order use cases, where a service may receive or provide specifications to other services or their methods, for example, as callbacks. The `func` type is central to this: It indicates the function’s **signature** (argument and results types, annotations), and values of this type are specifications to functions with that signature.
...
For more information about parameter naming, see [Naming arguments and results](/docs/current/building-apps/calling-dapps/candid/candid-concepts#service-naming).
...
#### Subtypes
The following modifications to a function type change it to a subtype as discussed in the rules for [service upgrades](/docs/current/building-apps/calling-dapps/candid/candid-concepts#upgrades):
...
Services may want to pass around specifications to not just individual functions (using the [`func` type](#type-func)), but specifications to whole services. In this case, Candid types can be used to declare the complete interface of such a service.

See [Candid service descriptions](/docs/current/building-apps/calling-dapps/candid/candid-concepts#candid-service-descriptions) for more details on the syntax of a service type.
...
This is exactly the same principle as discussed for upgrade rules in [service upgrades](/docs/current/building-apps/calling-dapps/candid/candid-concepts#upgrades).
