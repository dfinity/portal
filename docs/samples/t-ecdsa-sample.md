...
This guide presents a minimal example canister smart contract for showcasing the [threshold ECDSA](../developer-docs/integrations/t-ecdsa/index.md) API. 
...
```motoko
  //declare "ic" to be the management canister, which is evoked by `actor("aaaaa-aa")`. This is how you will obtain an ECDSA public key 
...
-   In the example code above, you use the bytes extracted from the `msg.caller` principal in the `derivation_path`, so that different callers of `public_key()` method of our canister will be able to get their own public keys.
...
For completeness of the example, this guide shows that the created signatures can be verified with the public key corresponding to the same canister and derivation path.
...
In this walkthrough, you deployed a sample smart contract that:
