...
This app demonstrates a real world use case for [ckBTC](https://internetcomputer.org/docs/current/building-dapps/chain-fusion/chain-key-tokens/ckbtc/overview/) on the Internet Computer. It is a simple point of sale app that allows users to charge customers using ckBTC. It also demonstrates the [HTTPS outcall](https://internetcomputer.org/docs/current/building-dapps/using-network-features/using-external-data/https-outcalls/overview) functionality by sending notifications to users when a payment is received.
