      ...
      <main className="w-full overflow-hidden">
        <AnimateSpawn variants={transitions.container}>
          <section className="max-w-page w-9/10 mx-auto relative mt-20 md:mt-40 lg:mb-30">
            ...
            >
              {filteredDfinityTools
                .slice(0, numberOfItems)
                .map((sample, index) => (
                  <div
                    className="relative rounded-xl bg-white h-full"
                    key={"dfinity" + index}
                  >
                    <img
                      src={sample.image}
                    ...
                    <div className="h-fit p-6 flex flex-col relative">
                      <div className="absolute flex h-8 -top-4 tw-title-navigation-on-page text-black capitalize px-3 py-1 rounded-full items-center bg-white-60 backdrop-blur-2xl">
                        <span>{sample.tags[0]}</span>
                      </div>
                      <span className="tw-heading-6 my-2 mr-3">
                        {sample.title}
                      </span>
                ...
                .map((sample, index) => (
                  ...
                  >
                    <div className="h-full py-6 px-4 flex flex-col relative">
                      <p className="tw-heading-5 mb-3 mx-2">{sample.title}</p>
                      ...
                      <p className="tw-paragraph mt-auto text-black-60 mb-2 mx-2 overflow-hidden">
                        {sample.body}
                      </p>
                      <div className={"flex flex-row gap-2 mt-3 mx-2"}>
                        {sample.links.map((link) => (
                          <Link
                            className="inline-flex relative border-solid border-black-30 border rounded-full p-1.5 -top-1 hover:text-white hover:border-infinite hover:bg-infinite"
                            to={link}
