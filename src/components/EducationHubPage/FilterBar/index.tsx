...
function filterCourses(
  ...
  selectedSpokenLanguages: CourseSpokenLanguage[],
  searchTerm: string
): CourseItem[] {
  ...
  if (selectedSpokenLanguages.length > 0) {
    courses = courses.filter(({ level }) =>
      spokenLanguage?.some((item) => selectedSpokenLanguages.includes(item))
    );
  }
...
function EducationHubPage() {
  ...
  const [selectedSpokenLanguages, setSelectedSpokenLanguages] = useQueryParam<
    CourseSpokenLanguage[]
