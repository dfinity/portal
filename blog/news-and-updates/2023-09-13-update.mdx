...
This week, we have a new `moc` release, version `0.10.0`, which introduces the new stable regions feature, plus we'll talk about some new resources for developers on our dev docs. 
...
A 'Region' is a new Motoko abstract data type that uses an experimental API which exposes low-level canister stable memory. `Region`s are unique since they use distinct portions of stable memory that never interfere with one another. New `Regions` can be created dynamically, and any old regions that are marked for garbage collection will be removed and reused for new `Regions` afterwards. 
...
- [Developer journey level 0](/docs/tutorials/developer-journey/level-0/index.md): Start your developer journey at the very beginning by following the level 0 module. In level 0, we cover the fundamentals of what the Internet Computer is, what terms you should be familiar with, and what tools we'll be using throughout our journey. 

- [Developer journey level 1](/docs/tutorials/developer-journey/level-1/index.md): In the next stage of the developer journey, become a space cadet by getting comfortable creating your first dapp, managing your canisters, and acquiring and using cycles.

- [Pulling canister dependencies](/docs/developer-docs/setup/pulling-canister-dependencies.md): Learn how to use the new `dfx deps` feature to pull third-party canister dependencies. 

- [Sharing links to dapps](/docs/developer-docs/production/social-sharing.md): Want to learn the best way to share your dapp with others? Check out this documentation to find out. 

- [Service Nervous System (SNS) documentation](/docs/developer-docs/integrations/sns/index.md): Over the past few months, our documentation on SNSs has been greatly expanded. New docs include a pre-flight checklist, guidelines on integrating with the ledger and index canisters, and guides for testing your SNS before deploying it. 
