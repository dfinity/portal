...
vetKeys strives to resolve this challenge, as it leverages the BLS signatures, which are the native signature scheme on ICP and can be used as cryptographic decryption keys. Since BLS signatures are computed in a distributed manner on ICP, there is no central authority that is assigning or managing keys on the behalf of users. 
