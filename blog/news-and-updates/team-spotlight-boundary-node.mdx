...
description: In this interview, we sat down with the Boundary Node team to talk about what role ICP boundary nodes play on the network, what functionalities they provide, and how developers benefit from their unique attributes.
tags: [Team spotlight]
...
**Hello everyone and welcome to this edition of the team spotlight series! In this interview, we sat down with the Boundary Node team to talk about what role ICP boundary nodes play on the network, what functionalities they provide, and how developers benefit from their unique attributes.**
...
**What is unique about ICP boundary nodes in comparison to how other chains provide a gateway to their network?**
...
*We have faced many challenges so far: we worked hard on automating processes and stabilizing the nodes, we enabled developers to set up already over 300 custom domains for their canisters. However, the biggest challenge is yet to come as we are working on the new boundary architecture that splits today’s boundary nodes into API boundary nodes and HTTP gateways.*
