...
PocketIC v4 has been release, introducing new capability with agent-based tools through HTTPS compatibility. This release also enables you to browse frontend canisters while they are running on PocketIC and make concurrent update calls!

To use the HTTPS compatibility, you need to explicitly make an instance live by calling `make_live(...)` on it which does the following:
...
- Returns a gateway URL which can then be passed to tools using HTTPS, such as agents.
