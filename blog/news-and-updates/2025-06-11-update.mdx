...
description: This week, there is a new release of Motoko, the production key release for vetKeys, and new management canister endpoints to support sleeping in update calls.
tags: [Devs]
...
Hello developers, and welcome to this week's developer weekly update! This week, there is a new release of Motoko, the production key release for vetKeys, and new management canister endpoints to support sleeping in update calls. Let's get started!
...
To support sleeping during update calls, two new management canister endpoints will be added. These endpoints are:

- `observe`: Introduces an `await` condition. This endpoint can only be called by a controller of a given canister or the canister itself. It can only be called through bounded-wait calls.

- `notify`: Used to notify callers of the `observe` endpoint of the number of pending calls with a matching canister ID and event ID. This endpoint can only be called by a controller of a given canister or the canister itself. 

[Read more about the new endpoints and view examples of how to use them](https://forum.dfinity.org/t/sleeping-in-update-calls/50345).
