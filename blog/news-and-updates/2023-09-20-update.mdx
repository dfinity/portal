...
In the latest version of the Rust CDK, the Candid export workflow has changed. In this update, there is no need to compile for WASI separately. Instead, canisters should still use the `ic_cdk::export_candid!()` function to export the canister's Candid file, then the [candid_extractor](https://github.com/dfinity/cdk-rs/tree/main/src/candid-extractor) can be used to extract the Candid from the canister's Wasm. The candid_extract for a CLI tool used to extract Candid definition from the canister's Wasm. 

Let's take a look at an example. 

To start, import and add the `export_candid` macro at the end of your `lib.rs` file:
...
Then, install the [candid_extractor](https://github.com/dfinity/cdk-rs/tree/main/src/candid-extractor):
...
In the script, update the variable `CANISTERS` with the name(s) of the canister(s) in your project using a comma separated list.
