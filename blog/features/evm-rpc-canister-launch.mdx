...
is now available for use. It is in beta, and will be while we iron out any kinks
and ensure that it meets all the demands of your dapps safely and performantly.
Give it a run and let us know if you run into any issues, by either
[opening up an issue on GitHub](https://github.com/internet-computer-protocol/evm-rpc-canister/issues/new),
or on the [Feedback Board](https://dx.internetcomputer.org).
...
<div className={"flex flex-row gap-2"}>
  ...
  <a
    ...
    href={"https://internetcomputer.org/docs/current/developer-docs/integrations/ethereum/evm-rpc"}
  >
    View Docs
  </a>
</div>
...
helps you keep your dapps more secure by automatically inferring consensus among
multiple RPC providers with each outbound request.

## The problem

Building an integration between a canister and the Ethereum network currently
requires the use of HTTPS outcalls. These calls are facilitated by a service
that requires the use of subscription-based API keys. This presents developers
with the following challenges:

### Management of API keys

RPC providers for Ethereum typically charge a subscription-based fee in order to
use their services to communicate with the Ethereum network. While some offer
free tiers, the quotas can be exhausted quickly in a production setting.

Furthermore, it is not straight forward to manage an API key (let alone
multiple) in the context of a canister. There are a number of challenges to
overcome. How do you ensure that a single developer managing API keys does not
pose a centralization risk to the users of the dapp? What if you just want to
kick the tires? What if you want to build something quickly? How do you keep
...
deciding to build something. How much engineering effort will this take? How
much do I need to pay for third-party services (e.g. Ethereum RPC providers)?
How important is trust / security to my users? If managing API keys
independently, the total cost begins to balloon as usage scales and the need for
additional layers of consensus becomes more important - the latter requires
querying multiple RPC providers, which each require an individual subscription /
...
think about when setting up your integrations with Ethereum.
...
:::info
Please ensure you [install](/docs/current/developer-docs/setup/install/) `dfx` before proceeding with the steps below.
:::
...
```json
{
  "canisters": {
    ...
    "evm_rpc_example_frontend": {
      "dependencies": ["evm_rpc"],
      "frontend": {
        "entrypoint": "src/evm_rpc_example_frontend/src/index.html"
      },
      "source": [
        "src/evm_rpc_example_frontend/assets",
        "dist/evm_rpc_example_frontend/"
      ],
      "type": "assets"
    }
  },
...
exposed mainly to give you access to any Ethereum RPC API method in case the
