...
If the query call resulted in a reply, the response is a CBOR (see [CBOR](/docs/current/specifications/ic-interface-spec#cbor)) map with the following fields:
...
- `reject_code (nat)`: The reject code (see Reject codes: /docs/current/specifications/ic-interface-spec#reject-codes).

- `reject_message (text)`: a textual diagnostic message.

- `error_code (text)`: an optional implementation-specific textual error code (see Error codes: /docs/current/specifications/ic-interface-spec#error-codes).

- `signatures ([+ node-signature])`: a list containing one node signature for the returned query response.

The query's signature (whose type is denoted as node-signature) is a [CBOR](/docs/current/specifications/ic-interface-spec#cbor) (see CBOR) map with the following fields:
